...
检测器列出了当前载具收到的所有信息，以及其源组件标识和更新频率。您可以深入查看单个消息，以获取消息ID、源组件ID以及所有单个字段的值。您也可以实时绘制字段值，从多个消息中选择多个字段以显示在两个图表中的一个。
...
4. 启用相邻的复选框，将字段添加到图表(图1显示在图2下方)。
   ...
   - 一个图表可以有多个字段和多个消息中的字段 (这些都在每个图表上列出)。包含正在绘制的字段的消息用星号高亮显示。
