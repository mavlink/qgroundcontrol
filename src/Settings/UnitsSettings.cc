...
{
    qmlRegisterUncreatableType<UnitsSettings>(
        "QGroundControl.SettingsManager", 1, 0, "UnitsSettings", "Reference only");
}

DECLARE_SETTINGSFACT_NO_FUNC(UnitsSettings, horizontalDistanceUnits)
{
    if (!_horizontalDistanceUnitsFact) {
        // Distance/Area/Speed units settings can't be loaded from json since it creates an infinite loop of meta data
        // loading.
...
DECLARE_SETTINGSFACT_NO_FUNC(UnitsSettings, areaUnits)
{
    if (!_areaUnitsFact) {
        ...
        QStringList enumStrings;
        QVariantList enumValues;
        enumStrings << "SquareFeet"
                    << "SquareMeters"
                    << "SquareKilometers"
                    << "Hectares"
                    << "Acres"
                    << "SquareMiles";
        enumValues << QVariant::fromValue(static_cast<uint32_t>(AreaUnitsSquareFeet))
...
DECLARE_SETTINGSFACT_NO_FUNC(UnitsSettings, speedUnits)
{
    if (!_speedUnitsFact) {
        ...
        QStringList enumStrings;
        QVariantList enumValues;
        enumStrings << "Feet/second"
                    << "Meters/second"
                    << "Miles/hour"
                    << "Kilometers/hour"
                    << "Knots";
        enumValues << QVariant::fromValue(static_cast<uint32_t>(SpeedUnitsFeetPerSecond))
...
DECLARE_SETTINGSFACT_NO_FUNC(UnitsSettings, temperatureUnits)
{
    if (!_temperatureUnitsFact) {
        ...
        enumStrings << "Celsius"
                    << "Farenheit";
        enumValues << QVariant::fromValue(static_cast<uint32_t>(TemperatureUnitsCelsius))
                   << QVariant::fromValue(static_cast<uint32_t>(TemperatureUnitsFarenheit));
        FactMetaData* metaData = new FactMetaData(FactMetaData::valueTypeUint32, this);
...
DECLARE_SETTINGSFACT_NO_FUNC(UnitsSettings, weightUnits)
{
    if (!_weightUnitsFact) {
        ...
        enumStrings << "Grams"
                    ...
                    << "Ounces"
                    << "Pounds";
        enumValues << QVariant::fromValue(static_cast<uint32_t>(WeightUnitsGrams))
