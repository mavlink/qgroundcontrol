...
const int RadioConfigTest::_testCenterValue
    = RadioConfigTest::_testMinValue + ((RadioConfigTest::_testMaxValue - RadioConfigTest::_testMinValue) / 2);

const struct RadioConfigTest::ChannelSettings
    RadioConfigTest::_rgChannelSettingsPX4[RadioComponentController::_chanMaxPX4]
    = {
        // Function										Min                 Max
        // # Reversed
        ...
        // Channels 5-11: Not mapped to function, Simulate invalid Min/Max, since available channel Min/Max is still
        // shown.
        ...
        // Channels 14-17: Not mapped to function, Simulate invalid Min/Max, since available channel Min/Max is still
        // shown
        {RadioComponentController::rcCalFunctionMax, _testCenterValue, _testCenterValue, 0, false},
        {RadioComponentController::rcCalFunctionMax, _testCenterValue, _testCenterValue, 0, false},
        {RadioComponentController::rcCalFunctionMax, _testCenterValue, _testCenterValue, 0, false},
...
// Note the: 1500/*RadioComponentController::_rcCalPWMCenterPoint*/ entries. For some reason I couldn't get the compiler
// to do the right thing with the constant. So I just hacked inthe real value instead of fighting with it any longer.
const struct RadioConfigTest::ChannelSettings
    RadioConfigTest::_rgChannelSettingsValidatePX4[RadioComponentController::_chanMaxPX4]
    = {
        // Function										Min Value
        // Max Value									Trim Value
        // Reversed

        // Channels 0: not mapped and should be set to defaults
        {RadioComponentController::rcCalFunctionMax, RadioComponentController::_rcCalPWMDefaultMinValue,
            RadioComponentController::_rcCalPWMDefaultMaxValue, 1500 /*RadioComponentController::_rcCalPWMCenterPoint*/,
            false},
...
const struct RadioConfigTest::ChannelSettings
    RadioConfigTest::_rgChannelSettingsAPM[RadioComponentController::_chanMaxAPM]
    = {
        // Function										Min                 Max
        // # Reversed
...
// to do the right thing with the constant. So I just hacked inthe real value instead of fighting with it any longer.
const struct RadioConfigTest::ChannelSettings
    RadioConfigTest::_rgChannelSettingsValidateAPM[RadioComponentController::_chanMaxAPM]
    = {
        // Function										Min Value
        // Max Value									Trim Value
        // Reversed
...
void RadioConfigTest::_init(MAV_AUTOPILOT firmwareType)
{
    ...
    // This test is so quick that it tends to finish before the mission item protocol completes. This causes an error to
    // pop up. So we wait a little to let mission items sync.
    QTest::qWait(500);
    ...
    for (const QVariant& varVehicleComponent : _autopilot->vehicleComponents()) {
        if (firmwareType == MAV_AUTOPILOT_PX4) {
            PX4RadioComponent* radioComponent
                = qobject_cast<PX4RadioComponent*>(varVehicleComponent.value<VehicleComponent*>());
            if (radioComponent) {
                vehicleComponent = radioComponent;
                break;
            }
        } else {
            APMRadioComponent* radioComponent
                = qobject_cast<APMRadioComponent*>(varVehicleComponent.value<VehicleComponent*>());
            if (radioComponent) {
                vehicleComponent = radioComponent;
                break;
            }
...
void RadioConfigTest::_stickMoveAutoStep(const char* functionStr,
    enum RadioComponentController::rcCalFunctions function, enum RadioConfigTest::MoveToDirection direction,
    bool identifyStep)
{
    Q_UNUSED(functionStr);
    qCDebug(RadioConfigTestLog) << "_stickMoveAutoStep function:direction:reversed:identifyStep" << functionStr
                                << function << direction << identifyStep;
    ...
    if (!identifyStep && direction != moveToCenter) {
        // We have already identified the function channel mapping. Move other channels around to make sure there is no
        // impact.
...
void RadioConfigTest::_fullCalibrationWorker(MAV_AUTOPILOT firmwareType)
{
    ...
    for (int function = 0; function < RadioComponentController::rcCalFunctionMax; function++) {
        ...
        if (!found) {
            ...
            if (paramName) {
                _rgFunctionChannelMap[function] = _vehicle->parameterManager()
                                                      ->getParameter(FactSystem::defaultComponentId, paramName)
                                                      ->rawValue()
                                                      .toInt();
    ...
    _beginCalibration();
    _stickMoveAutoStep(
        "Throttle", RadioComponentController::rcCalFunctionThrottle, moveToMax, true /* identify step */);
    _stickMoveAutoStep(
        "Throttle", RadioComponentController::rcCalFunctionThrottle, moveToMin, false /* not identify step */);
    ...
    _stickMoveAutoStep(
        "Pitch", RadioComponentController::rcCalFunctionPitch, moveToCenter, false /* not identify step */);
    _switchMinMaxStep();
...
void RadioConfigTest::_fullCalibration_px4_test(void) { _fullCalibrationWorker(MAV_AUTOPILOT_PX4); }

void RadioConfigTest::_fullCalibration_apm_test(void) { _fullCalibrationWorker(MAV_AUTOPILOT_ARDUPILOTMEGA); }
...
void RadioConfigTest::_channelHomePosition(void)
{
    ...
    // Throttle to min - 1 (throttle is not reversed). We do this so that the trim value is below the min value. This
    // should end up being validated and raised to min value. If not, something is wrong with RC Cal code.
    _mockLink->emitRemoteControlChannelRawChanged(
        _rgFunctionChannelMap[RadioComponentController::rcCalFunctionThrottle], _testMinValue - 1);
}
...
void RadioConfigTest::_validateParameters(void)
{
    ...
    for (int chanFunction = 0; chanFunction < RadioComponentController::rcCalFunctionMax; chanFunction++) {
        ...
        if (paramName) {
            qCDebug(RadioConfigTestLog) << "Validate" << chanFunction
                                        << _vehicle->parameterManager()
                                               ->getParameter(FactSystem::defaultComponentId, paramName)
                                               ->rawValue()
                                               .toInt();
            QCOMPARE(_vehicle->parameterManager()
                         ->getParameter(FactSystem::defaultComponentId, paramName)
                         ->rawValue()
                         .toInt(),
    ...
    for (int chan = 0; chan < _chanMax(); chan++) {
        ...
        int rcMinActual = _vehicle->parameterManager()
                              ->getParameter(FactSystem::defaultComponentId, minTpl.arg(oneBasedChannel))
                              ->rawValue()
                              .toInt(&convertOk);
        QCOMPARE(convertOk, true);
        int rcMaxActual = _vehicle->parameterManager()
                              ->getParameter(FactSystem::defaultComponentId, maxTpl.arg(oneBasedChannel))
                              ->rawValue()
                              .toInt(&convertOk);
        QCOMPARE(convertOk, true);
        int rcTrimActual = _vehicle->parameterManager()
                               ->getParameter(FactSystem::defaultComponentId, trimTpl.arg(oneBasedChannel))
                               ->rawValue()
                               .toInt(&convertOk);
        QCOMPARE(convertOk, true);
        ...
        if (_vehicle->px4Firmware()) {
            float rcReversedFloat = _vehicle->parameterManager()
                                        ->getParameter(FactSystem::defaultComponentId, revTplPX4.arg(oneBasedChannel))
                                        ->rawValue()
                                        .toFloat(&convertOk);
            QCOMPARE(convertOk, true);
            rcReversedActual = (rcReversedFloat == -1.0f);
        } else {
            rcReversedActual = _vehicle->parameterManager()
                                   ->getParameter(FactSystem::defaultComponentId, revTplAPM.arg(oneBasedChannel))
                                   ->rawValue()
                                   .toBool();
        }

        qCDebug(RadioConfigTestLog) << "_validateParameters expected channel:min:max:trim:rev" << chan << rcMinExpected
                                    << rcMaxExpected << rcTrimExpected << rcReversedExpected;
        qCDebug(RadioConfigTestLog) << "_validateParameters actual channel:min:max:trim:rev" << chan << rcMinActual
                                    << rcMaxActual << rcTrimActual << rcReversedActual;
...
bool RadioConfigTest::_px4Vehicle(void) const { return _vehicle->firmwareType() == MAV_AUTOPILOT_PX4; }
