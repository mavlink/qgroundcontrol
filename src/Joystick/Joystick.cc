...
const char* Joystick::_rgFunctionSettingsKey[Joystick::maxFunction]
    = {"RollAxis", "PitchAxis", "YawAxis", "ThrottleAxis", "GimbalPitchAxis", "GimbalYawAxis"};
...
Joystick::Joystick(
    const QString& name, int axisCount, int buttonCount, int hatCount, MultiVehicleManager* multiVehicleManager)
    : _name(name)
...
void Joystick::_loadSettings()
{
    ...
    if (_txModeSettingsKey && activeVehicle)
        _transmitterMode
            = settings.value(_txModeSettingsKey, activeVehicle->firmwarePlugin()->defaultJoystickTXMode()).toInt();
    ...
    _throttleMode
        = static_cast<ThrottleMode_t>(settings.value(_throttleModeSettingsKey, ThrottleModeDownZero).toInt(&convertOk));
    badSettings |= !convertOk;

    qCDebug(JoystickLog) << "_loadSettings calibrated:txmode:throttlemode:exponential:deadband:badsettings"
                         << _calibrated << _transmitterMode << _throttleMode << _exponential << _deadband
                         << badSettings;
    ...
    for (int axis = 0; axis < _axisCount; axis++) {
        ...
        qCDebug(JoystickLog) << "_loadSettings axis:min:max:trim:reversed:deadband:badsettings" << axis
                             << calibration->min << calibration->max << calibration->center << calibration->reversed
                             << calibration->deadband << badSettings;
    }
...
void Joystick::_saveButtonSettings()
{
    ...
    for (int button = 0; button < _totalButtonCount; button++) {
        if (_buttonActionArray[button]) {
            ...
            qCDebug(JoystickLog) << "_saveButtonSettings button:action" << button << _buttonActionArray[button]->action
                                 << _buttonActionArray[button]->repeat;
        }
    }
}
...
void Joystick::_saveSettings()
{
    ...
    qCDebug(JoystickLog) << "_saveSettings calibrated:throttlemode:deadband:txmode" << _calibrated << _throttleMode
                         << _deadband << _circleCorrection << _transmitterMode;
    ...
    for (int axis = 0; axis < _axisCount; axis++) {
        ...
        qCDebug(JoystickLog) << "_saveSettings name:axis:min:max:trim:reversed:deadband" << _name << axis
                             << calibration->min << calibration->max << calibration->center << calibration->reversed
    ...
    for (int function = 0; function < maxFunction; function++) {
        ...
        qCDebug(JoystickLog) << "_saveSettings name:function:axis" << _name << function
                             << _rgFunctionSettingsKey[function];
    }
    _saveButtonSettings();
}
...
void Joystick::run()
{
    ...
    while (!_exitThread) {
        ...
        _handleAxis();
        QGC::SLEEP::msleep(
            qMin(static_cast<int>(1000.0f / _maxAxisFrequencyHz), static_cast<int>(1000.0f / _maxButtonFrequencyHz))
            / 2);
    }
    _close();
}
                    ...
                    if (_rgButtonValues[buttonIndex] == BUTTON_DOWN) {
                        ...
                        for (int multiIndex = 0; multiIndex < _totalButtonCount; multiIndex++) {
                            if (multiIndex != buttonIndex) {
                                if (_buttonActionArray[multiIndex]
                                    && _buttonActionArray[multiIndex]->action == buttonAction) {
                                    // We found a multi-button action
                                    if (_rgButtonValues[multiIndex] == BUTTON_DOWN
                                        || _rgButtonValues[multiIndex] == BUTTON_REPEAT) {
                                        // So far so good
...
void Joystick::_handleAxis()
{
    ...
    if (_axisTime.elapsed() > axisDelay) {
        ...
        if (_activeVehicle->joystickEnabled() && !_calibrationMode && _calibrated) {
            ...
            float throttle = _adjustRange(
                _rgAxisValues[axis], _rgCalibration[axis], _throttleMode == ThrottleModeDownZero ? false : _deadband);
            ...
            qCDebug(JoystickValuesLog) << "name:roll:pitch:yaw:throttle:gimbalPitch:gimbalYaw" << name() << roll
                                       << -pitch << yaw << throttle << gimbalPitch << gimbalYaw;
...
int Joystick::throttleMode() { return _throttleMode; }
...
bool Joystick::negativeThrust() const { return _negativeThrust; }
...
float Joystick::exponential() const { return _exponential; }
...
bool Joystick::accumulator() const { return _accumulator; }
...
bool Joystick::deadband() const { return _deadband; }
...
bool Joystick::circleCorrection() const { return _circleCorrection; }
...
void Joystick::_executeButtonAction(const QString& action, bool buttonDown)
{
    ...
    if (action == _buttonActionArm) {
        ...
        || _activeVehicle->extraJoystickFlightModes().contains(action)) {
        if (buttonDown)
