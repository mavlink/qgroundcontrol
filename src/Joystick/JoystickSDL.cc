...
QMap<QString, Joystick*> JoystickSDL::discover()
{
    ...
    for (int n = 0; n < count; ++n) {
        ...
        const char *rawName = SDL_GetJoystickNameForID(jid);
        QString name = rawName ? QString::fromUtf8(rawName) : QStringLiteral("Unknown");

        if (previous.contains(name)) {
            JoystickSDL *js = static_cast<JoystickSDL*>(previous.take(name));
            if (js && (js->instanceId() != jid)) {
                js->setInstanceId(jid);
            }
            (void) current.insert(name, js);
            continue;
        }
        ...
        if ((axisCount < 0) || (buttonCount < 0) || (hatCount < 0)) {
            qCWarning(JoystickSDLLog) << "Failed to query joystick capabilities:" << SDL_GetError();
            continue;
        }
        ...
        while (current.contains(name)) {
            name = QStringLiteral("%1 %2").arg(baseName).arg(dupIdx++);
        }
...
bool JoystickSDL::_open()
{
    if (_isGamepad) {
        ...
        if (!_sdlJoystick) {
            qCWarning(JoystickSDLLog) << "SDL_GetGamepadJoystick failed:" << SDL_GetError();
