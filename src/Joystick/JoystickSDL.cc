...
///
/// Thread Safety:
/// - This cache is ONLY accessed from discover() which runs on the main thread
/// - SDL_LockJoysticks() protects the SDL device list during enumeration
/// - The cache itself is NOT thread-safe; all access must be from main thread
/// - Cleared in shutdown() when no more discoveries will occur
static QMap<QString, Joystick*> s_discoveryCache;

/// SDL event watcher callback - called from SDL's event processing thread
///
/// Thread Safety:
/// - This callback may be invoked from any thread that calls SDL_PumpEvents()
/// - In QGC, this is typically the joystick polling thread (via pumpEvents())
/// - All state modifications are done via Qt::QueuedConnection to ensure
///   they execute on the main thread where JoystickManager lives
/// - SDL_RemoveEventWatch() blocks until any active callback completes,
///   preventing races during shutdown
/// - The manager null check handles the case where the instance is being destroyed
...
QMap<QString, Joystick*> JoystickSDL::discover()
{
    ...
    // RAII lock to ensure device list doesn't change during enumeration
