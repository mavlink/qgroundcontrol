    ...
    , _currentLocale(QLocale::system())
{
    ...
    // Install event filter for signal compression (replaces deprecated compressEvent)
...
bool QGCApplication::eventFilter(QObject *watched, QEvent *event)
{
    ...
    // For Qt 6.8+, we handle signal compression through event filtering
    // instead of the deprecated compressEvent method
    if (event->type() == QEvent::MetaCall) {
        // For Qt 6.8+, QMetaCallEvent is not accessible
        // We need a different strategy - use a timer-based deduplication
