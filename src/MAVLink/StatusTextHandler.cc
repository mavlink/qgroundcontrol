...
void StatusTextHandler::_handleTextMessage(uint32_t newCount)
{
    if (newCount == 0) {
        resetAllMessages();
        return;
    ...
    const uint32_t errorCount = getErrorCount();
    const uint32_t warnCount = getWarningCount();
    const uint32_t normalCount = getNormalCount();

    MessageType type = newCount ? m_messageType : MessageType::MessageNone;
    if (errorCount != getErrorCount()) {
        m_errorCount = errorCount;
        type = MessageType::MessageError;
    }

    if (warnCount != getWarningCount()) {
        m_warningCount = warnCount;
        if (!messageTypeError()) {
            type = MessageType::MessageWarning;
        }
    }

    if (normalCount != getNormalCount()) {
        m_normalCount = normalCount;
        if (!messageTypeError() && !messageTypeWarning()) {
            type = MessageType::MessageNormal;
        }
    }
