...
QGC_LOGGING_CATEGORY(ParameterManagerDebugCacheFailureLog,
    "ParameterManagerDebugCacheFailureLog") // Turn on to debug parameter cache crc misses

const QHash<int, QString> _mavlinkCompIdHash {{MAV_COMP_ID_CAMERA, "Camera1"}, {MAV_COMP_ID_CAMERA2, "Camera2"},
    {MAV_COMP_ID_CAMERA3, "Camera3"}, {MAV_COMP_ID_CAMERA4, "Camera4"}, {MAV_COMP_ID_CAMERA5, "Camera5"},
    {MAV_COMP_ID_CAMERA6, "Camera6"}, {MAV_COMP_ID_SERVO1, "Servo1"}, {MAV_COMP_ID_SERVO2, "Servo2"},
    {MAV_COMP_ID_SERVO3, "Servo3"}, {MAV_COMP_ID_SERVO4, "Servo4"}, {MAV_COMP_ID_SERVO5, "Servo5"},
    {MAV_COMP_ID_SERVO6, "Servo6"}, {MAV_COMP_ID_SERVO7, "Servo7"}, {MAV_COMP_ID_SERVO8, "Servo8"},
    {MAV_COMP_ID_SERVO9, "Servo9"}, {MAV_COMP_ID_SERVO10, "Servo10"}, {MAV_COMP_ID_SERVO11, "Servo11"},
    {MAV_COMP_ID_SERVO12, "Servo12"}, {MAV_COMP_ID_SERVO13, "Servo13"}, {MAV_COMP_ID_SERVO14, "Servo14"},
    {MAV_COMP_ID_GIMBAL, "Gimbal1"}, {MAV_COMP_ID_ADSB, "ADSB"}, {MAV_COMP_ID_OSD, "OSD"}, {MAV_COMP_ID_FLARM, "FLARM"},
    {MAV_COMP_ID_GIMBAL2, "Gimbal2"}, {MAV_COMP_ID_GIMBAL3, "Gimbal3"}, {MAV_COMP_ID_GIMBAL4, "Gimbal4"},
    {MAV_COMP_ID_GIMBAL5, "Gimbal5"}, {MAV_COMP_ID_GIMBAL6, "Gimbal6"}, {MAV_COMP_ID_IMU, "IMU1"},
    {MAV_COMP_ID_IMU_2, "IMU2"}, {MAV_COMP_ID_IMU_3, "IMU3"}, {MAV_COMP_ID_GPS, "GPS1"}, {MAV_COMP_ID_GPS2, "GPS2"}};
...
void ParameterManager::_updateProgressBar(void)
{
    ...
    if (waitingWriteParamCount == 0) {
        ...
        _setLoadProgress((double)(qMax(_waitingWriteParamBatchCount - waitingWriteParamCount, 1))
            / (double)(_waitingWriteParamBatchCount + 1));
    ...
    if (waitingReadParamNameCount == 0) {
        ...
        _setLoadProgress((double)(qMax(_waitingReadParamNameBatchCount - waitingReadParamNameCount, 1))
            / (double)(_waitingReadParamNameBatchCount + 1));
        return;
    }
}
...
void ParameterManager::_handleParamValue(int componentId, QString parameterName, int parameterCount, int parameterIndex,
    MAV_PARAM_TYPE mavParamType, QVariant parameterValue)
{
    ...
    if (parameterIndex == 65535 && parameterName != "_HASH_CHECK" && _initialRequestTimeoutTimer.isActive()) {
        qCDebug(ParameterManagerVerbose1Log)
    ...
    if (!_initialLoadComplete && !_logReplay && _debugCacheCRC.contains(componentId) && _debugCacheCRC[componentId]) {
        if (_debugCacheMap[componentId].contains(parameterName)) {
            ...
            if (memcmp(cacheData, vehicleData, dataSize)) {
                qDebug() << "Cache/Vehicle values differ for name:cache:actual" << parameterName << parameterValue
                         << cacheParamTypeVal.second;
            }
    ...
    if (!_waitingReadParamIndexMap.contains(componentId)) {
        ...
        for (int waitingIndex = 0; waitingIndex < parameterCount; waitingIndex++) {
            // This will add the new component id, as well as the the new waiting index and set the retry count for that
            // index to 0
    ...
    if (_waitingReadParamIndexMap[componentId].count()) {
        qCDebug(ParameterManagerVerbose2Log)
            << _logVehiclePrefix(componentId) << "_waitingReadParamIndexMap:" << _waitingReadParamIndexMap[componentId];
    }
    if (_waitingReadParamNameMap[componentId].count()) {
        qCDebug(ParameterManagerVerbose2Log)
            << _logVehiclePrefix(componentId) << "_waitingReadParamNameMap" << _waitingReadParamNameMap[componentId];
    }
    if (_waitingWriteParamNameMap[componentId].count()) {
        qCDebug(ParameterManagerVerbose2Log)
            << _logVehiclePrefix(componentId) << "_waitingWriteParamNameMap" << _waitingWriteParamNameMap[componentId];
    }
    ...
    if (waitingReadParamNameCount) {
        qCDebug(ParameterManagerVerbose1Log)
            << _logVehiclePrefix(componentId) << "waitingReadParamNameCount:" << waitingReadParamNameCount;
    }
    ...
    if (waitingWriteParamNameCount) {
        qCDebug(ParameterManagerVerbose1Log)
            << _logVehiclePrefix(componentId) << "waitingWriteParamNameCount:" << waitingWriteParamNameCount;
    }
    ...
    if (totalWaitingParamCount) {
        ...
        if (!_mapCompId2FactMap.contains(_vehicle->defaultComponentId())) {
            ...
            qCDebug(ParameterManagerLog) << _logVehiclePrefix(
                -1) << "Restarting _waitingParamTimeoutTimer (still waiting for default component params)";
            _waitingParamTimeoutTimer.start();
        } else {
            qCDebug(ParameterManagerVerbose1Log)
...
void ParameterManager::_factRawValueUpdateWorker(
    int componentId, const QString& name, FactMetaData::ValueType_t valueType, const QVariant& rawValue)
{
...
void ParameterManager::refreshAllParameters(uint8_t componentId)
{
    ...
    if (_tryftp && (componentId == MAV_COMP_ID_ALL || componentId == MAV_COMP_ID_AUTOPILOT1)) {
        ...
        if (ftpManager->download(MAV_COMP_ID_AUTOPILOT1, "@PARAM/param.pck",
                QStandardPaths::writableLocation(QStandardPaths::TempLocation), "", false /* No filesize check */)) {
        ...
        mavlink_msg_param_request_list_pack_chan(mavlink->getSystemId(), mavlink->getComponentId(),
            sharedLink->mavlinkChannel(), &msg, _vehicle->id(), componentId);
...
bool ParameterManager::_fillIndexBatchQueue(bool waitingParamTimeout)
{
    ...
    for (int componentId : _waitingReadParamIndexMap.keys()) {
        if (_waitingReadParamIndexMap[componentId].count()) {
            qCDebug(ParameterManagerLog) << _logVehiclePrefix(componentId) << "_waitingReadParamIndexMap count"
                                         << _waitingReadParamIndexMap[componentId].count();
            qCDebug(ParameterManagerVerbose1Log) << _logVehiclePrefix(componentId) << "_waitingReadParamIndexMap"
        ...
        for (int paramIndex : _waitingReadParamIndexMap[componentId].keys()) {
            ...
            if (_disableAllRetries
                || _waitingReadParamIndexMap[componentId][paramIndex] > _maxInitialLoadRetrySingleParam) {
...
void ParameterManager::_waitingParamTimeout(void)
{
    ...
    if (!paramsRequested && !_waitingForDefaultComponent
        && !_mapCompId2FactMap.contains(_vehicle->defaultComponentId())) {
        // Initial load is complete but we still don't have any default component params. Wait one more cycle to see if
        // the any show up.
        qCDebug(ParameterManagerLog) << _logVehiclePrefix(
            -1) << "Restarting _waitingParamTimeoutTimer - still don't have default component params"
        ...
        for (int componentId : _waitingWriteParamNameMap.keys()) {
            for (const QString& paramName : _waitingWriteParamNameMap[componentId].keys()) {
                ...
                if (_waitingWriteParamNameMap[componentId][paramName] <= _maxReadWriteRetry) {
                    ...
                    qCDebug(ParameterManagerLog)
                        << _logVehiclePrefix(componentId) << "Write resend for (paramName:" << paramName
                        << "retryCount:" << _waitingWriteParamNameMap[componentId][paramName] << ")";
                    ...
                    QString errorMsg = tr("Parameter write failed: veh:%1 comp:%2 param:%3")
                                           .arg(_vehicle->id())
                                           .arg(componentId)
                                           .arg(paramName);
        ...
        for (int componentId : _waitingReadParamNameMap.keys()) {
            for (const QString& paramName : _waitingReadParamNameMap[componentId].keys()) {
                ...
                if (_waitingReadParamNameMap[componentId][paramName] <= _maxReadWriteRetry) {
                    _readParameterRaw(componentId, paramName, -1);
                    qCDebug(ParameterManagerLog)
                        << _logVehiclePrefix(componentId) << "Read re-request for (paramName:" << paramName
                        << "retryCount:" << _waitingReadParamNameMap[componentId][paramName] << ")";
                    if (++batchCount > maxBatchSize) {
                        goto Out;
                    }
                } else {
                    ...
                    QString errorMsg = tr("Parameter read failed: veh:%1 comp:%2 param:%3")
                                           .arg(_vehicle->id())
                                           .arg(componentId)
                                           .arg(paramName);
                    qCDebug(ParameterManagerLog) << errorMsg;
...
void ParameterManager::_sendParamSetToVehicle(
    int componentId, const QString& paramName, FactMetaData::ValueType_t valueType, const QVariant& value)
{
    WeakLinkInterfacePtr weakLink = _vehicle->vehicleLinkManager()->primaryLink();
    ...
    if (!weakLink.expired()) {
        ...
        mavlink_msg_param_set_encode_chan(
            _mavlink->getSystemId(), _mavlink->getComponentId(), sharedLink->mavlinkChannel(), &msg, &p);
        _vehicle->sendMessageOnLinkThreadSafe(sharedLink.get(), msg);
    }
}
...
void ParameterManager::_tryCacheHashLoad(int vehicleId, int componentId, QVariant hash_value)
{
    ...
    for (const QString& name : cacheMap.keys()) {
        ...
        if (_vehicle->compInfoManager()
                ->compInfoParam(MAV_COMP_ID_AUTOPILOT1)
                ->factMetaDataForName(name, fact_type)
                ->volatileValue()) {
    ...
    if (crc32_value == hash_value.toUInt()) {
        qCInfo(ParameterManagerLog) << "Parameters loaded from cache"
                                    << qPrintable(QFileInfo(cacheFile).absoluteFilePath());
        ...
        connect(ani, &QVariantAnimation::valueChanged, this,
            [this](const QVariant& value) { _setLoadProgress(value.toDouble()); });

        // Hide 500ms after animation finishes
        connect(ani, &QVariantAnimation::finished, this,
            [this] { QTimer::singleShot(500, [this] { _setLoadProgress(0); }); });

        ani->start(QAbstractAnimation::DeleteWhenStopped);
    } else {
        qCInfo(ParameterManagerLog) << "Parameters cache match failed"
                                    << qPrintable(QFileInfo(cacheFile).absoluteFilePath());
        ...
        if (!line.startsWith("#")) {
            ...
            if (wpParams.size() == 5) {
                if (_vehicle->id() != lineMavId) {
                    return QString("The parameters in the stream have been saved from System Id %1, but the current "
                                   "vehicle has the System Id %2.")
                        .arg(lineMavId)
                        .arg(_vehicle->id());
                }
...
void ParameterManager::writeParametersToStream(QTextStream& stream)
{
    ...
    stream << "# Version: " << _vehicle->firmwareMajorVersion() << "." << _vehicle->firmwareMinorVersion() << "."
           << _vehicle->firmwarePatchVersion() << " " << _vehicle->firmwareVersionTypeString() << "\n";
    ...
    for (int componentId : _mapCompId2FactMap.keys()) {
        for (const QString& paramName : _mapCompId2FactMap[componentId].keys()) {
            ...
            if (fact) {
                stream << _vehicle->id() << "\t" << componentId << "\t" << paramName << "\t"
                       << fact->rawValueStringFullPrecision() << "\t"
                       << QString("%1").arg(factTypeToMavType(fact->type())) << "\n";
            } else {
    ...
    if (initialLoadFailures) {
        ...
        QString errorMsg
            = tr("%1 was unable to retrieve the full set of parameters from vehicle %2. "
                 ...
                 "If you are using modified firmware, you may need to resolve any vehicle startup errors to resolve "
                 "the issue. "
...
QString ParameterManager::_remapParamNameToVersion(const QString& paramName)
{
    ...
    const FirmwarePlugin::remapParamNameMajorVersionMap_t& majorVersionRemap
        = _vehicle->firmwarePlugin()->paramNameRemapMajorVersionMap();
    ...
    for (int currentMinorVersion = _vehicle->firmwarePlugin()->remapParamNameHigestMinorVersionNumber(majorVersion);
        ...
        if (remapMinorVersion.contains(currentMinorVersion)) {
            ...
            if (remap.contains(mappedParamName)) {
                ...
                qCDebug(ParameterManagerLog) << "_remapParamNameToVersion: remapped currentMinor:from:to"
                                             << currentMinorVersion << mappedParamName << toParamName;
...
void ParameterManager::_loadOfflineEditingParams(void)
{
    ...
    while (!paramStream.atEnd()) {
        ...
        FactMetaData* factMetaData = _vehicle->compInfoManager()
                                         ->compInfoParam(defaultComponentId)
                                         ->factMetaDataForName(paramName, fact->type());
        fact->setMetaData(factMetaData);
...
QList<int> ParameterManager::componentIds(void) { return _paramCountMap.keys(); }
    ...
    while (in.status() == QDataStream::Ok) {
            ...
            if (in.status() != QDataStream::Ok) {
                if (no_of_parameters_found == num_params)
                    ...
                    qCDebug(ParameterManagerLog)
                        ...
                        << "number of parameters expected:" << num_params << "actual:" << no_of_parameters_found;
                    goto Error;
                }
            }
        }
        ...
        if (no_read != name_len) {
            qCritical(ParameterManagerLog) << "_parseParamFile: Error: Unexpected EOF while reading parameterName"
                                           << "Expected:" << name_len << "Actual:" << no_read;
            goto Error;
        }
        name_buffer[common_len + name_len] = '\0';
        QString parameterName(name_buffer);
        qCDebug(ParameterManagerVerbose2Log) << "_parseParamFile: parameter" << parameterName << "name_len" << name_len
                                             << "common_len" << common_len << "ptype" << ptype << "flags" << flags;
        ...
        if (++no_of_parameters_found > num_params) {
            qCDebug(ParameterManagerLog) << "_parseParamFile: Error: more parameters in file than expected."
                                         << "Expected:" << num_params << "Actual:" << no_of_parameters_found;
            goto Error;
        }
        ...
        if (_mapCompId2FactMap.contains(componentId) && _mapCompId2FactMap[componentId].contains(parameterName)) {
            ...
            qCDebug(ParameterManagerVerbose1Log)
                << _logVehiclePrefix(componentId) << "Adding new fact" << parameterName;

            fact = new Fact(componentId, parameterName, factType, this);
            FactMetaData* factMetaData = _vehicle->compInfoManager()
                                             ->compInfoParam(componentId)
                                             ->factMetaDataForName(parameterName, fact->type());
            fact->setMetaData(factMetaData);
