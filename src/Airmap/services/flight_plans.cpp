...
airmap::services::FlightPlans::FlightPlans(
    const std::shared_ptr<Dispatcher>& dispatcher, const std::shared_ptr<airmap::Client>& client)
...
void airmap::services::FlightPlans::for_id(const ForId::Parameters& parameters, const ForId::Callback& cb)
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->flight_plans().for_id(parameters,
...
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->flight_plans().create_by_polygon(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}
...
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->flight_plans().update(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}
...
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->flight_plans().delete_(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}

void airmap::services::FlightPlans::render_briefing(
    const RenderBriefing::Parameters& parameters, const RenderBriefing::Callback& cb)
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->flight_plans().render_briefing(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}
...
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->flight_plans().submit(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}
