...
std::shared_ptr<airmap::services::Advisory> airmap::services::Advisory::create(
...
airmap::services::Advisory::Advisory(
    const std::shared_ptr<Dispatcher>& dispatcher, const std::shared_ptr<airmap::Client>& client)
...
void airmap::services::Advisory::for_id(const ForId::Parameters& parameters, const ForId::Callback& cb)
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->advisory().for_id(parameters,
...
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->advisory().search(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}

void airmap::services::Advisory::report_weather(
    const ReportWeather::Parameters& parameters, const ReportWeather::Callback& cb)
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->advisory().report_weather(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}
