...
std::shared_ptr<airmap::services::Aircrafts> airmap::services::Aircrafts::create(
...
airmap::services::Aircrafts::Aircrafts(
    const std::shared_ptr<Dispatcher>& dispatcher, const std::shared_ptr<airmap::Client>& client)
...
void airmap::services::Aircrafts::manufacturers(
    const Manufacturers::Parameters& parameters, const Manufacturers::Callback& cb)
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->aircrafts().manufacturers(parameters,
...
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->aircrafts().models(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}
...
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->aircrafts().model_for_id(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}
