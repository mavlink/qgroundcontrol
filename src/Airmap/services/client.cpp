...
class ContextRunner {
public:
    explicit ContextRunner(const std::shared_ptr<airmap::Context>& context)
        : context_ {context}
    {
    }
    ...
    const std::shared_ptr<airmap::Context>& context() const { return context_; }
                ...
                dispatcher->dispatch_to_qt([dispatcher, configuration, parent, cr, result, cb]() {
                    if (result) {
                        cb(CreateResult {new Client {
                            std::unique_ptr<Private> {new Private {configuration, cr, dispatcher, result.value()}},
                            parent}});
                    } else {
...
airmap::services::Client::Client(std::unique_ptr<Private>&& d, QObject* parent)
    : QObject {parent}
    , d_ {std::move(d)}
{
}
...
// From airmap::Client
airmap::Authenticator& airmap::services::Client::authenticator() { return *d_->authenticator_; }

airmap::Advisory& airmap::services::Client::advisory() { return *d_->advisory_; }

airmap::Aircrafts& airmap::services::Client::aircrafts() { return *d_->aircrafts_; }

airmap::Airspaces& airmap::services::Client::airspaces() { return *d_->airspaces_; }

airmap::FlightPlans& airmap::services::Client::flight_plans() { return *d_->flight_plans_; }

airmap::Flights& airmap::services::Client::flights() { return *d_->flights_; }

airmap::Pilots& airmap::services::Client::pilots() { return *d_->pilots_; }

airmap::RuleSets& airmap::services::Client::rulesets() { return *d_->rulesets_; }

airmap::Status& airmap::services::Client::status() { return *d_->status_; }

airmap::Telemetry& airmap::services::Client::telemetry() { return *d_->telemetry_; }

airmap::Traffic& airmap::services::Client::traffic() { return *d_->traffic_; }
