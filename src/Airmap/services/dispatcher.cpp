...
airmap::services::Dispatcher::Event::Event(const std::function<void()>& task)
    : QEvent {registered_type()}
    , task_ {task}
{
}

void airmap::services::Dispatcher::Event::dispatch() { task_(); }
...
void airmap::services::Dispatcher::dispatch_to_qt(const std::function<void()>& task) { to_qt_->dispatch(task); }
