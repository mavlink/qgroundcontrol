...
std::shared_ptr<airmap::services::RuleSets> airmap::services::RuleSets::create(
...
airmap::services::RuleSets::RuleSets(
    const std::shared_ptr<Dispatcher>& dispatcher, const std::shared_ptr<airmap::Client>& client)
...
void airmap::services::RuleSets::search(const Search::Parameters& parameters, const Search::Callback& cb)
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->rulesets().search(parameters,
...
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->rulesets().for_id(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}
...
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->rulesets().fetch_rules(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}

void airmap::services::RuleSets::evaluate_rulesets(
    const EvaluateRules::Parameters& parameters, const EvaluateRules::Callback& cb)
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->rulesets().evaluate_rulesets(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}

void airmap::services::RuleSets::evaluate_flight_plan(
    const EvaluateFlightPlan::Parameters& parameters, const EvaluateFlightPlan::Callback& cb)
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->rulesets().evaluate_flight_plan(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}
