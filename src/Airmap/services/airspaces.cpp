...
std::shared_ptr<airmap::services::Airspaces> airmap::services::Airspaces::create(
...
airmap::services::Airspaces::Airspaces(
    const std::shared_ptr<Dispatcher>& dispatcher, const std::shared_ptr<airmap::Client>& client)
...
void airmap::services::Airspaces::search(const Search::Parameters& parameters, const Search::Callback& cb)
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->airspaces().search(parameters,
...
{
    dispatcher_->dispatch_to_airmap([sp = shared_from_this(), parameters, cb]() {
        sp->client_->airspaces().for_ids(parameters,
            [sp, cb](const auto& result) { sp->dispatcher_->dispatch_to_qt([sp, result, cb]() { cb(result); }); });
    });
}
