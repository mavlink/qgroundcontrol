...
public:
    ...
    AirspaceFlightAuthorization::AuthorizationStatus status() override;
...
public:
    ...
    QString flightPlanID() override
    {
        return _flight.flight_plan_id ? QString::fromStdString(_flight.flight_plan_id.get()) : QString();
    }
    QString createdTime() override;
    QString startTime() override;
    QString endTime() override;
    QDateTime qStartTime() override;
    QGeoCoordinate takeOff() override
    {
        return QGeoCoordinate(static_cast<double>(_flight.latitude), static_cast<double>(_flight.longitude));
    }
...
private:
    ...
    void _updateRulesAndFeatures(std::vector<airmap::RuleSet::Id>& rulesets,
        std::unordered_map<std::string, airmap::RuleSet::Feature::Value>& features, bool updateFeatures = false);
