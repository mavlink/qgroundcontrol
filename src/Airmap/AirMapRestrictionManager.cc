...
//-----------------------------------------------------------------------------
void AirMapRestrictionManager::setROI(const QGCGeoBoundingCube& roi, bool reset)
{
    if (qgcApp()->toolbox()->settingsManager()->airMapSettings()->enableAirspace()->rawValue().toBool()) {
        ...
        if (reset
            || (!_lastROI.isValid() || _lastROI.pointNW.distanceTo(roi.pointNW) > RESTRICTION_UPDATE_DISTANCE
                || _lastROI.pointSE.distanceTo(roi.pointSE) > RESTRICTION_UPDATE_DISTANCE)
...
//-----------------------------------------------------------------------------
void AirMapRestrictionManager::_getColor(const Airspace& airspace, QColor& color, QColor& lineColor, float& lineWidth)
{
    if (airspace.type() == Airspace::Type::airport) {
        ...
        QString classification
            = QString::fromStdString(airspace.details_for_controlled_airspace().airspace_classification).toUpper();
        if (classification == "B") {
            ...
            //-- Don't know it
            qCWarning(AirMapManagerLog) << "Unknown airspace classification:"
                                        << QString::fromStdString(
                                               airspace.details_for_controlled_airspace().airspace_classification);
...
//-----------------------------------------------------------------------------
void AirMapRestrictionManager::_requestRestrictions(const QGCGeoBoundingCube& roi)
{
    if (!_shared.client()) {
    ...
    _shared.client()->airspaces().search(params, [this, isAlive](const Airspaces::Search::Result& result) {
        if (!isAlive.lock())
            return;
        ...
        if (result) {
            ...
            for (const auto& airspace : airspaces) {
                ...
                case Geometry::Type::point: {
                    ...
                    _circles.append(new AirspaceCircularRestriction(QGeoCoordinate(point.latitude, point.longitude), 0.,
                        QString::fromStdString(airspace.id()), color, lineColor, lineWidth));
                    // TODO: radius???
                } break;
            ...
            emit error("Failed to retrieve Geofences", QString::fromStdString(result.error().message()), description);
        }
        _state = State::Idle;
    });
}
...
//-----------------------------------------------------------------------------
void AirMapRestrictionManager::_addPolygonToList(const airmap::Geometry::Polygon& polygon, const QString advisoryID,
    const QColor color, const QColor lineColor, float lineWidth)
{
    ...
    if (polygon.inner_rings.size() > 0) {
        // no need to support those (they are rare, and in most cases, there's a more restrictive polygon filling the
        // hole)
