...
//-----------------------------------------------------------------------------
AirspaceFlightAuthorization::AuthorizationStatus AirMapFlightAuthorization::status()
{
    switch (_auth.status) {
...
//-----------------------------------------------------------------------------
QString AirMapFlightInfo::createdTime()
{
    return QDateTime::fromMSecsSinceEpoch(static_cast<qint64>(airmap::milliseconds_since_epoch(_flight.created_at)))
        .toString("yyyy MM dd - hh:mm:ss");
}

//-----------------------------------------------------------------------------
QString AirMapFlightInfo::startTime()
{
...
//-----------------------------------------------------------------------------
QDateTime AirMapFlightInfo::qStartTime()
{
    return QDateTime::fromMSecsSinceEpoch(static_cast<qint64>(airmap::milliseconds_since_epoch(_flight.start_time)));
}

//-----------------------------------------------------------------------------
bool AirMapFlightInfo::active()
{
    QDateTime end
        = QDateTime::fromMSecsSinceEpoch(static_cast<qint64>(airmap::milliseconds_since_epoch(_flight.end_time)));
    QDateTime now = QDateTime::currentDateTime();
    return end > now;
}

//-----------------------------------------------------------------------------
void AirMapFlightInfo::setEndFlight(DateTime end)
{
...
//-----------------------------------------------------------------------------
QString AirMapFlightInfo::endTime()
{
    return QDateTime::fromMSecsSinceEpoch(static_cast<qint64>(airmap::milliseconds_since_epoch(_flight.end_time)))
        .toString("yyyy MM dd - hh:mm:ss");
}
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::setFlightStartTime(QDateTime start)
{
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::setFlightStartsNow(bool now)
{
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::setFlightDuration(int seconds)
{
...
//-----------------------------------------------------------------------------
QDateTime AirMapFlightPlanManager::flightStartTime() const { return _flightStartTime; }

//-----------------------------------------------------------------------------
int AirMapFlightPlanManager::flightDuration() const { return _flightDuration; }

//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::startFlightPlanning(PlanMasterController* planController)
{
    if (!_shared.client()) {
    ...
    if (!_planController) {
        ...
        connect(planController->missionController(), &MissionController::missionBoundingCubeChanged, this,
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::submitFlightPlan()
{
    if (flightPlanID().isEmpty()) {
    ...
    _shared.client()->flight_plans().submit(params, [this, isAlive](const FlightPlans::Submit::Result& result) {
        ...
        if (result) {
            ...
            emit error("Failed to submit Flight Plan", QString::fromStdString(result.error().message()), description);
            _state = State::Idle;
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::updateFlightPlan()
{
    //-- Are we enabled?
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::endFlight(QString flightID)
{
    ...
    if (_shared.pilotID().isEmpty()) {
        ...
        _shared.doRequestWithLogin([this, isAlive](const QString& login_token) {
            ...
            _shared.client()->pilots().authenticated(
                params, [this, isAlive](const Pilots::Authenticated::Result& result) {
                    if (!isAlive.lock())
                        return;
                    if (_state != State::GetPilotID)
                        return;
                    if (result) {
                        ...
                        emit error(
                            "Failed to get pilot ID", QString::fromStdString(result.error().message()), description);
                        return;
                    }
                });
        });
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::_endFlight()
{
...
//-----------------------------------------------------------------------------
bool AirMapFlightPlanManager::_collectFlightData()
{
    ...
    // altitude reference for AirMap is takeoff altitude & all altitudes provided in the bounding cube are relative to
    // takeoff already
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::_createFlightPlan()
{
    _flight.reset();
    ...
    if (_shared.pilotID().isEmpty() && !_shared.settings().userName.isEmpty()
        && !_shared.settings().password.isEmpty()) {
            ...
            _shared.client()->pilots().authenticated(
                params, [this, isAlive](const Pilots::Authenticated::Result& result) {
                    ...
                    if (result) {
                        ...
                        QString description = QString::fromStdString(
                            result.error().description() ? result.error().description().get() : "");
                        emit error("Failed to create Flight Plan", QString::fromStdString(result.error().message()),
                            description);
                        return;
                    }
                });
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::_updateRulesAndFeatures(std::vector<RuleSet::Id>& rulesets,
    std::unordered_map<std::string, RuleSet::Feature::Value>& features, bool updateFeatures)
{
                                ...
                                if (feature && feature->value().isValid()) {
                                    ...
                                    case AirspaceRuleFeature::Boolean:
                                        if (feature->value().toInt() == 0 || feature->value().toInt() == 1) {
                                            features[feature->name().toStdString()]
                                                = RuleSet::Feature::Value(feature->value().toBool());
                                        } else {
                                    ...
                                    case AirspaceRuleFeature::Float:
                                        ...
                                        if (std::isfinite(feature->value().toFloat())) {
                                            features[feature->name().toStdString()]
                                                = RuleSet::Feature::Value(feature->value().toDouble());
                                        }
                                        break;
                                    case AirspaceRuleFeature::String:
                                        ...
                                        if (!feature->value().toString().isEmpty()) {
                                            features[feature->name().toStdString()]
                                                = RuleSet::Feature::Value(feature->value().toString().toStdString());
                                        }
                                        break;
                                    default:
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::_updateFlightStartEndTime(DateTime& start_time, DateTime& end_time)
{
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::_uploadFlightPlan()
{
    ...
    _shared.doRequestWithLogin([this, isAlive](const QString& login_token) {
        ...
        params.authorization = login_token.toStdString();
        //-- Create flight plan
        _shared.client()->flight_plans().create_by_polygon(
            params, [this, isAlive](const FlightPlans::Create::Result& result) {
                if (!isAlive.lock())
                    return;
                if (_state != State::FlightUpload)
                    return;
                ...
                if (result) {
                    ...
                    QString description = QString::fromStdString(
                        result.error().description() ? result.error().description().get() : "");
                    emit error(
                        "Flight Plan creation failed", QString::fromStdString(result.error().message()), description);
                }
            });
    });
}
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::_updateFlightPlanOnTimer() { _updateFlightPlan(false); }

//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::_updateFlightPlan(bool interactive)
{
...
//-----------------------------------------------------------------------------
static bool adv_sort(QObject* a, QObject* b)
{
...
//-----------------------------------------------------------------------------
bool AirMapFlightPlanManager::_findBriefFeature(const QString& name)
{
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::_pollBriefing()
{
    ...
    params.id = flightPlanID().toStdString();
    std::weak_ptr<LifetimeChecker> isAlive(_instance);
    _shared.client()->flight_plans().render_briefing(
        params, [this, isAlive](const FlightPlans::RenderBriefing::Result& result) {
            if (!isAlive.lock())
                return;
            ...
            if (result) {
                        ...
                        for (const auto& feature : rule.features) {
                            ...
                            if (rule.status == RuleSet::Rule::Status::missing_info) {
                                if (!_findBriefFeature(pFeature->name())) {
                                    ...
                                    qCDebug(AirMapManagerLog) << "Adding briefing feature" << pFeature->name()
                                                              << pFeature->description() << pFeature->type();
                                } else {
                                    qCDebug(AirMapManagerLog)
                                        << "Skipping briefing feature duplicate" << pFeature->name()
                                        << pFeature->description() << pFeature->type();
                                }
                            }
                        }
                        //-- When a flight is first created, we send no features. That means that all "missing_info" are
                        //"relevant" features.
                        //   We keep a list of them so they will be always shown to the user even when they are no
                        //   longer "missing_info"
                ...
                if (briefing.evaluation.authorizations.size() == 0) {
                    ...
                    for (const auto& authorization : briefing.evaluation.authorizations) {
                        ...
                        qCDebug(AirMapManagerLog) << "Autorization:" << pAuth->name() << " (" << pAuth->message() << ")"
                                                  << static_cast<int>(pAuth->status());
                    ...
                    qCDebug(AirMapManagerLog)
                        << "Flight approval: accepted=" << accepted << "rejected" << rejected << "pending" << pending;
                ...
                QString description
                    = QString::fromStdString(result.error().description() ? result.error().description().get() : "");
                emit error("Brief Request failed", QString::fromStdString(result.error().message()), description);
            }
        });
}

//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::_missionChanged()
{
    //-- Are we enabled?
    if (!qgcApp()->toolbox()->settingsManager()->airMapSettings()->enableAirMap()->rawValue().toBool()) {
        return;
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::loadFlightList(QDateTime startTime, QDateTime endTime)
{
    ...
    if (!_shared.client()) {
        ...
        emit error(tr("Failed to get flight lists"), tr("AirMap application API key needed for flight lists."),
            tr("Set your AirMap application API key in Settings -> AirMap"));
        return;
    }
    if (!_shared.isLoggedIn()) {
        ...
        emit error(tr("Failed to get flight lists"), tr("AirMap username/password required for flight lists."),
            tr("Set your AirMap username/password in Settings -> AirMap"));
        return;
    }
    ...
    _rangeEnd = endTime;
    qCDebug(AirMapManagerLog) << "List flights from:" << _rangeStart.toString("yyyy MM dd - hh:mm:ss") << "to"
                              << _rangeEnd.toString("yyyy MM dd - hh:mm:ss");
    if (_shared.pilotID().isEmpty()) {
        //-- Need to get the pilot id
        qCDebug(AirMapManagerLog) << "Getting pilot ID";
        _state = State::GetPilotID;
...
//-----------------------------------------------------------------------------
void AirMapFlightPlanManager::_loadFlightList()
{
    ...
    _shared.doRequestWithLogin([this, isAlive](const QString& login_token) {
        if (!isAlive.lock())
            return;
        if (_state != State::LoadFlightList)
            return;
        ...
        _shared.client()->flights().search(params, [this, isAlive](const Flights::Search::Result& result) {
            ...
            if (result && result.value().flights.size() > 0) {
                ...
                for (const auto& flight : response.flights) {
                    ...
                    qCDebug(AirMapManagerLog)
                        << "Found:" << pFlight->flightID() << pFlight->flightPlanID() << pFlight->endTime();
                }
