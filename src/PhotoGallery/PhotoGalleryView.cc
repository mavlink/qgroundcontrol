...
void drawCameraTrigger(QPainter * painter, const QRectF & bounds, bool active)
{
    ...
    path.translate(bounds.left(), bounds.top());
    painter->fillPath(path, active ? Qt::gray : Qt::white);
    painter->setPen(QPen(Qt::black, w * 0.025, Qt::SolidLine, Qt::FlatCap, Qt::MiterJoin));
    painter->drawPath(path);
}
...
void PhotoGalleryView::paintGallery(QPainter * painter, const QSizeF & bounds, const GalleryViewState & view) const
{
    ...
    int start_index = screenToImageIndex(view, bounds, QPointF(0, 0));
    int end_index = screenToImageIndex(view, bounds, QPointF(bounds.width() - 1, bounds.height() - 1)) + 1;
    ...
    while (n < num_photos && n < end_index) {
        ...
        if (n == -1) {
            paintPhotoPlaceholder(painter, dst);
        }
        const auto & item = _model->data(PhotoGalleryModelIndex(n));
        if (item.image) {
            paintImageThumbnail(painter, *item.image, dst,
                any_selected ?
...
void PhotoGalleryView::paintPhotoPlaceholder(
    QPainter * painter, const QRectF & dst_bounds) const
{
    painter->fillRect(dst_bounds, Qt::gray);
    double camera_size = std::min(dst_bounds.width(), dst_bounds.height()) / 2;
    QRectF camera_bounds(
        dst_bounds.x() + (dst_bounds.width() - camera_size) / 2,
        dst_bounds.y() + (dst_bounds.height() - camera_size) / 2,
        camera_size, camera_size);
    drawCameraTrigger(painter, camera_bounds, true);
}
...
void PhotoGalleryView::handleFinishPhotoOperation()
{
    if (_photo_operation) {
        _photo_operation = nullptr;
        update();
    }
}
...
void PhotoGalleryView::handleLongClick(const QPointF & where)
{
    if (_view_state.mode == ViewMode::Gallery) {
        ...
        if (trashCanBounds(size()).contains(where)) {
            const auto & item = _model->data(PhotoGalleryModelIndex(_view_state.single.index));
            if (!item.id.isEmpty()) {
                _model->remove({item.id});
            }
            _view_state.mode = ViewMode::Gallery;
            update();
            return;
...
void PhotoGalleryView::triggerPhoto()
{
    if (!_trigger) {
        return;
    }
    ...
    _photo_operation = _trigger->takePhoto();
    if (!_photo_operation) {
        return;
    }
    connect(_photo_operation, &AbstractPhotoTriggerOperation::finish, this, &PhotoGalleryView::handleFinishPhotoOperation);
    update();
}
