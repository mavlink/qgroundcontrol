...
class AbstractPhotoTriggerOperation;
...
private slots:
    ...
    void handleFinishPhotoOperation();
...
private:
    ...
    /// Draw placeholder for "photo in progress" in lieu of a thumbnail picture.
    /// This is used in gallery view mode to keep the first slot "occupied"
    /// while we are taking the picture and transferring it.
    void paintPhotoPlaceholder(
    ...
    /// On-going photo operation
    ///
    /// If this is non-null, then user has clicked the control to take a picture
    /// and the operation is still in progress.
    AbstractPhotoTriggerOperation* _photo_operation = nullptr;
