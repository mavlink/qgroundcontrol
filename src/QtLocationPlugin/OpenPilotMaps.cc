...
const QString ProviderStrings::kLevelsForSigPacSpainMap[] =
    { QS("0"),         QS("1"),         QS("2"),         QS("3"),       QS("4"),
      QS("MTNSIGPAC"),
      QS("MTN2000"),   QS("MTN2000"),   QS("MTN2000"),   QS("MTN2000"), QS("MTN2000"),
      QS("MTN200"),    QS("MTN200"),    QS("MTN200"),
      QS("MTN25"),     QS("MTN25"),
      QS("ORTOFOTOS"), QS("ORTOFOTOS"), QS("ORTOFOTOS"), QS("ORTOFOTOS") };

ProviderStrings::ProviderStrings()
{
    // Google version strings
    VersionGoogleMap            = QS("m@313");
    VersionGoogleSatellite      = QS("s@177");
    VersionGoogleLabels         = QS("h@313");
    VersionGoogleTerrain        = QS("t@132,r@313");
    SecGoogleWord               = QS("Galileo");

    // Google (China) version strings
    VersionGoogleMapChina       = QS("m@132");
    VersionGoogleSatelliteChina = QS("s@71");
    VersionGoogleLabelsChina    = QS("h@132");
    VersionGoogleTerrainChina   = QS("t@125,r@132");

    // Google (Korea) version strings
    VersionGoogleMapKorea       = QS("kr1.12");
    VersionGoogleSatelliteKorea = QS("66");
    VersionGoogleLabelsKorea    = QS("kr1t.12");

    /// <summary>
    /// Google Maps API generated using http://greatmaps.codeplex.com/
    /// from http://code.google.com/intl/en-us/apis/maps/signup.html
    /// </summary>
    GoogleMapsAPIKey      = QS("ABQIAAAAWaQgWiEBF3lW97ifKnAczhRAzBk5Igf8Z5n2W3hNnMT0j2TikxTLtVIGU7hCLLHMAuAMt-BO5UrEWA");

    // Yahoo version strings
    VersionYahooMap             = QS("4.3");
    VersionYahooSatellite       = QS("1.9");
    VersionYahooLabels          = QS("4.3");

    // BingMaps
    VersionBingMaps             = QS("563");

    // YandexMap
    VersionYandexMap            = QS("2.16.0");

    /// <summary>
    /// Bing Maps Customer Identification, more info here
    /// http://msdn.microsoft.com/en-us/library/bb924353.aspx
    /// </summary>
    BingMapsClientToken = QString();
}
...
void UrlFactory::_googleVersionCompleted()
{
    ...
    QRegExp reg(QS("\"*http://mts0.google.com/vt/lyrs=m@(\\d*)"), Qt::CaseInsensitive);
    if (reg.indexIn(html) != -1) {
        QStringList gc = reg.capturedTexts();
        VersionGoogleMap = QS("m@%1").arg(gc[1]);
        VersionGoogleMapChina = VersionGoogleMap;
        VersionGoogleMapKorea = VersionGoogleMap;
    }
    reg = QRegExp(QS("\"*http://mts0.google.com/vt/lyrs=h@(\\d*)"), Qt::CaseInsensitive);
    if (reg.indexIn(html) != -1) {
        QStringList gc = reg.capturedTexts();
        VersionGoogleLabels = QS("h@%1").arg(gc[1]);
        VersionGoogleLabelsChina = VersionGoogleLabels;
        VersionGoogleLabelsKorea = VersionGoogleLabels;
    }
    reg = QRegExp(QS("\"*http://khms0.google.com/kh/v=(\\d*)"), Qt::CaseInsensitive);
    if (reg.indexIn(html) != -1) {
        QStringList gc = reg.capturedTexts();
        VersionGoogleSatellite = QS("s@") + gc[1];
...
void UrlFactory::_tryCorrectGoogleVersions()
{
    ...
    if(_network)
    {
        ...
        QString url(QS("http://maps.google.com/maps?output=classic"));
...
QString UrlFactory::makeImageUrl(const MapType &type, const QPoint& pos, const int &zoom, const QString &language)
{
    ...
    case GoogleMap:
    {
        ...
        QString server  = QS("mt");
        QString request = QS("vt");
        QString sec1    ; // after &x=...
        QString sec2    ; // after &zoom=...
        _getSecGoogleWords(pos, sec1, sec2);
        _tryCorrectGoogleVersions();
        return QS("http://%1%2.google.com/%3/lyrs=%4&hl=%5&x=%6%7&y=%8&z=%9&s=%10&scale=2").arg(server).arg(_getServerNum(pos, 4)).arg(request).arg(VersionGoogleMap).arg(language).arg(pos.x()).arg(sec1).arg(pos.y()).arg(zoom).arg(sec2);
    }
    break;
    case GoogleSatellite:
    {
        ...
        _getSecGoogleWords(pos, sec1, sec2);
        _tryCorrectGoogleVersions();
        return QS("http://%1%2.google.com/%3/lyrs=%4&hl=%5&x=%6%7&y=%8&z=%9&s=%10&scale=2").arg(server).arg(_getServerNum(pos, 4)).arg(request).arg(VersionGoogleSatellite).arg(language).arg(pos.x()).arg(sec1).arg(pos.y()).arg(zoom).arg(sec2);
    }
    break;
    case GoogleLabels:
    {
        QString server  = QS("mts");
        QString request = QS("vt");
        QString sec1    ; // after &x=...
        QString sec2    ; // after &zoom=...
        _getSecGoogleWords(pos, sec1, sec2);
        _tryCorrectGoogleVersions();
        return QS("http://%1%2.google.com/%3/lyrs=%4&hl=%5&x=%6%7&y=%8&z=%9&s=%10").arg(server).arg(_getServerNum(pos, 4)).arg(request).arg(VersionGoogleLabels).arg(language).arg(pos.x()).arg(sec1).arg(pos.y()).arg(zoom).arg(sec2);
    }
    break;
    case GoogleTerrain:
    {
        ...
        _getSecGoogleWords(pos, sec1, sec2);
        _tryCorrectGoogleVersions();
        return QS("http://%1%2.google.com/%3/v=%4&hl=%5&x=%6%7&y=%8&z=%9&s=%10&scale=2").arg(server).arg(_getServerNum(pos, 4)).arg(request).arg(VersionGoogleTerrain).arg(language).arg(pos.x()).arg(sec1).arg(pos.y()).arg(zoom).arg(sec2);
    }
    break;
    case GoogleMapChina:
    {
    ...
    case GoogleSatelliteChina:
    {
        ...
        return QS("http://%1%2.google.cn/%3/lyrs=%4&gl=cn&x=%5%6&y=%7&z=%8&s=%9").arg(server).arg(_getServerNum(pos, 4)).arg(request).arg(VersionGoogleSatelliteChina).arg(pos.x()).arg(sec1).arg(pos.y()).arg(zoom).arg(sec2);
    }
    break;
    case GoogleLabelsChina:
    {
        ...
        return QS("http://%1%2.google.cn/%3/imgtp=png32&lyrs=%4&hl=%5&gl=cn&x=%6%7&y=%8&z=%9&s=%10").arg(server).arg(_getServerNum(pos, 4)).arg(request).arg(VersionGoogleLabelsChina).arg(QS("zh-CN")).arg(pos.x()).arg(sec1).arg(pos.y()).arg(zoom).arg(sec2);
    }
    break;
    case GoogleTerrainChina:
    {
        ...
        return QS("http://%1%2.google.com/%3/lyrs=%4&hl=%5&gl=cn&x=%6%7&y=%8&z=%9&s=%10").arg(server).arg(_getServerNum(pos, 4)).arg(request).arg(VersionGoogleTerrainChina).arg(QS("zh-CN")).arg(pos.x()).arg(sec1).arg(pos.y()).arg(zoom).arg(sec2);
    }
    break;
    case GoogleMapKorea:
    {
        ...
        return QS("http://%1%2.google.com/%3/lyrs=%4&hl=%5&x=%6%7&y=%8&z=%9&s=%10").arg(server).arg(_getServerNum(pos, 4)).arg(request).arg(VersionGoogleMapKorea).arg(language).arg(pos.x()).arg(sec1).arg(pos.y()).arg(zoom).arg(sec2);
    }
    break;
    case GoogleSatelliteKorea:
    {
        QString server  = QS("khms");
        QString request = QS("kh");
        QString sec1    ; // after &x=...
        QString sec2    ; // after &zoom=...
        _getSecGoogleWords(pos, sec1, sec2);
        _tryCorrectGoogleVersions();
        // http://khm1.google.co.kr/kh/v=54&x=109&y=49&z=7&s=
        return QS("http://%1%2.google.co.kr/%3/v=%4&x=%5%6&y=%7&z=%8&s=%9").arg(server).arg(_getServerNum(pos, 4)).arg(request).arg(VersionGoogleSatelliteKorea).arg(pos.x()).arg(sec1).arg(pos.y()).arg(zoom).arg(sec2);
    }
    break;
    case GoogleLabelsKorea:
    {
        QString server  = QS("mts");
        QString request = QS("mt");
        QString sec1    ; // after &x=...
        QString sec2    ; // after &zoom=...
        _getSecGoogleWords(pos, sec1, sec2);
        _tryCorrectGoogleVersions();
        // http://mts1.gmaptiles.co.kr/mt/v=kr1t.11&hl=lt&x=109&y=50&z=7&s=G
        return QS("http://%1%2.gmaptiles.co.kr/%3/v=%4&hl=%5&x=%6%7&y=%8&z=%9&s=%10").arg(server).arg(_getServerNum(pos, 4)).arg(request).arg(VersionGoogleLabelsKorea).arg(language).arg(pos.x()).arg(sec1).arg(pos.y()).arg(zoom).arg(sec2);
    }
    break;
    case YahooMap:
    {
        return QS("http://maps%1.yimg.com/hx/tl?v=%2&.intl=%3&x=%4&y=%5&z=%6&r=1").arg(((_getServerNum(pos, 2)) + 1)).arg(VersionYahooMap).arg(language).arg(pos.x()).arg((((1 << zoom) >> 1) - 1 - pos.y())).arg((zoom + 1));
    }
    case YahooSatellite:
    {
    ...
    case OpenStreetMap:
    {
        ...
        return QS("http://%1.tile.openstreetmap.org/%2/%3/%4.png").arg(letter).arg(zoom).arg(pos.x()).arg(pos.y());
    }
    break;
    case OpenStreetOsm:
    {
        char letter = "abc"[_getServerNum(pos, 3)];
        return QS("http://%1.tah.openstreetmap.org/Tiles/tile/%2/%3/%4.png").arg(letter).arg(zoom).arg(pos.x()).arg(pos.y());
    }
    break;
    case OpenStreetMapSurfer:
    {
        // http://tiles1.mapsurfer.net/tms_r.ashx?x=37378&y=20826&z=16
        return QS("http://tiles1.mapsurfer.net/tms_r.ashx?x=%1&y=%2&z=%3").arg(pos.x()).arg(pos.y()).arg(zoom);
    }
    break;
    case OpenStreetMapSurferTerrain:
    {
        // http://tiles2.mapsurfer.net/tms_t.ashx?x=9346&y=5209&z=14
        return QS("http://tiles2.mapsurfer.net/tms_t.ashx?x=%1&y=%2&z=%3").arg(pos.x()).arg(pos.y()).arg(zoom);
    }
    break;
    case BingMap:
    {
    ...
    case ArcGIS_Map:
    {
        ...
        return QS("http://server.arcgisonline.com/ArcGIS/rest/services/ESRI_StreetMap_World_2D/MapServer/tile/%1/%2/%3").arg(zoom).arg(pos.y()).arg(pos.x());
    }
    break;
    case ArcGIS_Satellite:
    {
        // http://server.arcgisonline.com/ArcGIS/rest/services/ESRI_Imagery_World_2D/MapServer/tile/1/0/1.jpg
        return QS("http://server.arcgisonline.com/ArcGIS/rest/services/ESRI_Imagery_World_2D/MapServer/tile/%1/%2/%3").arg(zoom).arg(pos.y()).arg(pos.x());
    }
    break;
    case ArcGIS_ShadedRelief:
    {
        ...
        return QS("http://server.arcgisonline.com/ArcGIS/rest/services/ESRI_ShadedRelief_World_2D/MapServer/tile/%1/%2/%3").arg(zoom).arg(pos.y()).arg(pos.x());
    }
    break;
    case ArcGIS_Terrain:
    {
        // http://server.arcgisonline.com/ArcGIS/rest/services/NGS_Topo_US_2D/MapServer/tile/4/3/15
        return QS("http://server.arcgisonline.com/ArcGIS/rest/services/NGS_Topo_US_2D/MapServer/tile/%1/%2/%3").arg(zoom).arg(pos.y()).arg(pos.x());
    }
    break;
    case ArcGIS_MapsLT_OrtoFoto:
    {
        ...
        // TODO verificar
        return QS("http://dc1.maps.lt/cache/mapslt_ortofoto/map/_alllayers/L%1/R%2/C%3.jpg").arg(zoom, 2, 10, (QChar)'0').arg(pos.y(), 8, 16, (QChar)'0').arg(pos.x(), 8, 16, (QChar)'0');
    }
    break;
    case ArcGIS_MapsLT_Map:
    {
        ...
        return QS("http://dc1.maps.lt/cache/mapslt/map/_alllayers/L%1/R%2/C%3.png").arg(zoom, 2, 10, (QChar)'0').arg(pos.y(), 8, 16, (QChar)'0').arg(pos.x(), 8, 16, (QChar)'0');
    }
    break;
    case ArcGIS_MapsLT_Map_Labels:
    {
        ...
        // TODO verificar
        return QS("http://dc1.maps.lt/cache/mapslt_ortofoto_overlay/map/_alllayers/L%1/R%2/C%3.png").arg(zoom, 2, 10, (QChar)'0').arg(pos.y(), 8, 16, (QChar)'0').arg(pos.x(), 8, 16, (QChar)'0');
    }
    break;
    case PergoTurkeyMap:
    {
    ...
    case SigPacSpainMap:
    {
        return QS("http://sigpac.mapa.es/kmlserver/raster/%1@3785/%2.%3.%4.img").arg(kLevelsForSigPacSpainMap[zoom]).arg(zoom).arg(pos.x()).arg((2 << (zoom - 1)) - pos.y() - 1);
    }
    break;
    case YandexMapRu:
    {
        QString server = QS("vec");
        // http://vec01.maps.yandex.ru/tiles?l=map&v=2.10.2&x=1494&y=650&z=11
        return QS("http://%1").arg(server) + QS("0%2.maps.yandex.ru/tiles?l=map&v=%3&x=%4&y=%5&z=%6").arg(_getServerNum(pos, 4) + 1).arg(VersionYandexMap).arg(pos.x()).arg(pos.y()).arg(zoom);
    }
    break;
    default:
