...
QGC_LOGGING_CATEGORY(QGCTileCacheDatabaseLog, "test.qtlocationplugin.qgctilecachedatabase")
...
bool QGCTileCacheDatabase::saveTile(const TileInfo &tile, quint64 setID)
{
    ...
    (void) query.prepare(u"INSERT OR IGNORE INTO Tiles(hash, format, tile, size, type, date) "
                          "VALUES(?, ?, ?, ?, ?, ?)"_s);
...
bool QGCTileCacheDatabase::saveTileBatch(const QList<TileInfo> &tiles, quint64 setID)
{
    ...
    (void) insertQuery.prepare(u"INSERT OR IGNORE INTO Tiles(hash, format, tile, size, type, date) "
                                "VALUES(?, ?, ?, ?, ?, ?)"_s);
    (void) findQuery.prepare(u"SELECT tileID FROM Tiles WHERE hash = ?"_s);
    (void) setQuery.prepare(u"INSERT OR IGNORE INTO SetTiles(setID, tileID) "
                             "VALUES(?, ?)"_s);
...
bool QGCTileCacheDatabase::getTile(const QString &hash, TileInfo &tile)
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"SELECT * FROM Tiles WHERE hash = ?"_s);
    ...
    (void) const_cast<QGCTileCacheDatabase*>(this)->updateTileAccess(hash);
...
quint64 QGCTileCacheDatabase::getTileID(const QString &hash) const
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"SELECT tileID FROM Tiles WHERE hash = ?"_s);
    query.addBindValue(hash);
...
bool QGCTileCacheDatabase::deleteTile(quint64 tileID)
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"DELETE FROM Tiles WHERE tileID = ?"_s);
...
bool QGCTileCacheDatabase::deleteTilesMatchingBytes(const QByteArray &bytes)
{
    ...
    (void) query.prepare(u"SELECT tileID, tile FROM Tiles WHERE LENGTH(tile) = ?"_s);
...
bool QGCTileCacheDatabase::updateTileAccess(const QString &hash)
{
    if (!isOpen()) {
        return false;
    }
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"UPDATE Tiles SET accessCount = accessCount + 1, "
...
bool QGCTileCacheDatabase::getTileSet(quint64 setID, TileSetInfo &info)
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"SELECT * FROM TileSets WHERE setID = ?"_s);
...
bool QGCTileCacheDatabase::deleteTileSet(quint64 setID)
{
    ...
    (void) query.prepare(u"DELETE FROM TilesDownload WHERE setID = ?"_s);
    query.addBindValue(setID);
    (void) query.exec();

    // Delete the tile set
    (void) query.prepare(u"DELETE FROM TileSets WHERE setID = ?"_s);
    query.addBindValue(setID);
    (void) query.exec();

    // Delete set-tile associations
    (void) query.prepare(u"DELETE FROM SetTiles WHERE setID = ?"_s);
    query.addBindValue(setID);
    (void) query.exec();
...
bool QGCTileCacheDatabase::renameTileSet(quint64 setID, const QString &newName)
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"UPDATE TileSets SET name = ? WHERE setID = ?"_s);
...
bool QGCTileCacheDatabase::findTileSetID(const QString &name, quint64 &setID)
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"SELECT setID FROM TileSets WHERE name = ?"_s);
...
bool QGCTileCacheDatabase::addTileToSet(quint64 tileID, quint64 setID)
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"INSERT OR IGNORE INTO SetTiles(tileID, setID) VALUES(?, ?)"_s);
    query.addBindValue(tileID);
    query.addBindValue(setID);
...
bool QGCTileCacheDatabase::removeTileFromSet(quint64 tileID, quint64 setID)
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"DELETE FROM SetTiles WHERE tileID = ? AND setID = ?"_s);
    query.addBindValue(tileID);
    query.addBindValue(setID);
...
bool QGCTileCacheDatabase::getTilesForSet(quint64 setID, QList<quint64> &tileIDs)
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"SELECT tileID FROM SetTiles WHERE setID = ?"_s);
    query.addBindValue(setID);
...
bool QGCTileCacheDatabase::isTileInSet(quint64 tileID, quint64 setID) const
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"SELECT 1 FROM SetTiles WHERE tileID = ? AND setID = ? LIMIT 1"_s);
    query.addBindValue(tileID);
    query.addBindValue(setID);
...
bool QGCTileCacheDatabase::getDownloadList(quint64 setID, int limit, QList<TileDownloadInfo> &tiles)
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"SELECT hash, type, x, y, z "
...
bool QGCTileCacheDatabase::updateDownloadState(quint64 setID, const QString &hash, int state)
{
    ...
    if (state == static_cast<int>(QGCTile::StateComplete)) {
        (void) query.prepare(u"DELETE FROM TilesDownload WHERE setID = ? AND hash = ?"_s);
        query.addBindValue(setID);
        query.addBindValue(hash);
    } else if (hash == "*") {
        (void) query.prepare(u"UPDATE TilesDownload SET state = ? WHERE setID = ?"_s);
...
int QGCTileCacheDatabase::getDownloadQueueCount(quint64 setID) const
{
    ...
    const QSqlDatabase db = _getDatabase();
    QSqlQuery query(db);
    (void) query.prepare(u"SELECT COUNT(*) FROM TilesDownload WHERE setID = ? AND state = 0"_s);
    query.addBindValue(setID);
...
bool QGCTileCacheDatabase::exportDatabase(const QString &targetPath, const QList<quint64> &setIDs, std::function<void(int)> progressCallback)
{
    ...
    for (const quint64 setID : setIDs) {
        ...
        for (const quint64 tileID : std::as_const(tileIDs)) {
            const QSqlDatabase db = _getDatabase();
            QSqlQuery query(db);
            (void) query.prepare(u"SELECT * FROM Tiles WHERE tileID = ?"_s);
            query.addBindValue(tileID);
            if (!query.exec() || !query.next()) {
                continue;
            }
            ...
            currentProgress++;
            const int progress = static_cast<int>((static_cast<double>(currentProgress) / static_cast<double>(totalTiles)) * 100.0);
...
bool QGCTileCacheDatabase::_importDatabaseReplace(const QString &sourcePath, std::function<void(int)> progressCallback)
{
    ...
    // Reopen the database
    if (!open()) {
        _lastError = u"Failed to open database"_s;
        return false;
    }
...
bool QGCTileCacheDatabase::_importDatabaseMerge(const QString &sourcePath, std::function<void(int)> progressCallback)
{
    ...
    for (TileSetInfo &setInfo : tileSets) {
        ...
        for (const quint64 tileID : std::as_const(tileIDs)) {
            ...
            (void) tileQuery.prepare(u"SELECT * FROM Tiles WHERE tileID = ?"_s);
