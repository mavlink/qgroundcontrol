...
void QGCCacheWorker::_importSets(QGCMapTask *mtask)
{
    ...
    auto notifyImportFailure = [task](const QString &message) {
    ...
    if (versionStatus != VersionReadStatus::Success) {
        ...
        notifyImportFailure(versionError);
        return;
    }

    if (task->replace()) {
        (void) _importReplace(task, notifyImportFailure);
    } else {
...
bool QGCCacheWorker::_importReplace(QGCImportTileTask *task, const std::function<void(const QString &)> &notifyImportFailure)
{
    _disconnectDB();
    _defaultSet = UINT64_MAX;
    ...
    if (!QFile::copy(task->path(), _databasePath)) {
        notifyImportFailure(tr("Failed to copy imported cache database."));
        return false;
    }
...
{
    return QSqlDatabase::database(kSession);
}
