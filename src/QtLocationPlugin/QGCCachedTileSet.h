...
public:
    ...
    {
        if (num != _uniqueTileCount) {
            _uniqueTileCount = num;
            emit uniqueTileCountChanged();
        }
    }
    ...
    {
        if (size != _uniqueTileSize) {
            _uniqueTileSize = size;
...
private:
    // In-flight bulk GET. Retains the request + attempt count so a transient failure
    // can be retried with backoff (the tile's x/y/z aren't recoverable from the hash).
    struct DownloadReply
    {
        QNetworkReply* reply = nullptr;
        QNetworkRequest request;
        int attempt = 0;
        // Host whose shared HostConcurrencyGate slot this download holds; released
        // when the download terminates (success, terminal error, pause, retry bail).
        QString host;
    };

    void _prepareDownload();
    void _doneWithDownload();
    void _startTileGet(const QString& hash, const QNetworkRequest& request, int attempt, const QString& host);
    void _replyFinished(const QString& hash, QNetworkReply* reply);
    void _replyError(const QString& hash, QNetworkReply* reply, QNetworkReply::NetworkError error);
    bool _retryOrFail(const QString& hash, const DownloadReply& context, int statusCode,
    ...
    // Bumped on pause/cancel so a retry timer scheduled in a prior download session
    // can't fire a duplicate GET after resume re-fetches the same tile.
    quint64 _downloadEpoch = 0;
    QDateTime _creationDate;

    // All download state below is touched only on the main thread: task-result
    // signals (tileListFetched) are delivered via a queued connection and the
    // QNAM replies are parented to this (main-thread) object, so no locking is
    // needed.
    QHash<QString, DownloadReply> _replies;
    QQueue<QGCTile*> _tilesToDownload;
    QGCMapEngineManager* _manager = nullptr;
    QNetworkAccessManager* _networkManager = nullptr;

    // Constant for a download session; recomputed when a fresh download starts so
    // a per-tile-completion call into QNetworkInformation is avoided.
    qsizetype _maxConcurrentDownloads = 0;
