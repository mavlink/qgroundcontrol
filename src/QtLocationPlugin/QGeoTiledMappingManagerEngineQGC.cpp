...
#define QGCGEOMAPTYPE(a, b, c, d, e, f) QGeoMapType(a, b, c, d, e, f, QByteArray("QGroundControl"), cameraCaps)
    ...
    /*
     * Google and Bing don't seem kosher at all. This was based on original code from OpenPilot and heavily modified to
     * be used in QGC.
     */
    ...
    while (i.hasNext()) {
        ...
        if (!i.value()->_isElevationProvider()) {
            mapList.append(QGCGEOMAPTYPE(i.value()->getMapStyle(), i.key(), i.key(), false, false,
                getQGCMapEngine()->urlFactory()->getIdFromType(i.key())));
        }
    }
...
//-----------------------------------------------------------------------------
QGeoMap* QGeoTiledMappingManagerEngineQGC::createMap() { return new QGeoTiledMapQGC(this); }
...
void QGeoTiledMappingManagerEngineQGC::_setCache(const QVariantMap& parameters)
{
    ...
    if (memLimit < 1024 * 1024)
        memLimit = 1024 * 1024;
    //-- On the other hand, Qt uses signed 32-bit integers. Limit to 1G to round it down (you don't need more than
    // that).
