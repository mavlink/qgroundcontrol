...
extern "C"
{
    ...
    QGC_JNI_EXPORT void gst_android_on_init_complete(void (*callback)(bool, void*), void *userdata)
    {
        ...
        if (prev_cb) {
            qCWarning(AndroidInitLog, "gst_android_on_init_complete: replacing existing callback");
            prev_cb(false, prev_data);
        }
    }

    QGC_JNI_EXPORT void gst_android_clear_init_complete(void *userdata)
    {
        QMutexLocker locker(&_gst_init_mutex);
        if (_gst_init_callback_data != userdata) {
            return;
        }
