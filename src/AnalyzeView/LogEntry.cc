...
LogDownloadData::LogDownloadData(QGCLogEntry *entry)
    : ID(entry->id())
    , entry(entry)
{
...
{
    return qMin(static_cast<uint32_t>(qCeil(static_cast<qreal>((entry->size() - (current_chunk * kChunkSize))) / static_cast<qreal>(MAVLINK_MSG_LOG_DATA_FIELD_DATA_LEN))), kTableBins);
}
