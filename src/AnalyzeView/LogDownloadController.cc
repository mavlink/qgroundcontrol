...
LogDownloadWorker::LogDownloadWorker(const QString &dir, QObject *parent)
    : QObject(parent)
    , _dir(dir)
{
...
LogDownloadWorker::~LogDownloadWorker()
{
    // qCDebug(LogDownloadControllerLog) << Q_FUNC_INFO << this;
}

void LogDownloadWorker::process()
{
    downloadToDirectory(_dir);
}

void LogDownloadWorker::downloadToDirectory(const QString &dir)
{
...
void LogDownloadController::download(const QString &path)
{
    ...
    LogDownloadWorker *const worker = new LogDownloadWorker(dir);

    worker->moveToThread(_workerThread);

    (void) connect(_workerThread, &QThread::started, worker, &LogDownloadWorker::process);
    (void) connect(worker, &LogDownloadWorker::downloadCompleted, this, &LogDownloadController::_onDownloadCompleted);
    (void) connect(worker, &LogDownloadWorker::downloadCompleted, _workerThread, &QThread::quit);
    (void) connect(_workerThread, &QThread::finished, worker, &QObject::deleteLater);
...
void LogDownloadController::_onDownloadCompleted()
{
