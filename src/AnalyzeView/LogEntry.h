...
{
    explicit LogDownloadData(LogEntry * const entry);
    ~LogDownloadData();
...
public:
    explicit LogEntry(uint logId, const QDateTime &dateTime = QDateTime(), uint logSize = 0, bool received = false, QObject *parent = nullptr);
    ~LogEntry() override;
