...
void LinkManager::suspendConfigurationUpdates(bool suspend) { _configUpdateSuspended = suspend; }
...
void LinkManager::_addMAVLinkForwardingLink(void)
{
    if (_toolbox->settingsManager()->appSettings()->forwardMavlink()->rawValue().toBool()) {
        ...
        if (!foundMAVLinkForwardingLink) {
            ...
            QString hostName
                = _toolbox->settingsManager()->appSettings()->forwardMavlinkHostName()->rawValue().toString();
...
void LinkManager::_addZeroConfAutoConnectLink(void)
{
    ...
    connect(browser.get(), &QMdnsEngine::Browser::serviceAdded, this,
        [checkIfConnectionLinkExist, this](const QMdnsEngine::Service& service) {
            qCDebug(LinkManagerVerboseLog) << "Found Zero-Conf:" << service.type() << service.name()
                                           << service.hostname() << service.port() << service.attributes();
                ...
                case QGCSerialPortInfo::BoardTypePixhawk:
                    if (_autoConnectSettings->autoConnectPixhawk()->rawValue().toBool()) {
                        pSerialConfig = new SerialConfiguration(
                            tr("%1 on %2 (AutoConnect)").arg(boardName).arg(portInfo.portName().trimmed()));
...
bool LinkManager::isBluetoothAvailable(void) { return qgcApp()->isBluetoothAvailable(); }
