...
public:
    ...
    Q_PROPERTY(
        bool incrementVehicleId READ incrementVehicleId WRITE setIncrementVehicleId NOTIFY incrementVehicleIdChanged)

    int firmware(void) { return (int)_firmwareType; }
    void setFirmware(int type)
    {
        _firmwareType = (MAV_AUTOPILOT)type;
    ...
    {
        _vehicleType = (MAV_TYPE)type;
        emit vehicleChanged();
    }
    void setIncrementVehicleId(bool incrementVehicleId)
    {
        _incrementVehicleId = incrementVehicleId;
        emit incrementVehicleIdChanged();
    }
    ...
    {
        _boardVendorId = vendorId;
        _boardProductId = productId;
    }
    ...
    typedef enum {
        ...
        FailMissingParamOnInitialReqest, // Not all params are sent on initial request, should still succeed since QGC
                                         // will re-query missing params
        ...
        FailInitialConnectRequestMessageAutopilotVersionLost, // REQUEST_MESSAGE:AUTOPILOT_VERSION success,
                                                              // AUTOPILOT_VERSION never sent
        FailInitialConnectRequestMessageProtocolVersionFailure, // REQUEST_MESSAGE:PROTOCOL_VERSION returns failure
        FailInitialConnectRequestMessageProtocolVersionLost, // REQUEST_MESSAGE:PROTOCOL_VERSION success,
                                                             // PROTOCOL_VERSION never sent
    } FailureMode_t;
...
public:
    ...
    {
        _apmSendHomePositionOnEmptyList = sendHomePositionOnEmptyList;
    }
    ...
    void setMissionItemFailureMode(
    ...
    static MockLink* startPX4MockLink(
        bool sendStatusText, MockConfiguration::FailureMode_t failureMode = MockConfiguration::FailNone);
    ...
    // Special commands for testing COMMAND_LONG handlers. By default all commands except for
    // MAV_CMD_MOCKLINK_NO_RESPONSE_NO_RETRY should retry.
...
private:
    ...
    static MockLink* _startMockLinkWorker(QString configName, MAV_AUTOPILOT firmwareType, MAV_TYPE vehicleType,
    ...
    int _currentParamRequestListComponentIndex; // Current component index for param request list workflow, -1 for no
                                                // request in progress
