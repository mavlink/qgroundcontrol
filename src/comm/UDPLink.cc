...
bool UDPLink::_hardwareConnect()
{
    ...
    _connectState
        = _socket->bind(host, _udpConfig->localPort(), QAbstractSocket::ReuseAddressHint | QUdpSocket::ShareAddress);
    if (_connectState) {
...
bool UDPLink::isConnected() const { return _connectState; }

void UDPLink::_registerZeroconf(uint16_t port, const std::string& regType)
{
#if defined(QGC_ZEROCONF_ENABLED)
    DNSServiceErrorType result
        = DNSServiceRegister(&_dnssServiceRef, 0, 0, 0, regType.c_str(), NULL, NULL, htons(port), 0, NULL, NULL, NULL);
...
UDPConfiguration::~UDPConfiguration() { _clearTargetHosts(); }
...
void UDPConfiguration::setLocalPort(quint16 port) { _localPort = port; }
