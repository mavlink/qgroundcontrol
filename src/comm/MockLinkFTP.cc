...
void MockLinkFTP::_listCommand(
    uint8_t senderSystemId, uint8_t senderComponentId, MavlinkFTP::Request* request, uint16_t seqNumber)
{
    ...
    if (!path.isEmpty() && path != "/") {
        _sendNak(
            senderSystemId, senderComponentId, MavlinkFTP::kErrFail, outgoingSeqNumber, MavlinkFTP::kCmdListDirectory);
        return;
    }
    ...
    if (request->hdr.offset > (uint32_t)_fileList.size()) {
        _sendNak(
            senderSystemId, senderComponentId, MavlinkFTP::kErrEOF, outgoingSeqNumber, MavlinkFTP::kCmdListDirectory);
        return;
    }
    ...
    uint8_t senderSystemId, uint8_t senderComponentId, MavlinkFTP::Request* request, uint16_t seqNumber)
{
    ...
    if (!tmpFilename.isEmpty()) {
        ...
        if (!_currentFile.open(QIODevice::ReadOnly)) {
            _sendNakErrno(
                senderSystemId, senderComponentId, _currentFile.error(), outgoingSeqNumber, MavlinkFTP::kCmdOpenFileRO);
            return;
        }
    } else {
        _sendNak(senderSystemId, senderComponentId, MavlinkFTP::kErrFailFileNotFound, outgoingSeqNumber,
    ...
    uint8_t senderSystemId, uint8_t senderComponentId, MavlinkFTP::Request* request, uint16_t seqNumber)
{
    ...
    if (request->hdr.session != _sessionId) {
        _sendNak(senderSystemId, senderComponentId, MavlinkFTP::kErrInvalidSession, outgoingSeqNumber,
            MavlinkFTP::kCmdReadFile);
        return;
    }
    ...
    if (readOffset != 0) {
        ...
        if (_errMode == errModeNakSecondResponse) {
            // Nak error all subsequent requests
            _sendNak(
                senderSystemId, senderComponentId, MavlinkFTP::kErrFail, outgoingSeqNumber, MavlinkFTP::kCmdReadFile);
            return;
        } else if (_errMode == errModeNoSecondResponse) {
...
void MockLinkFTP::_burstReadCommand(
    uint8_t senderSystemId, uint8_t senderComponentId, MavlinkFTP::Request* request, uint16_t seqNumber)
{
    uint16_t outgoingSeqNumber = _nextSeqNumber(seqNumber);
    MavlinkFTP::Request response {};
    ...
    if (request->hdr.session != _sessionId) {
        _sendNak(
            senderSystemId, senderComponentId, MavlinkFTP::kErrFail, outgoingSeqNumber, MavlinkFTP::kCmdBurstReadFile);
        return;
    }
    ...
    if (burstOffset >= _currentFile.size()) {
        ...
        _sendNak(
            senderSystemId, senderComponentId, MavlinkFTP::kErrEOF, outgoingSeqNumber, MavlinkFTP::kCmdBurstReadFile);
    }
}

void MockLinkFTP::_terminateCommand(
    uint8_t senderSystemId, uint8_t senderComponentId, MavlinkFTP::Request* request, uint16_t seqNumber)
{
    uint16_t outgoingSeqNumber = _nextSeqNumber(seqNumber);
    ...
    if (request->hdr.session != _sessionId) {
        _sendNak(senderSystemId, senderComponentId, MavlinkFTP::kErrInvalidSession, outgoingSeqNumber,
            MavlinkFTP::kCmdTerminateSession);
        return;
    }
...
void MockLinkFTP::mavlinkMessageReceived(const mavlink_message_t& message)
{
    ...
    if (_randomDropsEnabled && request->hdr.opcode != MavlinkFTP::kCmdOpenFileRO
    ...
    if (request->hdr.opcode != MavlinkFTP::kCmdResetSessions
        ...
        if (_errMode == errModeNoResponse) {
            ...
            _sendNak(message.sysid, message.compid, MavlinkFTP::kErrFail, outgoingSeqNumber,
                (MavlinkFTP::OpCode_t)request->hdr.opcode);
            return;
        }
    }
    ...
    default:
        ...
        _sendNak(message.sysid, message.compid, MavlinkFTP::kErrUnknownCommand, outgoingSeqNumber,
            (MavlinkFTP::OpCode_t)request->hdr.opcode);
        break;
    }
}
...
/// @brief Sends an Ack
void MockLinkFTP::_sendAck(
    uint8_t targetSystemId, uint8_t targetComponentId, uint16_t seqNumber, MavlinkFTP::OpCode_t reqOpcode)
{
    MavlinkFTP::Request ackResponse {};
...
void MockLinkFTP::_sendNak(uint8_t targetSystemId, uint8_t targetComponentId, MavlinkFTP::ErrorCode_t error,
...
void MockLinkFTP::_sendNakErrno(uint8_t targetSystemId, uint8_t targetComponentId, uint8_t nakErrno, uint16_t seqNumber,
...
void MockLinkFTP::_sendResponse(
    uint8_t targetSystemId, uint8_t targetComponentId, MavlinkFTP::Request* request, uint16_t seqNumber)
{
    ...
    // kCmdOpenFileRO and kCmdResetSessions don't support retry so we can't drop those
    if (_randomDropsEnabled && request->hdr.req_opcode != MavlinkFTP::kCmdOpenFileRO
        && request->hdr.req_opcode != MavlinkFTP::kCmdResetSessions) {
        if ((rand() % 5) == 0) {
