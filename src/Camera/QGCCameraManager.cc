...
{
    // Stop per-camera timers and clean up
...
void QGCCameraManager::_handleCameraInfo(const mavlink_message_t& message)
{
    ...
    qCDebug(CameraManagerLog) << reinterpret_cast<const char*>(info.model_name)
...
void QGCCameraManager::_checkForLostCameras()
{
    ...
    for (const QString& key : stale) {
        CameraStruct* pInfo = _cameraInfoRequest.value(key, nullptr);
        if (!pInfo) {
            continue;
        }
        ...
        pInfo->backoffTimer.stop();
        (void) QObject::disconnect(&pInfo->backoffTimer, nullptr, nullptr, nullptr);
        delete pInfo;
        (void) _cameraInfoRequest.remove(key);
    }
...
static void _handleCameraInfoRetry(QGCCameraManager::CameraStruct *cameraInfo)
{
    ...
    if ((cameraInfo->retryCount >= 2) && ((cameraInfo->retryCount % 2) == 0)) {
        ...
        // Guard manager lifetime and bind the context to the timer object
        QPointer<QGCCameraManager> mgrGuard(manager);
        (void) QObject::connect(&cameraInfo->backoffTimer, &QTimer::timeout,
                                &cameraInfo->backoffTimer,  // context dies with timer
                                [mgrGuard, cameraInfo]() {
            if (!mgrGuard) {
                return;
            }
            _requestCameraInfoHelper(mgrGuard.data(), cameraInfo);
        }, Qt::UniqueConnection);
