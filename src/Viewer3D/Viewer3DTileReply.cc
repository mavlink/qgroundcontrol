...
Viewer3DTileReply::Viewer3DTileReply(int zoomLevel, int tileX, int tileY, int mapId, const QString& mapType,
...
void Viewer3DTileReply::_prepareDownload()
{
    const QNetworkRequest request =
        QGeoTileFetcherQGC::getNetworkRequest(_tile.mapId, _tile.x, _tile.y, _tile.zoomLevel);
...
void Viewer3DTileReply::_onCacheHit(QSharedPointer<QGCCacheTile> tile)
{
    if (!tile) {
        _onCacheMiss();
        return;
...
{
    qCWarning(Viewer3DTileReplyLog) << "Request error for tile x:" << _tile.x << "y:" << _tile.y
                                    << "zoom:" << _tile.zoomLevel;
    _timeoutTimer->stop();
    disconnect(_timeoutTimer, &QTimer::timeout, this, &Viewer3DTileReply::_onTimeout);
    _disconnectReply();
...
{
    qCDebug(Viewer3DTileReplyLog) << "Timeout for tile x:" << _tile.x << "y:" << _tile.y << "retry:" << _timeoutCounter
                                  << "/" << kMaxRetries;
...
{
    const SharedMapProvider mapProvider = UrlFactory::getMapProviderFromQtMapId(_tile.mapId);
    return mapProvider && std::dynamic_pointer_cast<const BingMapProvider>(mapProvider) && !_tile.data.isEmpty() &&
           _tile.data == _bingNoTileImage;
}
