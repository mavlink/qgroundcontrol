...
public:
    ...
    static int day(int y, int m = 1, int d = 1)
    {
        ...
        return (1461 * y) / 4 // Julian years converted to days.  Julian year is 365 +
                              // 1/4 = 1461/4 days.
            ...
            + (greg ? (y / 100) / 4 - (y / 100) + 2 : 0) + (153 * m + 2) / 5 // The zero-based start of the m'th month
    ...
    static int day(int y, int m, int d, bool check)
    {
        ...
        if (!(s > 0 && y == y1 && m == m1 && d == d1))
            throw GeographicErr("Invalid date " + str(y) + "-" + str(m) + "-" + str(d)
                + (s > 0 ? "; use " + str(y1) + "-" + str(m1) + "-" + str(d1) : " before 0001-01-01"));
        return s;
    }
    ...
    {
        if (!Math::isfinite(x))
            return x < 0 ? std::string("-inf") : (x > 0 ? std::string("inf") : std::string("nan"));
        std::ostringstream s;
    ...
    {
        unsigned beg = 0, end = unsigned(s.size());
     ...
     **********************************************************************/
    template <typename T> GEOGRAPHICLIB_DEPRECATED("Use Utility::val<T>(s)") static T num(const std::string& s)
    {
        return val<T>(s);
    }
    ...
    template <typename T> static T fract(const std::string& s)
    {
        ...
        return !(delim != std::string::npos && delim >= 1 && delim + 2 <= s.size()) ? val<T>(s) :
                                                                                    // delim in [1, size() - 2]
    ...
    static void readarray(std::istream& str, IntT array[], size_t num)
    {
        ...
        if (sizeof(IntT) == sizeof(ExtT)
            && std::numeric_limits<IntT>::is_integer == std::numeric_limits<ExtT>::is_integer) {
...
#endif
        {
            ...
            while (k) {
                ...
                for (int j = 0; j < n; ++j)
                    ...
                    array[i++] = IntT(bigendp == Math::bigendian ? buffer[j] : Math::swab<ExtT>(buffer[j]));
                k -= n;
            }
        }
        return;
...
#endif
        {
            ...
            while (k) {
                ...
                for (int j = 0; j < n; ++j)
                    ...
                    buffer[j] = bigendp == Math::bigendian ? ExtT(array[i++]) : Math::swab<ExtT>(ExtT(array[i++]));
 ...
 **********************************************************************/
template <> inline std::string Utility::val<std::string>(const std::string& s) { return trim(s); }
...
template <> inline bool Utility::val<bool>(const std::string& s)
{
    ...
    if (is >> x) {
        int pos = int(is.tellg()); // Returns -1 at end of string?
        if (!(pos < 0 || pos == int(t.size())))
            throw GeographicErr("Extra text " + t.substr(pos) + " at end of " + t);
        return x;
    }
    for (std::string::iterator p = t.begin(); p != t.end(); ++p)
