...
private:
    ...
    enum {
        ...
        = 0x14, ///< device is using silicon not suitable for the target the bootloader was used for
    ...
    bool _inBootloaderMode
        = false; ///< true: board is in bootloader mode, false: special case for SiK Radio, board is in command mode
    ...
    static const int _bootloaderVersionV2CorrectFlash
        = 5; ///< Anything below this bootloader version on V2 boards cannot trust flash size
};
