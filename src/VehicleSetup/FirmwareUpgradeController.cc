...
FirmwareUpgradeController::~FirmwareUpgradeController() { qgcApp()->toolbox()->linkManager()->setConnectionsAllowed(); }
...
void FirmwareUpgradeController::flash(
    AutoPilotStackType_t stackType, FirmwareBuildType_t firmwareType, FirmwareVehicleType_t vehicleType)
{
...
void FirmwareUpgradeController::_foundBoard(
    bool firstAttempt, const QSerialPortInfo& info, int boardType, QString boardName)
{
    _boardInfo = info;
...
void FirmwareUpgradeController::_noBoardFound(void) { emit noBoardFound(); }

void FirmwareUpgradeController::_boardGone(void) { emit boardGone(); }
...
void FirmwareUpgradeController::_initFirmwareHash()
{
    ...
    FirmwareToUrlElement_t rgPX4FLowFirmwareArray[]
        = { {PX4FlowPX4, StableFirmware, DefaultVehicleFirmware,
                "http://px4-travis.s3.amazonaws.com/Flow/master/px4flow.px4"},
#if !defined(NO_ARDUPILOT_DIALECT)
              {PX4FlowAPM, StableFirmware, DefaultVehicleFirmware,
                  "http://firmware.ardupilot.org/Tools/PX4Flow/px4flow-klt-latest.px4"},
#endif
          };

    // We build the maps for PX4 firmwares dynamically using the data below
    for (auto& element : rgPX4FLowFirmwareArray) {
        _rgPX4FLowFirmware.insert(
            FirmwareIdentifier(element.stackType, element.firmwareType, element.vehicleType), element.url);
    }
}
...
void FirmwareUpgradeController::_bootloaderSyncFailed(void) { _errorCancel("Unable to sync with bootloader."); }
    ...
    int boardId)
{
    ...
    default:
        if (px4_board_name_map.contains(boardId)) {
            ...
            _rgFirmwareDynamic.insert(FirmwareIdentifier(AutoPilotStackPX4, DeveloperFirmware, DefaultVehicleFirmware),
...
void FirmwareUpgradeController::_downloadFirmware(void)
{
    ...
    connect(
        downloader, &QGCFileDownload::downloadComplete, this, &FirmwareUpgradeController::_firmwareDownloadComplete);
    connect(
        downloader, &QGCFileDownload::downloadProgress, this, &FirmwareUpgradeController::_firmwareDownloadProgress);
...
void FirmwareUpgradeController::_status(const QString& statusString) { _appendStatusLog(statusString); }
...
void FirmwareUpgradeController::_eraseComplete(void) { _eraseTimer.stop(); }
...
void FirmwareUpgradeController::_px4ReleasesGithubDownloadComplete(
    QString /*remoteFile*/, QString localFile, QString errorMsg)
{
    if (errorMsg.isEmpty()) {
        QFile jsonFile(localFile);
        if (!jsonFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
            qCWarning(FirmwareUpgradeLog)
...
void FirmwareUpgradeController::_ardupilotManifestDownloadComplete(
    QString remoteFile, QString localFile, QString errorMsg)
{
    if (errorMsg.isEmpty()) {
        ...
        QString jsonFileName(QDir(QStandardPaths::writableLocation(QStandardPaths::TempLocation))
                                 .absoluteFilePath("ArduPilot.Manifest.json"));
        ...
        for (int i = 0; i < rgFirmware.count(); i++) {
            ...
            if (firmwareVehicleType != DefaultVehicleFirmware && firmwareBuildType != CustomFirmware
                ...
                firmwareInfo.friendlyName = QStringLiteral("%1 - %2")
                                                .arg(brandName.isEmpty() ? platform : brandName)
                                                .arg(firmwareInfo.version);
            }
        }
