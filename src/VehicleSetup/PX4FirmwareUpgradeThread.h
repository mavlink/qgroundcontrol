...
private:
    ...
    bool _findBoardFirstAttempt
        = true; ///< true: we found the board right away, it needs to be unplugged and plugged back in
...
private slots:
    void _foundBoard(bool firstAttempt, const QGCSerialPortInfo& portInfo, int type, QString name)
    {
        emit foundBoard(firstAttempt, portInfo, type, name);
    }
