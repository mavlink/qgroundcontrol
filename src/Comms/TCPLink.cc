    ...
    , _socket(new QTcpSocket())
{
    ...
    _socket->setSocketOption(QAbstractSocket::SendBufferSizeSocketOption, 1024); // >= MAVLINK_MAX_PACKET_LEN
    _socket->setSocketOption(QAbstractSocket::ReceiveBufferSizeSocketOption, 1024); // >= MAVLINK_MAX_PACKET_LEN
...
bool TCPLink::_connect()
{
    ...
    // (void) QObject::connect(_socket, &QTcpSocket::bytesWritten, this, [this](qint64 written) {}, Qt::AutoConnection);
...
void TCPLink::_readBytes()
{
    ...
    if (byteCount <= 0) { // < MAVLINK_NUM_NON_PAYLOAD_BYTES
    ...
    if (_socket->read(buffer.data(), buffer.size()) < 0) { // readAll()
