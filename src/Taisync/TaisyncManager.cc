...
//-----------------------------------------------------------------------------
TaisyncManager::~TaisyncManager() { _close(); }

//-----------------------------------------------------------------------------
void TaisyncManager::_close()
{
    if (_taiSettings) {
        _taiSettings->close();
...
//-----------------------------------------------------------------------------
FactMetaData* TaisyncManager::_createMetadata(const char* name, QStringList enums)
{
...
//-----------------------------------------------------------------------------
void TaisyncManager::setToolbox(QGCToolbox* toolbox)
{
...
//-----------------------------------------------------------------------------
bool TaisyncManager::setRTSPSettings(QString uri, QString account, QString password)
{
...
//-----------------------------------------------------------------------------
bool TaisyncManager::setIPSettings(QString localIP_, QString remoteIP_, QString netMask_)
{
    bool res = false;
...
//-----------------------------------------------------------------------------
void TaisyncManager::_radioSettingsChanged(QVariant)
{
    if (_taiSettings && _isConnected) {
        _workTimer.stop();
        _taiSettings->setRadioSettings(
            _radioModeList[_radioMode->rawValue().toInt()], _radioChannel->enumStringValue());
...
//-----------------------------------------------------------------------------
void TaisyncManager::_videoSettingsChanged(QVariant)
{
    if (_taiSettings && _isConnected) {
        _workTimer.stop();
        _taiSettings->setVideoSettings(_videoOutputList[_videoOutput->rawValue().toInt()],
            _videoMode->enumStringValue(), _videoRateList[_videoRate->rawValue().toInt()]);
...
//-----------------------------------------------------------------------------
void TaisyncManager::_setEnabled()
{
...
//-----------------------------------------------------------------------------
void TaisyncManager::_restoreVideoSettings(Fact* setting)
{
    auto* pFact = qobject_cast<SettingsFact*>(setting);
    if (pFact) {
        pFact->setVisible(qgcApp()->toolbox()->corePlugin()->adjustSettingMetaData(
            VideoSettings::settingsGroup, *setting->metaData()));
    }
}
...
void TaisyncManager::_setVideoEnabled()
{
    ...
    bool enable
        = _appSettings->enableTaisyncVideo()->rawValue().toBool() && _appSettings->enableTaisync()->rawValue().toBool();
    if (enable) {
...
//-----------------------------------------------------------------------------
#if defined(__ios__) || defined(__android__)
void TaisyncManager::_readTelemBytes(QByteArray bytesIn)
{
...
//-----------------------------------------------------------------------------
#if defined(__ios__) || defined(__android__)
void TaisyncManager::_readUDPBytes()
{
...
//-----------------------------------------------------------------------------
void TaisyncManager::_connected()
{
...
//-----------------------------------------------------------------------------
void TaisyncManager::_disconnected()
{
...
//-----------------------------------------------------------------------------
void TaisyncManager::_checkTaisync()
{
    if (_enabled) {
        if (!_isConnected) {
            if (_taiSettings) {
...
//-----------------------------------------------------------------------------
void TaisyncManager::_updateSettings(QByteArray jSonData)
{
    _timeoutTimer.start();
    ...
    if (jSonData.contains("\"flight\":")) {
        ...
        if (idx >= 0)
            _radioMode->_containerSetRawValue(idx);
