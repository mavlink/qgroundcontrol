...
QVector3D geodeticToEcef(const QGeoCoordinate &llh)
{
    double x, y, z;
...
QGeoCoordinate ecefToGeodetic(const QVector3D &ecef)
{
    double lat, lon, alt;
...
QVector3D ecefToEnu(const QVector3D &ecef, const QGeoCoordinate &ref)
{
    // 1️⃣ ECEF ➜ geodetic (lat/lon/alt)
    double lat, lon, h;
    GeographicLib::Geocentric::WGS84().Reverse(
        ecef.x(), ecef.y(), ecef.z(), lat, lon, h);

    // 2️⃣ geodetic ➜ local ENU wrt 'ref'
    double e, n, u;
    GeographicLib::LocalCartesian(ref.latitude(),
                                  ref.longitude(),
                                  ref.altitude(),
                                  GeographicLib::Geocentric::WGS84())
        .Forward(lat, lon, h, e, n, u);
...
QVector3D enuToEcef(const QVector3D &enu, const QGeoCoordinate &ref)
{
    // 1️⃣ ENU ➜ geodetic (lat/lon/alt)
    double lat, lon, h;
    GeographicLib::LocalCartesian(ref.latitude(),
                                  ref.longitude(),
                                  ref.altitude(),
...
QVector3D gpsToEnu(const QGeoCoordinate &llh, const QGeoCoordinate &ref)
{
    double e, n, u;
    GeographicLib::LocalCartesian(
        ref.latitude(), ref.longitude(), ref.altitude(),
        GeographicLib::Geocentric::WGS84()
    ).Forward(llh.latitude(), llh.longitude(), llh.altitude(), e, n, u);
    return { static_cast<float>(e), static_cast<float>(n), static_cast<float>(u) };
}

QGeoCoordinate enuToGps(const QVector3D &enu, const QGeoCoordinate &ref)
{
    double lat, lon, alt;
    GeographicLib::LocalCartesian(
        ref.latitude(), ref.longitude(), ref.altitude(),
...
QVector3D mapGpsToLocalPoint(const QGeoCoordinate &llh, const QGeoCoordinate &ref)
{
    return ecefToEnu(geodeticToEcef(llh), ref);
}

QGeoCoordinate mapLocalToGpsPoint(const QVector3D &enu, const QGeoCoordinate &ref)
{
    return ecefToGeodetic(enuToEcef(enu, ref));
}
