...
QGC_LOGGING_CATEGORY(GeoFileIOLog, "Utilities.Geo.GeoFileIO")
...
TextResult loadText(const QString &filePath, const QString &formatName)
{
    ...
    if (file.size() > kMaxGeoFileSizeBytes) {
        result.error = formatLoadError(formatName,
            QCoreApplication::translate("GeoFileIO", "File exceeds maximum size limit (%1 MB)")
                .arg(kMaxGeoFileSizeBytes / (1024 * 1024)));
        return result;
    }
