...
LoadResult loadPointsFromString(const QString &content, const ParseOptions &options)
{
    ...
    if (lines.isEmpty()) {
        result.errorString = GeoFileIO::formatLoadError(QStringLiteral("CSV"), QObject::tr("File is empty"));
        return result;
    }
    ...
    if (opts.latColumn < 0 || opts.lonColumn < 0) {
        result.errorString = GeoFileIO::formatLoadError(QStringLiteral("CSV"), QObject::tr("Cannot detect latitude/longitude columns"));
        return result;
    }
    ...
    if (result.points.isEmpty()) {
        result.errorString = GeoFileIO::formatNoEntitiesError(QStringLiteral("CSV"), QObject::tr("coordinates"));
        return result;
    }
...
LoadResult loadPointsFromFile(const QString &filePath, const ParseOptions &options)
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        result.errorString = GeoFileIO::formatLoadError(QStringLiteral("CSV"), file.errorString());
        return result;
    }
    ...
    return loadPointsFromString(content, options);
}
                      ...
                      const QStringList &names, QString &errorString)
{
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        errorString = GeoFileIO::formatSaveError(QStringLiteral("CSV"), file.errorString());
        return false;
    }
...
bool loadPolylineFromFile(const QString &filePath, QList<QGeoCoordinate> &coords, QString &errorString)
{
    ...
    if (polylines.isEmpty()) {
        errorString = GeoFileIO::formatNoEntitiesError(QStringLiteral("CSV"), QObject::tr("polylines"));
        return false;
    }
...
bool loadPolylinesFromFile(const QString &filePath, QList<QList<QGeoCoordinate>> &polylines, QString &errorString)
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        errorString = GeoFileIO::formatLoadError(QStringLiteral("CSV"), file.errorString());
        return false;
    }
    ...
    QStringList lines = content.split(QRegularExpression("[\r\n]+"), Qt::SkipEmptyParts);
    if (lines.isEmpty()) {
        errorString = GeoFileIO::formatLoadError(QStringLiteral("CSV"), QObject::tr("File is empty"));
        return false;
    }
    ...
    if (allPoints.isEmpty()) {
        errorString = GeoFileIO::formatNoEntitiesError(QStringLiteral("CSV"), QObject::tr("coordinates"));
        return false;
    }
    ...
    if (polylines.isEmpty()) {
        errorString = GeoFileIO::formatLoadError(QStringLiteral("CSV"), QObject::tr("No valid polylines found (need at least 2 points per line)"));
        return false;
    }
...
bool loadPolygonFromFile(const QString &filePath, QList<QGeoCoordinate> &vertices, QString &errorString)
{
    Q_UNUSED(filePath);
    Q_UNUSED(vertices);
    errorString = GeoFileIO::formatLoadError(QStringLiteral("CSV"),
