...
double normalizeLongitude(double longitude)
{
    ...
    const double originalLongitude = longitude;
    longitude = std::fmod(longitude + 180.0, 360.0);
    if (longitude < 0.0) longitude += 360.0;
    longitude -= 180.0;

    // Preserve +180 for positive inputs on the wrap boundary.
    if (longitude == -180.0 && originalLongitude > 0.0) {
        longitude = 180.0;
    }
...
namespace {
    QString _toGeosPolygonWkt(const QList<QGeoCoordinate> &vertices)
    {
        QList<QGeoCoordinate> ring = vertices;
        GeoUtilities::removeClosingVertex(ring);
        if (ring.size() < 3) {
            return QString();
        }

        QString wkt = QStringLiteral("POLYGON((");
        for (int i = 0; i < ring.size(); ++i) {
            if (i != 0) {
                wkt += QStringLiteral(", ");
            }
            wkt += QStringLiteral("%1 %2")
                .arg(ring[i].longitude(), 0, 'f', 12)
                .arg(ring[i].latitude(), 0, 'f', 12);
        }
        // Close ring for WKT polygon.
        wkt += QStringLiteral(", %1 %2))")
            .arg(ring.first().longitude(), 0, 'f', 12)
            .arg(ring.first().latitude(), 0, 'f', 12);
        return wkt;
    }

    bool _geosDetectSelfIntersection(const QList<QGeoCoordinate> &vertices)
    {
        GEOSContextHandle_t context = GEOS_init_r();
        if (!context) {
            return false;
        }
        const auto contextCleanup = qScopeGuard([&]() { GEOS_finish_r(context); });

        GEOSWKTReader *reader = GEOSWKTReader_create_r(context);
        if (!reader) {
            return false;
        }
        const auto readerCleanup = qScopeGuard([&]() { GEOSWKTReader_destroy_r(context, reader); });

        const QByteArray wkt = _toGeosPolygonWkt(vertices).toUtf8();
        if (wkt.isEmpty()) {
            return false;
        }
        GEOSGeometry *geometry = GEOSWKTReader_read_r(context, reader, wkt.constData());
        if (!geometry) {
            return false;
        }
        const auto geometryCleanup = qScopeGuard([&]() { GEOSGeom_destroy_r(context, geometry); });

        char *reasonRaw = GEOSisValidReason_r(context, geometry);
        if (!reasonRaw) {
            return false;
        }

        const QString reason = QString::fromUtf8(reasonRaw);
        GEOSFree_r(context, reasonRaw);

        return reason.contains(QStringLiteral("Self-intersection"), Qt::CaseInsensitive) ||
               reason.contains(QStringLiteral("Ring Self-intersection"), Qt::CaseInsensitive);
    }
...
bool isSelfIntersecting(const QList<QGeoCoordinate> &vertices)
{
    ...
    if (_geosDetectSelfIntersection(vertices)) {
        return true;
    }
                                     ...
                                     QString &errorString)
{
    for (int i = 0; i < polylines.size(); ++i) {
        QString validationError;
        if (!validateCoordinates(polylines[i], validationError)) {
            errorString = QObject::tr("Polyline %1: %2").arg(i + 1).arg(validationError);
            return false;
        }
    }
    return true;
                  ...
                  int forceZone, bool forceSouthern)
{
    ...
    for (const auto &coord : coords) {
        ...
        if (naturalZone != forceZone) {
            if (std::abs(naturalZone - forceZone) > 1) {
                utmCoords.clear();
                return false;
            }
            utm.zone = forceZone;
...
namespace {
    ...
    [[maybe_unused]] const double kPlusCodeLatPrecision[] = {20.0, 1.0, 0.05, 0.0025, 0.000125};
    [[maybe_unused]] const double kPlusCodeLonPrecision[] = {20.0, 1.0, 0.05, 0.0025, 0.000125};
}
