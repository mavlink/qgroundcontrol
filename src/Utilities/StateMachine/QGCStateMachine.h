...
public:
    ...
    /// @param stateName Name for the state
    /// @param action Primary action to execute (returns true on success)
    /// @param maxRetries Number of retries after initial attempt
    /// @param retryDelayMsecs Delay between retries
    /// @param exhaustedBehavior Behavior when all options are exhausted
    /// @param fallback Optional fallback action
    /// @param rollback Optional rollback action
    /// @param timeoutMsecs Optional overall timeout
    /// @return The created state
    ...
    /// Create a logging error handler state that advances to @p nextState.
    FunctionState* addLogAndContinueErrorState(const QString& stateName,
                                               QAbstractState* nextState,
                                               const QString& message = QString());

    /// Create a logging error handler state that stops the machine.
    FunctionState* addLogAndStopErrorState(const QString& stateName,
                                           const QString& message = QString());
