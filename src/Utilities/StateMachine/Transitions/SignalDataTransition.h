...
public:
    ...
    SignalDataTransition(const Sender* sender, Func signalFn, QAbstractState* target,
...
SignalDataTransition<Args...>* makeSignalDataTransition(
    const Sender* sender, Func signalFn, QAbstractState* target,
    typename SignalDataTransition<Args...>::Guard guard = nullptr,
    typename SignalDataTransition<Args...>::Action action = nullptr)
{
    return new SignalDataTransition<Args...>(sender, signalFn, target, std::move(guard), std::move(action));
}
