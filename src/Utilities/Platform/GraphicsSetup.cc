...
// Parse "low:high" hex LUID from QGC_FORCE_VIDEO_GPU into a DXGI adapter LUID pair.
...
void configureForcedD3DAdapter(QQuickWindow* window)
{
    ...
    // setGraphicsDevice() must happen before scene graph initialization. Once the QRhi is created,
    // Qt Quick owns the adapter choice for the lifetime of this scene graph.
...
// Diagnostic-only HDR probe. This runs on the render thread where window->rhi() is valid.
// The temporary swapchain is never presented; it only exposes QRhi's HDR support query APIs.
void probeHdr(QQuickWindow* window)
{
    ...
    // Qt Quick owns the real window swapchain. Qt 6.10 has no public per-window HDR format setter,
    // so this cannot enable HDR output; it only reports what the backend/display could support.
...
void configureMainWindow(QQuickWindow* window)
{
    ...
    // HDR diagnostics need a live QRhi, so they run on the render thread after scene graph init.
