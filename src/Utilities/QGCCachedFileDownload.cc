...
bool QGCCachedFileDownload::download(const QString &url, int maxCacheAgeSec)
{
    ...
    if (metadata.isValid() && metadata.attributes().contains(QNetworkRequest::Attribute::User)) {
        ...
        const QDateTime creationTime = metadata.attributes().find(QNetworkRequest::Attribute::User)->toDateTime();
        ...
        const auto attributes = QVector<QPair<QNetworkRequest::Attribute, QVariant>>{qMakePair(QNetworkRequest::CacheLoadControlAttribute, QVariant{QNetworkRequest::PreferCache})};
        return _fileDownload->download(url, attributes);
    }
