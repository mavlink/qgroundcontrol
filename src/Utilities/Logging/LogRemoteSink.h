...
public:
    ...
    void send(const QGCLogEntry& entry);
...
private:
    ...
    QByteArray _formatBatch(const QQueue<QGCLogEntry>& entries, int startIdx, int count) const;
    QJsonObject _entryToJson(const QGCLogEntry& entry) const;
