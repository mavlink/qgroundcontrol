...
// Thread-safe buffer for categories registered before the manager exists.
// Uses function-local statics to avoid static-init ordering issues.
static QMutex& earlyMutex()
{
    static QMutex m;
    return m;
}

static QStringList*& earlyPending()
{
...
QGCLoggingCategoryManager::QGCLoggingCategoryManager() : QObject()
{
    ...
    {
        QMutexLocker locker(&earlyMutex());
        if (earlyPending()) {
            for (const QString& cat : std::as_const(*earlyPending())) {
                registerCategory(cat);
            }
            delete earlyPending();
            earlyPending() = nullptr;
        }
    }
}
