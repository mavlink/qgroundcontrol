...
QByteArray decompress(const QByteArray& data, qint64 maxDecompressedSize)
{
    if (data.isEmpty()) {
        return {};
    }
    ...
    if (header == HeaderCompressed) {
        // qCompress/qUncompress prepends a 4-byte big-endian expected size.
        // Validate it before allocating to prevent decompression bombs.
        if (maxDecompressedSize > 0 && payload.size() >= 4) {
            ...
            if (declaredSize > static_cast<quint32>(maxDecompressedSize)) {
                qCWarning(LogCompressionLog) << "Rejected decompression: declared size"
                                             << declaredSize << "exceeds limit" << maxDecompressedSize;
                return {};
            }
        }

        const QByteArray result = QGCCompression::uncompressData(payload);
