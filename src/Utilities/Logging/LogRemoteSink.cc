...
void LogRemoteSink::send(const QGCLogEntry& entry)
{
    {
        QMutexLocker locker(&_mutex);
        if (!_enabled) {
...
QByteArray LogRemoteSink::_formatBatch(const QQueue<QGCLogEntry>& entries, int startIdx, int count) const
{
    QString vehicleId;
    {
        QMutexLocker locker(&_mutex);
...
QJsonObject LogRemoteSink::_entryToJson(const QGCLogEntry& entry) const
{
    QJsonObject json;
    json["ts"] = entry.timestamp.toString(Qt::ISODateWithMs);
    json["lvl"] = QGCLogEntry::levelLabel(entry.level);
