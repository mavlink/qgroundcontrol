...
{
    qmlRegisterType<PX4SimpleFlightModesController>(
        "QGroundControl.Controllers", 1, 0, "PX4SimpleFlightModesController");
...
FactMetaData* PX4FirmwarePlugin::_getMetaDataForFact(
...
void PX4FirmwarePlugin::_getParameterMetaDataVersionInfo(
    const QString& metaDataFile, int& majorVersion, int& minorVersion)
{
...
{
    QList<MAV_CMD> supportedCommands = {MAV_CMD_NAV_WAYPOINT, MAV_CMD_NAV_LOITER_UNLIM, MAV_CMD_NAV_LOITER_TIME,
        MAV_CMD_NAV_RETURN_TO_LAUNCH, MAV_CMD_DO_JUMP, MAV_CMD_DO_DIGICAM_CONTROL, MAV_CMD_DO_SET_CAM_TRIGG_DIST,
        MAV_CMD_DO_SET_SERVO, MAV_CMD_DO_SET_ACTUATOR, MAV_CMD_DO_CHANGE_SPEED, MAV_CMD_DO_SET_HOME,
        MAV_CMD_DO_LAND_START, MAV_CMD_DO_SET_ROI_LOCATION, MAV_CMD_DO_SET_ROI_WPNEXT_OFFSET, MAV_CMD_DO_SET_ROI_NONE,
        MAV_CMD_DO_MOUNT_CONFIGURE, MAV_CMD_DO_MOUNT_CONTROL, MAV_CMD_SET_CAMERA_MODE, MAV_CMD_IMAGE_START_CAPTURE,
        MAV_CMD_IMAGE_STOP_CAPTURE, MAV_CMD_VIDEO_START_CAPTURE, MAV_CMD_VIDEO_STOP_CAPTURE, MAV_CMD_NAV_DELAY,
        MAV_CMD_CONDITION_YAW, MAV_CMD_NAV_LOITER_TO_ALT, MAV_CMD_DO_GRIPPER};
...
{
    vehicle->sendMavCommand(vehicle->defaultComponentId(), MAV_CMD_DO_REPOSITION,
        true, // show error if failed
        -1.0f, MAV_DO_REPOSITION_FLAGS_CHANGE_MODE, 0.0f, NAN, NAN, NAN, NAN);
}
...
void PX4FirmwarePlugin::guidedModeLand(Vehicle* vehicle) { _setFlightModeAndValidate(vehicle, _landingFlightMode); }

void PX4FirmwarePlugin::_mavCommandResult(
...
double PX4FirmwarePlugin::maximumHorizontalSpeedMultirotor(Vehicle* vehicle)
{
    ...
    if (vehicle->parameterManager()->parameterExists(FactSystem::defaultComponentId, speedParam)) {
        return vehicle->parameterManager()
            ->getParameter(FactSystem::defaultComponentId, speedParam)
            ->rawValue()
            .toDouble();
    }
...
double PX4FirmwarePlugin::maximumEquivalentAirspeed(Vehicle* vehicle)
{
    ...
    if (vehicle->parameterManager()->parameterExists(FactSystem::defaultComponentId, airspeedMax)) {
        return vehicle->parameterManager()
            ->getParameter(FactSystem::defaultComponentId, airspeedMax)
            ->rawValue()
            .toDouble();
    }
...
double PX4FirmwarePlugin::minimumEquivalentAirspeed(Vehicle* vehicle)
{
    ...
    if (vehicle->parameterManager()->parameterExists(FactSystem::defaultComponentId, airspeedMin)) {
        return vehicle->parameterManager()
            ->getParameter(FactSystem::defaultComponentId, airspeedMin)
            ->rawValue()
            .toDouble();
    }
...
void PX4FirmwarePlugin::guidedModeGotoLocation(Vehicle* vehicle, const QGeoCoordinate& gotoCoord)
{
    ...
    if (vehicle->capabilityBits() & MAV_PROTOCOL_CAPABILITY_COMMAND_INT) {
        vehicle->sendMavCommandInt(vehicle->defaultComponentId(), MAV_CMD_DO_REPOSITION, MAV_FRAME_GLOBAL,
            true, // show error is fails
            -1.0f, MAV_DO_REPOSITION_FLAGS_CHANGE_MODE, 0.0f, NAN, gotoCoord.latitude(), gotoCoord.longitude(),
            vehicle->altitudeAMSL()->rawValue().toFloat());
    } else {
        vehicle->sendMavCommand(vehicle->defaultComponentId(), MAV_CMD_DO_REPOSITION,
            true, // show error is fails
            -1.0f, MAV_DO_REPOSITION_FLAGS_CHANGE_MODE, 0.0f, NAN, static_cast<float>(gotoCoord.latitude()),
            static_cast<float>(gotoCoord.longitude()), vehicle->altitudeAMSL()->rawValue().toFloat());
    }
}
...
static void _pauseVehicleThenChangeAltResultHandler(void* resultHandlerData, int /*compId*/, MAV_RESULT commandResult,
...
void PX4FirmwarePlugin::guidedModeChangeAltitude(Vehicle* vehicle, double altitudeChange, bool pauseVehicle)
{
    ...
    if (pauseVehicle) {
        vehicle->sendMavCommandWithHandler(_pauseVehicleThenChangeAltResultHandler, resultData,
            vehicle->defaultComponentId(), MAV_CMD_DO_REPOSITION,
            -1.0f, // Don't change groundspeed
            MAV_DO_REPOSITION_FLAGS_CHANGE_MODE,
            0.0f, // Reserved
...
void PX4FirmwarePlugin::guidedModeChangeGroundSpeed(Vehicle* vehicle, double groundspeed)
{
    ...
    vehicle->sendMavCommand(vehicle->defaultComponentId(), MAV_CMD_DO_CHANGE_SPEED,
        true, // show error is fails
...
void PX4FirmwarePlugin::_handleAutopilotVersion(Vehicle* vehicle, mavlink_message_t* message)
{
    ...
    if (!instanceData->versionNotified) {
        ...
        if (notifyUser) {
            ...
            qgcApp()->showAppMessage(
                tr("QGroundControl supports PX4 Pro firmware Version %1.%2.%3 and above. You are using a version prior "
                   "to that which will lead to unpredictable results. Please upgrade your firmware.")
                    .arg(supportedMajorVersion)
                    .arg(supportedMinorVersion)
                    .arg(supportedPatchVersion));
        }
    }
}
...
QString PX4FirmwarePlugin::_versionRegex() { return QStringLiteral("v([0-9,\\.]*) Stable"); }
...
QString PX4FirmwarePlugin::getHobbsMeter(Vehicle* vehicle)
{
    ...
    if (vehicle->parameterManager()->parameterExists(FactSystem::defaultComponentId, HOOBS_HI)
        ...
        hobbsTimeSeconds
            = ((uint64_t)factHi->rawValue().toUInt() << 32 | (uint64_t)factLo->rawValue().toUInt()) / 1000000;
        qCDebug(VehicleLog) << "Hobbs Meter raw PX4:"
