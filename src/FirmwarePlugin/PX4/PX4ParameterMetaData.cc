...
QVersionNumber PX4ParameterMetaData::versionFromFilename(QStringView path);
{
    ...
    if (!xmlFile.exists()) {
        _outputFileWarning(xmlPath, QStringLiteral("Does not exist"), QString());
        return;
    }

    if (!xmlFile.open(QIODevice::ReadOnly)) {
        _outputFileWarning(xmlPath, QStringLiteral("Unable to open file"), xmlFile.errorString());
        return;
    }
    ...
    xmlFile.close();
    if (xml.hasError()) {
        _outputFileWarning(xmlPath, QStringLiteral("Badly formed XML"), xml.errorString());
        return;
    }
    ...
    while (!xml.atEnd() && ((majorVersion == -1) || (minorVersion == -1))) {
        if (xml.isStartElement()) {
            ...
            if (elementName == "parameter_version_major") {
                ...
                if (!convertOk) {
                    _outputFileWarning(xmlPath, QStringLiteral("Unable to convert parameter_version_major value to int"), QString());
                    return;
                }
            } else if (elementName == "parameter_version_minor") {
                bool convertOk;
                ...
                if (!convertOk) {
                    _outputFileWarning(xmlPath, QStringLiteral("Unable to convert parameter_version_minor value to int"), QString());
                    return;
                }
            }
        }
    ...
    if (majorVersion == -1) {
        _outputFileWarning(xmlPath, QStringLiteral("parameter_version_major is missing"), QString());
    }

    if (minorVersion == -1) {
        _outputFileWarning(xmlPath, QStringLiteral("parameter_version_minor tag is missing"), QString());
    }
}

void PX4ParameterMetaData::_outputFileWarning(const QString &xmlPath, const QString &error1, const QString &error2)
{
    qCWarning(PX4ParameterMetaDataLog) << QStringLiteral("Internal Error: Parameter meta data file '%1'. %2. error: %3").arg(metaDataFile).arg(error1).arg(error2);
}
