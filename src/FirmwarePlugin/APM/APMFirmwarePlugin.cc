...
{
    qmlRegisterType<APMFlightModesComponentController>(
        "QGroundControl.Controllers", 1, 0, "APMFlightModesComponentController");
    qmlRegisterType<APMAirframeComponentController>(
        "QGroundControl.Controllers", 1, 0, "APMAirframeComponentController");
    ...
    qmlRegisterType<APMSubMotorComponentController>(
        "QGroundControl.Controllers", 1, 0, "APMSubMotorComponentController");
}
...
void APMFirmwarePlugin::_handleIncomingParamValue(Vehicle* vehicle, mavlink_message_t* message)
{
    ...
    mavlink_msg_param_value_encode_chan(message->sysid, message->compid, channel, message, &paramValue);
}

void APMFirmwarePlugin::_handleOutgoingParamSetThreadSafe(
    Vehicle* /*vehicle*/, LinkInterface* outgoingLink, mavlink_message_t* message)
{
    ...
    mavlink_msg_param_set_encode_chan(
        message->sysid, message->compid, outgoingLink->mavlinkChannel(), message, &paramSet);
    ...
    if (messageText.contains(APM_FRAME_REXP)) {
        ...
        if (frameTypeRegex.indexIn(messageText) != -1) {
            ...
            if (!frameType.isEmpty()
                && (frameType == QStringLiteral("Y6") || frameType == QStringLiteral("OCTA_QUAD")
                    || frameType == QStringLiteral("COAX"))) {
                _coaxialMotors = true;
            }
        }
    }
    ...
    if (message->compid == MAV_COMP_ID_AUTOPILOT1) {
        ...
        if (vehicle->armed()) {
            ...
            if (!flying && vehicle->flying()) {
                ...
                flying
                    = heartbeat.system_status == MAV_STATE_CRITICAL || heartbeat.system_status == MAV_STATE_EMERGENCY;
            }
        }
        vehicle->_setFlying(flying);
    }

    // We need to know whether this component is part of the ArduPilot stack code or not so we can adjust mavlink quirks
    // appropriately. If the component sends a heartbeat we can know that. If it's doesn't there is pretty much no way
    // to know.
...
bool APMFirmwarePlugin::adjustIncomingMavlinkMessage(Vehicle* vehicle, mavlink_message_t* message)
{
    ...
    if (message->msgid == MAVLINK_MSG_ID_HEARTBEAT) {
        ...
        // Only translate messages which come from ArduPilot code. All other components are expected to follow current
        // mavlink spec.
    ...
    if (instanceData
        && (instanceData->lastBatteryStatusTime.secsTo(QTime::currentTime()) > reinitStreamsTimeoutSecs
            || instanceData->lastHomePositionTime.secsTo(QTime::currentTime()) > reinitStreamsTimeoutSecs)) {
...
void APMFirmwarePlugin::adjustOutgoingMavlinkMessageThreadSafe(
    Vehicle* vehicle, LinkInterface* outgoingLink, mavlink_message_t* message)
{
    switch (message->msgid) {
...
void APMFirmwarePlugin::_setInfoSeverity(mavlink_message_t* message) const
{
    ...
    Q_ASSERT(qgcApp()->thread() == QThread::currentThread());
    mavlink_msg_statustext_encode_chan(message->sysid, message->compid, channel, message, &statusText);
}
...
void APMFirmwarePlugin::initializeStreamRates(Vehicle* vehicle)
{
    ...
    if (qgcApp()->toolbox()->settingsManager()->appSettings()->apmStartMavlinkStreams()->rawValue().toBool()) {
        ...
        APMMavlinkStreamRateSettings* streamRates
            = qgcApp()->toolbox()->settingsManager()->apmMavlinkStreamRateSettings();
    ...
    vehicle->sendMavCommand(MAV_COMP_ID_AUTOPILOT1, MAV_CMD_SET_MESSAGE_INTERVAL, false /* showError */,
        MAVLINK_MSG_ID_HOME_POSITION, 1000000 /* 1 second interval in usec */);
...
void APMFirmwarePlugin::initializeVehicle(Vehicle* vehicle)
{
    ...
    if (qgcApp()->toolbox()->settingsManager()->videoSettings()->videoSource()->rawValue()
        == VideoSettings::videoSource3DRSolo) {
        _soloVideoHandshake();
    }
}

void APMFirmwarePlugin::setSupportedModes(QList<APMCustomMode> supportedModes) { _supportedModes = supportedModes; }
...
FactMetaData* APMFirmwarePlugin::_getMetaDataForFact(
    QObject* parameterMetaData, const QString& name, FactMetaData::ValueType_t type, MAV_TYPE vehicleType)
{
...
QList<MAV_CMD> APMFirmwarePlugin::supportedMissionCommands(QGCMAVLink::VehicleClass_t vehicleClass)
{
    ...
    if (vehicleClass == QGCMAVLink::VehicleClassVTOL) {
        ...
        || vehicleClass == QGCMAVLink::VehicleClassMultiRotor) {
        supportedCommands += flightCommands;
    }
...
QString APMFirmwarePlugin::getHobbsMeter(Vehicle* vehicle)
{
    ...
    if (vehicle->parameterManager()->parameterExists(FactSystem::defaultComponentId, "STAT_FLTTIME")) {
        ...
        qCDebug(VehicleLog) << "Hobbs Meter raw Ardupilot(s):"
                            << "(" << hobbsTimeSeconds << ")";
    }
...
bool APMFirmwarePlugin::isGuidedMode(const Vehicle* vehicle) const { return vehicle->flightMode() == "Guided"; }
...
void APMFirmwarePlugin::pauseVehicle(Vehicle* vehicle) { _setFlightModeAndValidate(vehicle, pauseFlightMode()); }
...
void APMFirmwarePlugin::guidedModeChangeAltitude(Vehicle* vehicle, double altitudeChange, bool pauseVehicle)
{
    ...
    if (abs(altitudeChange) < 0.01) {
        // This prevents unecessary changes to Guided mode when the users selects pause and doesn't really touch the
        // altitude slider
        return;
    }
    ...
    if (sharedLink) {
        ...
        mavlink_msg_set_position_target_local_ned_encode_chan(static_cast<uint8_t>(mavlink->getSystemId()),
            static_cast<uint8_t>(mavlink->getComponentId()), sharedLink->mavlinkChannel(), &msg, &cmd);
...
double APMFirmwarePlugin::minimumTakeoffAltitude(Vehicle* vehicle)
{
    ...
    if (vehicle->parameterManager()->parameterExists(FactSystem::defaultComponentId, takeoffAltParam)) {
        minTakeoffAlt = vehicle->parameterManager()
                            ->getParameter(FactSystem::defaultComponentId, takeoffAltParam)
                            ->rawValue()
                            .toDouble()
            / static_cast<double>(paramDivisor);
    }
...
void APMFirmwarePlugin::startMission(Vehicle* vehicle)
{
    ...
    if (!vehicle->armed()) {
        ...
        // In Ardupilot for vtols and airplanes we need to set the mode to auto and then arm, otherwise if arming in
        // guided If the vehicle has tilt rotors, it will arm them in forward flight position, being dangerous.
...
QString APMFirmwarePlugin::_getLatestVersionFileUrl(Vehicle* vehicle)
{
    ...
    if (qobject_cast<ArduPlaneFirmwarePlugin*>(vehicle->firmwarePlugin())) {
        ...
        qWarning() << "APMFirmwarePlugin::_getLatestVersionFileUrl Unknown vehicle firmware type"
                   << vehicle->vehicleType();
        return QString();
    }
}

QString APMFirmwarePlugin::_versionRegex() { return QStringLiteral(" V([0-9,\\.]*)$"); }
...
void APMFirmwarePlugin::_handleRCChannels(Vehicle* vehicle, mavlink_message_t* message)
{
    ...
    if (sharedLink) {
        ...
        MAVLinkProtocol* mavlink = qgcApp()->toolbox()->mavlinkProtocol();
        mavlink_msg_rc_channels_encode_chan(static_cast<uint8_t>(mavlink->getSystemId()),
            static_cast<uint8_t>(mavlink->getComponentId()), sharedLink->mavlinkChannel(), message, &channels);
    }
}
...
void APMFirmwarePlugin::_handleRCChannelsRaw(Vehicle* vehicle, mavlink_message_t* message)
{
    ...
    if (sharedLink) {
        ...
        MAVLinkProtocol* mavlink = qgcApp()->toolbox()->mavlinkProtocol();
        mavlink_msg_rc_channels_raw_encode_chan(static_cast<uint8_t>(mavlink->getSystemId()),
            static_cast<uint8_t>(mavlink->getComponentId()), sharedLink->mavlinkChannel(), message, &channels);
    }
}

void APMFirmwarePlugin::_sendGCSMotionReport(
