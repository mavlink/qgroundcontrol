...
public:
    CameraMetaData(const QString& canonicalName, const QString& brand, const QString& model, double sensorWidth,
        double sensorHeight, double imageWidth, double imageHeight, double focalLength, bool landscape,
        bool fixedOrientation, double minTriggerInterval, const QString& deprecatedTranslatedName,
    ...
    /// In older builds camera names were incorrect marked for translation. This leads to plan files which have are
    /// language dependant which is not a good thing. Newer plan files use the canonical name which is not translated.
    /// In order to support loading older plan files we continue to include the incorrect translation so we can match
    /// against them as needed. Newly added CameraMetaData entries should leave this value empty.
