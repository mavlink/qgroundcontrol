...
QGC_LOGGING_CATEGORY(FMTParameterMetaDataLog, "FMTParameterMetaDataLog")

FMTParameterMetaData::FMTParameterMetaData(void)
...
QVariant FMTParameterMetaData::_stringToTypedVariant(const QString& string, FactMetaData::ValueType_t type, bool* convertOk)
{
    QVariant var(string);
...
void FMTParameterMetaData::loadParameterFactMetaDataFile(const QString& metaDataFile)
{
    qCDebug(ParameterManagerLog) << "FMTParameterMetaData::loadParameterFactMetaDataFile" << metaDataFile;
    ...
    qCDebug(FMTParameterMetaDataLog) << "Loading parameter meta data:" << metaDataFile;
    ...
    while (!xml.atEnd()) {
        if (xml.isStartElement()) {
            ...
            if (elementName == "parameters") {
                ...
                qCDebug(FMTParameterMetaDataLog) << "Found group: " << factGroup;
                ...
                qCDebug(FMTParameterMetaDataLog) << "Found parameter name:" << name << " type:" << type << " default:" << strDefault;
                ...
                if (_mapParameterName2FactMetaData.contains(name)) {
                    ...
                    qCWarning(FMTParameterMetaDataLog) << "Duplicate parameter found:" << name;
                    badMetaData = true;
                    ...
                    if (xml.attributes().hasAttribute("default") && !strDefault.isEmpty()) {
                        ...
                        if (metaData->convertAndValidateRaw(strDefault, false, varDefault, errorString)) {
                            ...
                            qCWarning(FMTParameterMetaDataLog) << "Invalid default value, name:" << name << " type:" << type << " default:" << strDefault << " error:" << errorString;
                        }
                    }
                }
                ...
                if (!badMetaData) {
                    if (metaData) {
                        if (elementName == "short_desc") {
                            ...
                            text = text.replace("\n", " ");
                            qCDebug(FMTParameterMetaDataLog) << "Short description:" << text;
                            metaData->setShortDescription(text);

                        } else if (elementName == "long_desc") {
                            QString text = xml.readElementText();
                            text = text.replace("\n", " ");
                            qCDebug(FMTParameterMetaDataLog) << "Long description:" << text;
                            metaData->setLongDescription(text);

                        } else if (elementName == "min") {
                            QString text = xml.readElementText();
                            qCDebug(FMTParameterMetaDataLog) << "Min:" << text;

                            QVariant varMin;
                            if (metaData->convertAndValidateRaw(text, false /* convertOnly */, varMin, errorString)) {
                                metaData->setRawMin(varMin);
                            } else {
                                qCWarning(FMTParameterMetaDataLog) << "Invalid min value, name:" << metaData->name() << " type:" << metaData->type() << " min:" << text << " error:" << errorString;
                            }

                        } else if (elementName == "max") {
                            QString text = xml.readElementText();
                            qCDebug(FMTParameterMetaDataLog) << "Max:" << text;

                            QVariant varMax;
                            if (metaData->convertAndValidateRaw(text, false /* convertOnly */, varMax, errorString)) {
                                metaData->setRawMax(varMax);
                            } else {
                                qCWarning(FMTParameterMetaDataLog) << "Invalid max value, name:" << metaData->name() << " type:" << metaData->type() << " max:" << text << " error:" << errorString;
                            }

                        } else if (elementName == "unit") {
                            QString text = xml.readElementText();
                            qCDebug(FMTParameterMetaDataLog) << "Unit:" << text;
                            metaData->setRawUnits(text);

                        } else if (elementName == "decimal") {
                            QString text = xml.readElementText();
                            qCDebug(FMTParameterMetaDataLog) << "Decimal:" << text;
                            ...
                            if (convertOk) {
                                ...
                                qCWarning(FMTParameterMetaDataLog) << "Invalid decimals value, name:" << metaData->name() << " type:" << metaData->type() << " decimals:" << text << " error: invalid number";
                            }

                        } else if (elementName == "reboot_required") {
                            QString text = xml.readElementText();
                            qCDebug(FMTParameterMetaDataLog) << "RebootRequired:" << text;
                            ...
                            qCDebug(FMTParameterMetaDataLog) << "parameter value:"
                            ...
                            if (metaData->convertAndValidateRaw(enumValueStr, false /* validate */, enumValue, errorString)) {
                                ...
                                qCDebug(FMTParameterMetaDataLog) << "Invalid enum value, name:" << metaData->name()
                            ...
                            if (ok) {
                                ...
                                qCWarning(FMTParameterMetaDataLog) << "Invalid value for increment, name:" << metaData->name() << " increment:" << text;
                            }
                            ...
                            if (ok) {
                                ...
                                if (bit < 31) {
                                    ...
                                    if (metaData->convertAndValidateRaw(bitmaskRawValue, true, bitmaskValue, errorString)) {
                                        ...
                                        qCDebug(FMTParameterMetaDataLog) << "Invalid bitmask value, name:" << metaData->name()
                                                                         << " type:" << metaData->type() << " value:" << bitmaskValue
                                                                         << " error:" << errorString;
                                    }
                                } else {
                                    qCWarning(FMTParameterMetaDataLog) << "Invalid value for bitmask, bit:" << bit;
                                }
                            }
                        } else {
                            qCDebug(FMTParameterMetaDataLog) << "Unknown element in XML: " << elementName;
                        }
                    } else {
            ...
            if (elementName == "parameter") {
                ...
                if (metaData->defaultValueAvailable()) {
                    ...
                    if (!metaData->convertAndValidateRaw(metaData->rawDefaultValue(), false /* convertOnly */, var, errorString)) {
                        qCWarning(FMTParameterMetaDataLog) << "Invalid default value, name:" << metaData->name() << " type:" << metaData->type() << " default:" << metaData->rawDefaultValue() << " error:" << errorString;
                    }
                }
...
FactMetaData* FMTParameterMetaData::getMetaDataForFact(const QString& name, MAV_TYPE vehicleType)
{
...
void FMTParameterMetaData::addMetaDataToFact(Fact* fact, MAV_TYPE vehicleType)
{
    Q_UNUSED(vehicleType)
...
void FMTParameterMetaData::getParameterMetaDataVersionInfo(const QString& metaDataFile, int& majorVersion, int& minorVersion)
{
    QFile xmlFile(metaDataFile);
