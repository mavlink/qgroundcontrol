...
class VideoRecorderSessionController : public QObject
{
    Q_OBJECT
    Q_DISABLE_COPY_MOVE(VideoRecorderSessionController)

public:
    using RecorderFactory = std::function<VideoRecorder*(VideoStream*)>;

    explicit VideoRecorderSessionController(VideoStream* owner, QObject* parent = nullptr);
