...
// Resolve QRhi's Vulkan native handles; nullptr unless the active RHI is the Vulkan backend. Desktop Linux pins GL in
// Platform::initialize (Vulkan import is dormant), so on Linux this is the expected no-op path.
// We read nativeHandles() rather than QSGRendererInterface::getResource(): getResource exposes instance/physDev/
// queue-family but has no VkDevice accessor (and no D3D adapter-LUID query), so nativeHandles() is the required source.
const QRhiVulkanNativeHandles* qrhiVulkanHandles()
{
    QRhi* rhi = QGCRhiCapture::cachedRhi();
    if (!rhi || rhi->backend() != QRhi::Vulkan) {
        return nullptr;
    }
    return static_cast<const QRhiVulkanNativeHandles*>(rhi->nativeHandles());
}
...
bool primeLocked()
{
    ...
    const QRhiVulkanNativeHandles* nh = qrhiVulkanHandles();
    if (!nh || nh->physDev == VK_NULL_HANDLE) {
    ...
    s_queue = gst_vulkan_device_get_queue(s_device, nh->gfxQueueFamilyIdx, nh->gfxQueueIdx);
    if (!s_queue) {
