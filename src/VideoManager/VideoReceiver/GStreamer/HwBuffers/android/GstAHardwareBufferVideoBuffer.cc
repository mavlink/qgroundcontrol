...
void clearImageCacheLocked(PFNEGLDESTROYIMAGEKHRPROC destroyImage, QOpenGLFunctions* gl)
{
    for (std::size_t i = 0; i < s_imageCache.size(); ++i) {
        ...
        if (entry.inUse > 0) {
            // Live FrameTextures still holds this name; destroying now would UAF — leak the texture/image/AHB instead.
            qCWarning(GstAHWBufLog) << "AHB texture cache reset with entry still in use; leaking texture"
                                    << entry.textureName;
            entry = AhbCacheEntry{};
            continue;
        }
...
// Pin a cache entry by texture name so LRU eviction can't free a name a live frame holds; caller holds
// s_imageCacheMutex. Unmatched names (e.g. after a reset) are no-ops.
