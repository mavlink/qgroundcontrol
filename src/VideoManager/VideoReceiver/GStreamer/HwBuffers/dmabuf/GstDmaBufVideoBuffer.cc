...
// Producer dma-buf fence export (#6): optional. The header exists on Ubuntu 22.04 but predates the
...
// stale image, so we key on (fd, modifier, w, h, fourcc) and weak-ref the GstMemory to evict before the fd is reused.
...
struct DmaImageKeyHash
{
    std::size_t operator()(const DmaImageKey& k) const noexcept
    {
        ...
        h ^= std::hash<guint64>{}(k.modifier) + 0x9e3779b97f4a7c15ULL + (h << 6) + (h >> 2);
        h ^= std::hash<int>{}(k.width) + 0x9e3779b97f4a7c15ULL + (h << 6) + (h >> 2);
        h ^= std::hash<int>{}(k.height) + 0x9e3779b97f4a7c15ULL + (h << 6) + (h >> 2);
        h ^= std::hash<int>{}(k.fourcc) + 0x9e3779b97f4a7c15ULL + (h << 6) + (h >> 2);
        return h;
    }
};
...
// #6 best-effort GPU-side wait on the producer's fence: export a sync_file fd from the dma-buf, import it as an
...
void GstDmaBufVideoBuffer::resetCachedState() noexcept
{
    ...
    // Defer EGLImage teardown to the next render-thread mapTextures(); eglDestroyImage off the GL thread is unsafe.
...
QVideoFrameTexturesUPtr GstDmaBufVideoBuffer::mapTextures(QRhi& rhi, QVideoFrameTexturesUPtr& /*old*/)
{
    ...
    // Sync barrier for Intel iHD legacy LINEAR (no implicit fence). Prefer an EGL fence (#5) over a full-frame mmap;
    ...
    // #5 LINEAR sync barrier: prefer EGL_KHR_fence_sync (insert+client-wait, a GPU-completion wait) over mmapping the
    // whole frame. mmap remains the fallback when the fence ext is absent (a CPU-side read barrier).
    if (needLinearFence) {
        // #6: prefer importing the producer's dma-buf fence for a pure GPU-side wait; only if that fails do we fall
        // back to the #5 EGL-fence-then-mmap barrier below (never removed — just skipped on a confirmed GPU wait).
        int producerFd = -1;
    ...
    if (singleFdPacking) {
        ...
        const DmaImageKey key{fd0, drmModifier, GST_VIDEO_INFO_WIDTH(&_videoInfo), GST_VIDEO_INFO_HEIGHT(&_videoInfo),
                              singleFourcc};
        ...
        if (singleImage != EGL_NO_IMAGE_KHR) {
            for (int p = 0; p < planeCount; ++p) {
                ...
                // glGetError forces a pipeline flush — only call when category debug is on.
                if (Q_UNLIKELY(GstDmaBufLog().isDebugEnabled())) {
...
per_plane_path:
    for (int i = 0; i < planeCount && ok; ++i) {
        ...
        attribs[n++] = EGL_NONE;
        const bool cacheOn = imageCacheEnabled();
        // Per-plane key: same fd may back several planes (NV12 single-fd), so width/height/fourcc disambiguate slots.
        const DmaImageKey key{fd, drmModifier, planeWidth, planeHeight, fourcc};
        EGLImage image = EGL_NO_IMAGE_KHR;
