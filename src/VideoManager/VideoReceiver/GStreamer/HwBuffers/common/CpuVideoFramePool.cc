...
public:
    ...
    // wrapZeroCopy falls back to a copy so a compositor stall can't drain the decoder's HW pool. Backend-correctness
    // refinement: derive from the RHI's FramesInFlight (the prior constant 6 already covered typical 2-3 depths).
    static int maxLive() noexcept
    {
        static const int s_max = []() -> int {
            ...
            if (QRhi* rhi = QGCRhiCapture::cachedRhi()) {
                const int fif = rhi->resourceLimit(QRhi::FramesInFlight);
                if (fif > 0) {
