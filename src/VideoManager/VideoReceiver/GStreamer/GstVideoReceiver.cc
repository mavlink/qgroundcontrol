...
void GstVideoReceiver::_scheduleReconnect(const char *reason)
{
    ...
    QMetaObject::invokeMethod(this, [this, reason]() {
        const int next = qMin(_reconnectAttempts.load(std::memory_order_relaxed) + 1, 30);
        ...
        const int delaySec = qMin(1 << qMin(next - 1, 5), 30);
        ...
        qCInfo(GstVideoReceiverLog) << "Scheduling reconnect #" << attempts
                                    << "in" << delaySec << "s after" << reason << _uri;
        // _uri/_timeout are written by start() on the worker thread and read here on the GUI thread;
        // the epoch gate makes this safe — a superseding start() always runs stop() (epoch bump) first.
        QTimer::singleShot(delaySec * 1000, this, [this, epoch, attempts]() {
            ...
            if (_uri.isEmpty() || pipelineUp) return;  // pipeline already came back
            qCInfo(GstVideoReceiverLog) << "Reconnecting (attempt" << attempts << ")" << _uri;
            start(_timeout ? _timeout : 8);
        });
    }, Qt::QueuedConnection);
}
...
gboolean GstVideoReceiver::_onBusMessage(GstBus * /* bus */, GstMessage *msg, gpointer data)
{
    ...
    case GST_MESSAGE_QOS: {
        ...
        emit pThis->decoderStatsChanged();
        break;
    }
    case GST_MESSAGE_ELEMENT: {
        ...
        if (structure && gst_structure_has_name(structure, "qgc-caps-info")) {
            ...
            QMetaObject::invokeMethod(pThis, [pThis, format, resolution, src]() {
                for (auto *c : QGCQVideoSinkController::controllersOf(pThis)) {
                    if (static_cast<void*>(c->element()) == src) {
