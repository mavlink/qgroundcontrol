...
void GstVideoReceiver::_watchdog()
{
    _worker.dispatch([this]() {
        ...
        if (_decoding && !_removingDecoder) {
            ...
            elapsed = now - _lastSourceFrameTime;
...
GstElement *GstVideoReceiver::_makeSource(const QString &input)
{
    ...
    const bool isRtsp = input.contains("rtsp://", Qt::CaseInsensitive);
    ...
    do {
        ...
        if (isTcpMPEGTS) {
            ...
            if (source) {
                ...
                g_object_set(static_cast<gpointer>(source), "host", qPrintable(host), "port", port, nullptr);
            }
        } else if (isRtsp) {
            source = gst_element_factory_make("rtspsrc", "source");
            if (source) {
                g_object_set(static_cast<gpointer>(source), "location", qPrintable(input), "latency", 17, "udp-reconnect", 1, "timeout", _kUdpReconnect_us, NULL);
            }
