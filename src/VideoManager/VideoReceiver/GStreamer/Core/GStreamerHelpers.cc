...
bool isHardwareDecoderFactory(GstElementFactory* factory)
{
    ...
    static constexpr QLatin1String kHardwareTags[] = {
        QLatin1String("va"),   QLatin1String("nv"),     QLatin1String("qsv"),
        QLatin1String("msdk"), QLatin1String("vulkan"), QLatin1String("d3d"),
        QLatin1String("dxva"), QLatin1String("vtdec"),  QLatin1String("metal"),
    };
...
// Enumerate all video decoder factories once and apply two rank adjustments in
// a single pass: boost factories matching `requireHardware` to `prioritizedRank`
// and demote the opposite class to GST_RANK_NONE.
void prioritizeByHardwareClass(GstRegistry* registry, uint16_t prioritizedRank, bool requireHardware)
{
    ...
    for (GList* node = decoderFactories; node != nullptr; node = node->next) {
        ...
        if (isHW == requireHardware) {
            qCDebug(GStreamerHelpersLog) << "Boosting" << (requireHardware ? "hardware" : "software")
                                  << "decoder rank:" << featureName;
            changeFeatureRank(registry, featureName, prioritizedRank);
            ++matchedFactories;
        } else {
            qCDebug(GStreamerHelpersLog) << "Lowering" << (requireHardware ? "software" : "hardware")
                                  << "decoder rank:" << featureName;
...
void setCodecPriorities(VideoDecoderOptions option)
{
    ...
    switch (option) {
        ...
        case ForceVideoDecoderVAAPI:
            for (const char* name :
                 {"vaav1dec", "vah264dec", "vah265dec", "vajpegdec", "vampeg2dec", "vavp8dec", "vavp9dec"}) {
                changeFeatureRank(registry, name, PrioritizedRank);
            }
            break;
        case ForceVideoDecoderNVIDIA:
            for (const char* name : {"nvav1dec", "nvh264dec", "nvh265dec", "nvjpegdec", "nvmpeg2videodec",
                                     "nvmpeg4videodec", "nvmpegvideodec", "nvvp8dec", "nvvp9dec"}) {
                changeFeatureRank(registry, name, PrioritizedRank);
            }
            break;
        case ForceVideoDecoderDirectX3D:
            for (const char* name : {"d3d11av1dec", "d3d11h264dec", "d3d11h265dec", "d3d11mpeg2dec", "d3d11vp8dec",
                                     "d3d11vp9dec", "d3d12av1dec", "d3d12h264dec", "d3d12h265dec", "d3d12mpeg2dec",
                                     "d3d12vp8dec", "d3d12vp9dec", "dxvaav1decoder", "dxvah264decoder",
                                     "dxvah265decoder", "dxvampeg2decoder", "dxvavp8decoder", "dxvavp9decoder"}) {
                changeFeatureRank(registry, name, PrioritizedRank);
            }
            break;
        ...
        case ForceVideoDecoderIntel:
            for (const char* name :
                 {"qsvh264dec", "qsvh265dec", "qsvjpegdec", "qsvvp9dec", "msdkav1dec", "msdkh264dec", "msdkh265dec",
                  "msdkmjpegdec", "msdkmpeg2dec", "msdkvc1dec", "msdkvp8dec", "msdkvp9dec"}) {
                changeFeatureRank(registry, name, PrioritizedRank);
            }
            break;
