...
void populateAllocationQuery(GstQuery* query)
{
    ...
    GstVideoInfo vinfo;
    // DMA_DRM caps need the dma_drm-aware parser, else the min-buffer hint is dropped and va
    // stays on its copy-threshold pool.
    if (!GstHw::dmaDrmAwareVideoInfo(caps, &vinfo)) {
        return;
    }
    const gsize size = GST_VIDEO_INFO_SIZE(&vinfo);

    // ANY features means upstream hasn't committed to a memory type yet; treat it as HW so we
    // advertise the min-buffer hint instead of forcing a CPU pool that would defeat zero-copy.
    GstCapsFeatures* features = gst_caps_get_features(caps, 0);
    ...
    if (!need_pool) {
        // Upstream still needs to know which metas we accept even when we don't propose a pool.
        addConsumedAllocationMetas(query);
        return;
    }
