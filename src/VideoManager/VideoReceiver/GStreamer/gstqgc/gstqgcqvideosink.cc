...
void push_frame_queued(QVideoSink* sink, QVideoFrame&& frame)
{
    if (!sink)
        return;
    // Method-pointer overload guards a destroyed receiver internally — safe across the streaming
    // thread boundary where a QPointer captured into a lambda is not.
...
static GstFlowReturn gst_qgc_q_video_sink_show_frame(GstVideoSink* vsink, GstBuffer* buf)
{
    ...
    push_frame_queued(sink.data(), std::move(mapped.frame));
    return GST_FLOW_OK;
}
