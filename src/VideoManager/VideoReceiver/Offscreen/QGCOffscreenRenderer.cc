...
bool QGCOffscreenRenderer::load(const QUrl& qmlSource, const QSize& pixelSize)
{
    ...
    // initialize() creates the QRhi (scene graph not on a native surface). Must precede target setup.
...
QImage QGCOffscreenRenderer::renderToImage()
{
    ...
    // Documented QQuickRenderControl loop: polishItems -> beginFrame -> sync -> render -> endFrame.
    ...
    readback.completed = [&]() {
        ...
        result = img.copy();  // detach from the soon-to-be-freed readback buffer
        done = true;
    };
    ...
    // RHI Y-up vs QImage top-down: backends that report isYUpInFramebuffer() need a flip.
