...
void VideoManager::init(QQuickWindow *window)
{
    ...
    (void) connect(_videoSettings->lowLatencyMode(), &Fact::rawValueChanged, this, [this]() { _restartAllVideos(); });
...
bool VideoManager::_updateAutoStream(VideoReceiver *receiver)
{
    ...
    case VIDEO_STREAM_TYPE_RTSP:
        ...
        settingsChanged = _updateVideoUri(receiver, pInfo->uri());
        break;
    ...
    case VIDEO_STREAM_TYPE_RTPUDP: {
        QString url;
        if (pInfo->encoding() == VIDEO_STREAM_ENCODING_H264 || pInfo->encoding() == VIDEO_STREAM_ENCODING_UNKNOWN) {
            source = VideoSettings::videoSourceUDPH264;
            url = pInfo->uri().contains("udp://") ? pInfo->uri() : QStringLiteral("udp://0.0.0.0:%1").arg(pInfo->uri());
        } else if (pInfo->encoding() == VIDEO_STREAM_ENCODING_H265) {
            ...
            qCWarning(VideoManagerLog) << "Unknown encoding for RTPUDP";
            break;
        }

        settingsChanged = _updateVideoUri(receiver, url);
        break;
    }
    case VIDEO_STREAM_TYPE_MPEG_TS: {
        source = VideoSettings::videoSourceMPEGTS;
        const QString url = pInfo->uri().contains("mpegts://") ? pInfo->uri() : QStringLiteral("mpegts://0.0.0.0:%1").arg(pInfo->uri());
        settingsChanged = _updateVideoUri(receiver, url);
        break;
    }
    default:
