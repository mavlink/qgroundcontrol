...
VideoManager* VideoManager::instance() { return _videoManagerInstance(); }
...
void VideoManager::startRecording(const QString& videoFile) {
    ...
    if (!dir.exists()) {
        if (!QDir().mkpath(savePath)) {
            qgcApp()->showAppMessage(tr("Unable to record video. Save path '%1' cannot be created or is not writable.").arg(savePath));
            return;
        }
    }
...
bool VideoManager::isUvc() const { return (!_uvcVideoSourceID.isEmpty() && uvcEnabled() && hasVideo()); }
...
bool VideoManager::uvcEnabled() { return UVCReceiver::enabled(); }

bool VideoManager::qtmultimediaEnabled() { return QtMultimediaReceiver::enabled(); }
...
void VideoManager::initAfterQmlReady() {
...
void FinishVideoInitialization::run() {
    ...
    QMetaObject::invokeMethod(VideoManager::instance(), "initAfterQmlReady", Qt::QueuedConnection);
}
