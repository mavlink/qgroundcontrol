...
bool VideoManager::waitForVideoBackendReady(int timeoutMs)
{
    ...
    watcher.setFuture(_backendInitFuture);
...
bool VideoManager::_updateSettings(VideoReceiver *receiver)
{
    ...
    const int rtpLatencyMs = static_cast<int>(qMin(_videoSettings->rtpJitterLatencyMs()->rawValue().toUInt(), static_cast<uint>(INT_MAX)));
