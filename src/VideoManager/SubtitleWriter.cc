...
const int SubtitleWriter::_sampleRate
...
void SubtitleWriter::startCapturingTelemetry(const QString& videoFile)
{
    ...
    stream << QStringLiteral(
        ...
        "Style: "
        "Default,Monospace,30,&H00FFFFFF,&H000000FF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,2,2,1,10,10,10,1\n"
        "\n"
        "[Events]\n"
...
void SubtitleWriter::_captureTelemetry()
{
    ...
    for (const Fact* fact : _facts) {
        valuesStrings << QStringLiteral("%2 %3").arg(fact->cookedValueString()).arg(fact->cookedUnits());
