...
{
    // FIXME: What about a download in progress? Change _downloadManager to sub-object?
...
bool PX4FirmwareUpgrade::_findBoard(void)
{
    foreach (QSerialPortInfo info, QSerialPortInfo::availablePorts()) {
        ...
        // FIXME: Won't this || find a 3dr radio as well?
        // FIXME: Magic number
...
bool PX4FirmwareUpgrade::_bootloaderWrite(const char* data, qint64 maxSize, const QString errorPrefix)
{
    ...
    if (bytesWritten != maxSize) {
        _portErrorString = tr("Incorrect number of bytes returned for %1 write: %2").arg(errorPrefix).arg(bytesWritten);
        return false;
    }
...
bool PX4FirmwareUpgrade::_bootloaderProgram(void)
{
    ...
    while (bytesSent < (int)_imageSize) {
        ...
        // FIXME: Does this need any sort of padding on the end.
...
bool PX4FirmwareUpgrade::_bootloaderVerifyRev2(void)
{
#if 0
    uint8_t	file_buf[4];
    int count;
...
bool PX4FirmwareUpgrade::_bootloaderVerifyRev3(void)
{
    quint32 flashCRC;
    
    _bootloaderWrite(buf, 2, "PROTO_GET_CRC");
    _bootloaderRead((flashCRC, sizeof(crc), "PROTO_GET_CRC"));
