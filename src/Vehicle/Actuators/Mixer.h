...
public:
    ChannelConfig(QObject* parent, const MixerParameter& parameter, bool isActuatorTypeConfig = false)
        ...
        , _isActuatorTypeConfig(isActuatorTypeConfig)
    {
    }
...
public:
    ...
    ChannelConfigInstance* instantiate(int paramIndex, int actuatorTypeIndex, ParameterManager* parameterManager,
        std::function<void(Function, Fact*)> factAddedCb) override;
...
class MixerChannel : public QObject {
    Q_OBJECT
public:
    MixerChannel(QObject* parent, const QString& label, int actuatorFunction, int paramIndex, int actuatorTypeIndex,
