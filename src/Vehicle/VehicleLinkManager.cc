...
void VehicleLinkManager::_commRegainedOnLink(LinkInterface* link)
{
    ...
    if (_rgLinkInfo.count() > 1) {
        commRegainedMessage = tr("%1Communication regained on %2 link")
                                  .arg(_vehicle->_vehicleIdSpeech())
                                  .arg(isPrimaryLink ? tr("primary") : tr("secondary"));
    } else {
    ...
    if (_updatePrimaryLink()) {
        QString primarySwitchMessage
            = tr("%1Switching communication to new primary link").arg(_vehicle->_vehicleIdSpeech());
    }
...
void VehicleLinkManager::_commLostCheck(void)
{
    ...
    for (LinkInfo_t& linkInfo : _rgLinkInfo) {
        if (!linkInfo.commLost && !linkInfo.link->linkConfiguration()->isHighLatency()
            && linkInfo.heartbeatElapsedTimer.elapsed() > _heartbeatMaxElpasedMSecs) {
            ...
            if (_rgLinkInfo.count() > 1) {
                QString msg = tr("%1Communication lost on %2 link.")
                                  .arg(_vehicle->_vehicleIdSpeech())
                                  .arg(isPrimaryLink ? tr("primary") : tr("secondary"));
                _vehicle->_say(msg);
            }
