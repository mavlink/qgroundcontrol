...
public:
    ...
    QList<int> stickDisplayPositions() { return _stickDisplayPositions; }
    bool centeredThrottle() const { return _centeredThrottle; }

    void setTransmitterMode(int mode);
    void setCenteredThrottle(bool springLoaded);
...
private:
    ...
    void _saveCurrentRawValues();   ///< Saves the current channel values, so that we can detect when the use moves an input.
