...
const ComponentInformationManager::StateFn ComponentInformationManager::_rgStates[] = {
    ...
    ComponentInformationManager::_stateRequestCompInfoParam, ComponentInformationManager::_stateRequestCompInfoEvents,
    ComponentInformationManager::_stateRequestCompInfoActuators,
    ComponentInformationManager::_stateRequestAllCompInfoComplete};

const int ComponentInformationManager::_cStates
    = sizeof(ComponentInformationManager::_rgStates) / sizeof(ComponentInformationManager::_rgStates[0]);
...
const int RequestMetaDataTypeStateMachine::_cStates
    = sizeof(RequestMetaDataTypeStateMachine::_rgStates) / sizeof(RequestMetaDataTypeStateMachine::_rgStates[0]);
...
int ComponentInformationManager::stateCount(void) const { return _cStates; }

const ComponentInformationManager::StateFn* ComponentInformationManager::rgStates(void) const { return &_rgStates[0]; }
...
void ComponentInformationManager::requestAllComponentInformation(
...
void ComponentInformationManager::_stateRequestCompInfoGeneral(StateMachine* stateMachine)
{
    ...
    compMgr->_requestTypeStateMachine.request(
        compMgr->_compInfoMap[MAV_COMP_ID_AUTOPILOT1][COMP_METADATA_TYPE_GENERAL]);
}
...
void ComponentInformationManager::_stateRequestCompInfoComplete(void) { advance(); }
...
void ComponentInformationManager::_stateRequestCompInfoParam(StateMachine* stateMachine)
{
    ...
    if (compMgr->_isCompTypeSupported(COMP_METADATA_TYPE_PARAMETER)) {
        compMgr->_requestTypeStateMachine.request(
            compMgr->_compInfoMap[MAV_COMP_ID_AUTOPILOT1][COMP_METADATA_TYPE_PARAMETER]);
    } else {
...
void ComponentInformationManager::_stateRequestCompInfoEvents(StateMachine* stateMachine)
{
    ...
    if (compMgr->_isCompTypeSupported(COMP_METADATA_TYPE_EVENTS)) {
        compMgr->_requestTypeStateMachine.request(
            compMgr->_compInfoMap[MAV_COMP_ID_AUTOPILOT1][COMP_METADATA_TYPE_EVENTS]);
    } else {
...
void ComponentInformationManager::_stateRequestCompInfoActuators(StateMachine* stateMachine)
{
    ...
    if (compMgr->_isCompTypeSupported(COMP_METADATA_TYPE_ACTUATORS)) {
        compMgr->_requestTypeStateMachine.request(
            compMgr->_compInfoMap[MAV_COMP_ID_AUTOPILOT1][COMP_METADATA_TYPE_ACTUATORS]);
    } else {
...
{
    return qobject_cast<CompInfoGeneral*>(_compInfoMap[MAV_COMP_ID_AUTOPILOT1][COMP_METADATA_TYPE_GENERAL])
        ->isMetaDataTypeSupported(type);
}
...
int RequestMetaDataTypeStateMachine::stateCount(void) const { return _cStates; }
...
void RequestMetaDataTypeStateMachine::statesCompleted(void) const { _compMgr->_stateRequestCompInfoComplete(); }
...
static void _requestMessageResultHandlerDeprecated(void* resultHandlerData, MAV_RESULT result,
    Vehicle::RequestMessageResultHandlerFailureCode_t failureCode, const mavlink_message_t& message)
{
    RequestMetaDataTypeStateMachine* requestMachine = static_cast<RequestMetaDataTypeStateMachine*>(resultHandlerData);
    ...
    if (result == MAV_RESULT_ACCEPTED) {
        ...
        requestMachine->compInfo()->setUriMetaData(
            componentInformation.general_metadata_uri, componentInformation.general_metadata_file_crc);
    } else {
        switch (failureCode) {
        case Vehicle::RequestMessageFailureCommandError:
            qCDebug(ComponentInformationManagerLog)
                << QStringLiteral("MAV_CMD_REQUEST_MESSAGE COMPONENT_INFORMATION %1 error(%2)")
                       .arg(requestMachine->typeToString())
                       .arg(QGCMAVLink::mavResultToString(result));
            break;
        case Vehicle::RequestMessageFailureCommandNotAcked:
            qCDebug(ComponentInformationManagerLog) << QStringLiteral(
                "MAV_CMD_REQUEST_MESSAGE COMPONENT_INFORMATION %1 no response to command from vehicle")
                                                           .arg(requestMachine->typeToString());
            break;
        case Vehicle::RequestMessageFailureMessageNotReceived:
            qCDebug(ComponentInformationManagerLog) << QStringLiteral(
                "MAV_CMD_REQUEST_MESSAGE COMPONENT_INFORMATION %1 vehicle did not send requested message")
                                                           .arg(requestMachine->typeToString());
            break;
        default:
            break;
...
void RequestMetaDataTypeStateMachine::_stateRequestCompInfo(StateMachine* stateMachine)
{
    ...
    if (weakLink.expired()) {
        qCDebug(ComponentInformationManagerLog)
            << QStringLiteral("_stateRequestCompInfo Skipping component information %1 request due to no primary link")
                   .arg(requestMachine->typeToString());
        stateMachine->advance();
    } else {
        ...
        if (sharedLink->linkConfiguration()->isHighLatency() || sharedLink->isPX4Flow() || sharedLink->isLogReplay()) {
            qCDebug(ComponentInformationManagerLog)
                << QStringLiteral("_stateRequestCompInfo Skipping component information %1 request due to link type")
                       .arg(requestMachine->typeToString());
            stateMachine->advance();
        } else {
            qCDebug(ComponentInformationManagerLog)
                << "Requesting component metadata" << requestMachine->typeToString();
            vehicle->requestMessage(
                _requestMessageResultHandler, stateMachine, MAV_COMP_ID_AUTOPILOT1, MAVLINK_MSG_ID_COMPONENT_METADATA);
        }
    }
}
...
void RequestMetaDataTypeStateMachine::_stateRequestCompInfoDeprecated(StateMachine* stateMachine)
{
    ...
    if (weakLink.expired()) {
        ...
        if (sharedLink->linkConfiguration()->isHighLatency() || sharedLink->isPX4Flow() || sharedLink->isLogReplay()) {
            ...
            qCDebug(ComponentInformationManagerLog)
                << "Requesting component information" << requestMachine->typeToString();
            vehicle->requestMessage(_requestMessageResultHandlerDeprecated, stateMachine, MAV_COMP_ID_AUTOPILOT1,
...
QString RequestMetaDataTypeStateMachine::_downloadCompleteJsonWorker(const QString& fileName)
{
    ...
    if (fileName.endsWith(".lzma", Qt::CaseInsensitive) || fileName.endsWith(".xz", Qt::CaseInsensitive)) {
        outputFileName = (QDir(QStandardPaths::writableLocation(QStandardPaths::TempLocation))
                              .absoluteFilePath(_currentCacheFileTag));
...
{
    qCDebug(ComponentInformationManagerLog)
        << "RequestMetaDataTypeStateMachine::_ftpDownloadComplete fileName:errorMsg" << fileName << errorMsg;

    disconnect(_compInfo->vehicle->ftpManager(), &FTPManager::downloadComplete, this,
        &RequestMetaDataTypeStateMachine::_ftpDownloadComplete);
    disconnect(_compInfo->vehicle->ftpManager(), &FTPManager::commandProgress, this,
        &RequestMetaDataTypeStateMachine::_ftpDownloadProgress);
    if (errorMsg.isEmpty()) {
        if (_currentFileName) {
        ...
        qCWarning(ComponentInformationManagerLog)
            << "RequestMetaDataTypeStateMachine::_ftpDownloadComplete failed filename:errorMsg" << fileName << errorMsg;
    }
...
{
    qCDebug(ComponentInformationManagerLog)
        << "RequestMetaDataTypeStateMachine::_httpDownloadComplete remoteFile:localFile:errorMsg" << remoteFile
        << localFile << errorMsg;

    disconnect(qobject_cast<QGCFileDownload*>(sender()), &QGCFileDownload::downloadComplete, this,
    ...
    if (errorMsg.isEmpty()) {
        ...
        // Unit test should always succeed
        qCWarning(ComponentInformationManagerLog)
            << "RequestMetaDataTypeStateMachine::_httpDownloadCompleteMetaDataJson failed remoteFile:localFile:errorMsg"
            << remoteFile << localFile << errorMsg;
    }

    advance();
}

void RequestMetaDataTypeStateMachine::_requestFile(
    const QString& cacheFileTag, bool crcValid, const QString& uri, QString& outputFileName)
{
    ...
    if (_compInfo->available() && !uri.isEmpty()) {
        ...
        if (cachedFile.isEmpty()) {
            ...
            if (_uriIsMAVLinkFTP(uri)) {
                connect(ftpManager, &FTPManager::downloadComplete, this,
                    &RequestMetaDataTypeStateMachine::_ftpDownloadComplete);
                if (ftpManager->download(
                    ...
                    connect(ftpManager, &FTPManager::commandProgress, this,
                        &RequestMetaDataTypeStateMachine::_ftpDownloadProgress);
                } else {
                    qCWarning(ComponentInformationManagerLog)
                        << "RequestMetaDataTypeStateMachine::_requestFile FTPManager::download returned failure";
                    disconnect(ftpManager, &FTPManager::downloadComplete, this,
                        &RequestMetaDataTypeStateMachine::_ftpDownloadComplete);
                    advance();
                }
            } else {
                ...
                if (download->download(uri)) {
                    ...
                    qCWarning(ComponentInformationManagerLog)
                        << "RequestMetaDataTypeStateMachine::_requestFile QGCFileDownload::download returned failure";
                    disconnect(download, &QGCFileDownload::downloadComplete, this,
                        &RequestMetaDataTypeStateMachine::_httpDownloadComplete);
                    advance();
                }
            }
        ...
        qCDebug(ComponentInformationManagerLog)
            << "Skipping download. Component information not available for" << _currentCacheFileTag;
        advance();
    }
}
...
void RequestMetaDataTypeStateMachine::_stateRequestMetaDataJsonFallback(StateMachine* stateMachine)
{
    ...
    qCDebug(ComponentInformationManagerLog)
        << "RequestMetaDataTypeStateMachine::_stateRequestMetaDataJsonFallback: trying fallback download";
    ...
    requestMachine->_requestFile(
        fileTag, compInfo->crcMetaDataFallbackValid(), uri, requestMachine->_jsonMetadataFileName);
}
