...
void SendMavCommandWithSignallingTest::_testCaseWorker(TestCase_t& testCase)
{
    ...
    QCOMPARE(vehicle->_findMavCommandListEntryIndex(
                 MAV_COMP_ID_AUTOPILOT1, MockLink::MAV_CMD_MOCKLINK_ALWAYS_RESULT_ACCEPTED),
        -1);
...
void SendMavCommandWithSignallingTest::_duplicateCommand(void)
{
    ...
    QVERIFY(QTest::qWaitFor(
        [&]() { return _mockLink->sendMavCommandCount(MockLink::MAV_CMD_MOCKLINK_NO_RESPONSE) == 1; }, 10));
    QSignalSpy spyResult(vehicle, &Vehicle::mavCommandResult);
    vehicle->sendMavCommand(MAV_COMP_ID_AUTOPILOT1, MockLink::MAV_CMD_MOCKLINK_NO_RESPONSE, true /* showError */);
    ...
    QVERIFY(
        vehicle->_findMavCommandListEntryIndex(MAV_COMP_ID_AUTOPILOT1, MockLink::MAV_CMD_MOCKLINK_NO_RESPONSE) != -1);
}
