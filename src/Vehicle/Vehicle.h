...
signals:
    ...
    void BatMonDataChanged(uint8_t compid, uint16_t volt, int16_t current, uint8_t soc, float temp, uint16_t batStatus, uint32_t safetystatus, uint32_t operationstatus, uint16_t cellvolt1, uint16_t cellvolt2, uint16_t cellvolt3, uint16_t cellvolt4, uint16_t cellvolt5, uint16_t cellvolt6);
    ...
    void SensPowerBoardChanged(uint64_t timestamp, uint8_t status, uint8_t led_status, float system_volt, float servo_volt, float digital_volt, float mot_l_amp, float mot_r_amp, float analog_amp, float digital_amp, float ext_amp, float aux_amp);
    void thrustChanged(Vehicle* vehicle, double thrust); //Needed for energy widget, but could be avoided if we add a fact
