...
bool Vehicle::_apmArmingNotRequired()
{
    ...
    Fact* const param = _parameterManager->getParameter(ParameterManager::defaultComponentId, armingRequireParam);
...
int Vehicle::motorCount()
{
    ...
    if (_vehicleType == MAV_TYPE_SUBMARINE) {
        Fact* const param = parameterManager()->getParameter(_compID, "FRAME_CONFIG");
        if (param) {
            frameType = param->rawValue().toInt();
        }
    }
...
/*                                 Signing                                   */
/*===========================================================================*/
/* Wiring lives in VehicleSigningController; Vehicle just owns the instance. */

void Vehicle::_createSigningController()
{
    _signingController = new VehicleSigningController(this);
    connect(_vehicleLinkManager, &VehicleLinkManager::primaryLinkChanged, _signingController,
            &VehicleSigningController::onPrimaryLinkChanged);
}
