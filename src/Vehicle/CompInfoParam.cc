...
{
    qCDebug(CompInfoParamLog) << "setJson: metadataJsonFileName:translationJsonFileName" << metadataJsonFileName
                              << translationJsonFileName;

    if (metadataJsonFileName.isEmpty()) {
        // This will fall back to using the old FirmwarePlugin mechanism for parameter meta data.
        // In this case paramter metadata is loaded through the _parameterMajorVersionKnown call which happens after
        // parameter are downloaded
        return;
    }
    ...
    for (QJsonValue parameterValue : rgParameters) {
        ...
        if (!parameterValue.isObject()) {
            qCWarning(CompInfoParamLog) << "Metadata json read failed: compid:" << compId
...
FactMetaData* CompInfoParam::factMetaDataForName(const QString& name, FactMetaData::ValueType_t type)
{
    ...
    if (_noJsonMetadata) {
        ...
        if (opaqueMetaData) {
            factMetaData
                = vehicle->firmwarePlugin()->_getMetaDataForFact(opaqueMetaData, name, type, vehicle->vehicleType());
        }
    }
...
FirmwarePlugin* CompInfoParam::_anyVehicleTypeFirmwarePlugin(MAV_AUTOPILOT firmwareType)
{
    ...
    MAV_TYPE anySupportedVehicleType = QGCMAVLink::vehicleClassToMavType(
        pluginMgr->supportedVehicleClasses(QGCMAVLink::firmwareClass(firmwareType))[0]);

    return pluginMgr->firmwarePluginForAutopilot(firmwareType, anySupportedVehicleType);
}

QString CompInfoParam::_parameterMetaDataFile(
    Vehicle* vehicle, MAV_AUTOPILOT firmwareType, int& majorVersion, int& minorVersion)
{
    ...
    if (firmwareType != MAV_AUTOPILOT_PX4) {
        ...
        QFile cacheFile(cacheDir.filePath(
            QString("%1.%2.%3.xml").arg(_cachedMetaDataFilePrefix).arg(firmwareType).arg(wantedMajorVersion)));
        if (cacheFile.exists()) {
            ...
            if (wantedMajorVersion != cacheMajorVersion) {
                qWarning() << "Parameter meta data cache corruption:" << cacheFile.fileName()
                           << "major version does not match file name"
                           << "actual:excepted" << cacheMajorVersion << wantedMajorVersion;
            } else {
                qCDebug(CompInfoParamLog) << "Direct cache hit on file:major:minor" << cacheFile.fileName()
                                          << cacheMajorVersion << cacheMinorVersion;
                cacheHit = true;
            }
        }
        ...
        if (!cacheHit) {
            ...
            if (cacheHitIndex != -1) {
                ...
                if (majorVersion != cacheMajorVersion) {
                    qWarning() << "Parameter meta data cache corruption:" << cacheFile.fileName()
                               << "major version does not match file name"
                               << "actual:excepted" << majorVersion << cacheMajorVersion;
                    cacheHit = false;
                } else {
                    qCDebug(CompInfoParamLog) << "Indirect cache hit on file:major:minor:want" << cacheFile.fileName()
                                              << cacheMajorVersion << cacheMinorVersion << wantedMajorVersion;
                    cacheHit = true;
                }
            }
        }
        ...
        if (cacheHit) {
            // Cache hit is available, we need to check if internal meta data is a better match, if so use internal
            // version
        ...
        qCDebug(CompInfoParamLog) << "_parameterMetaDataFile returning file:major:minor" << metaDataFile << majorVersion
                                  << minorVersion;
...
void CompInfoParam::_cachePX4MetaDataFile(const QString& metaDataFile)
{
    ...
    qCDebug(CompInfoParamLog) << "ParameterManager::cacheMetaDataFile file:major;minor" << metaDataFile
    ...
    qCDebug(CompInfoParamLog) << "ParameterManager::cacheMetaDataFile cacheHit file:firmware:major;minor" << cacheHit
                              << cacheMajorVersion << cacheMinorVersion;
    ...
    if (cacheHit.isEmpty()) {
        ...
        //      Cache new file if newer/equal minor version. We cache if equal to allow flashing test builds with new
        //      parameter metadata. Also delete older cache file.
    ...
    if (cacheNewFile) {
        ...
        QDir cacheDir = QFileInfo(settings.fileName()).dir();
        QFile cacheFile(cacheDir.filePath(
            QString("%1.%2.%3.xml").arg(_cachedMetaDataFilePrefix).arg(MAV_AUTOPILOT_PX4).arg(newMajorVersion)));
