...
{
    // Broadcast only when every ESC is selected and all share the same pending
    // changes. This is intentionally strict — partial selection always uses
    // individual writes to avoid accidentally overwriting ESCs the user didn't select.
...
void AM32EepromFactGroupListModel::requestWriteAll(Vehicle* vehicle, const QList<int>& escIndices)
{
    ...
    if (_allEscsHaveMatchingChanges(escIndices)) {
        ...
        qCDebug(AM32EepromLog) << "All ESCs have matching changes, using broadcast write";
        ...
        qCDebug(AM32EepromLog) << "ESCs have different changes, writing individually";
...
void AM32EepromFactGroup::_handleEscEeprom(Vehicle *vehicle, const mavlink_message_t &message)
{
    ...
    if (eeprom.length != AM32EepromFactGroupListModel::kAM32EepromSize) {
    ...
    AM32EepromSchema* schema = AM32EepromSchema::instance();
    QString fwVersion = firmwareVersionString();
    for (AM32Setting* setting : _settings) {
        const AM32FieldDef* baseDef = schema->field(setting->name());
        if (baseDef && (!baseDef->versionOverrides.isEmpty() || !baseDef->firmwareVersionOverrides.isEmpty())) {
            AM32FieldDef updatedDef = *baseDef;
            updatedDef.applyVersionOverrides(eepromVer);
            updatedDef.applyFirmwareVersionOverrides(fwVersion);
...
void AM32EepromFactGroupListModel::_updateMajorityMatches()
{
    ...
    for (const QString& settingName : settingNames) {
        ...
        // Find the most common value. On ties, the lowest raw value wins
        // (QMap iterates in key order), so behavior is deterministic.
    ...
    , _escIndex(escIndex)
{
    ...
    AM32EepromSchema* schema = AM32EepromSchema::instance();
    // Connect first, then check — avoids race where schema loads between check and connect.
    // _initializeSettingsFromSchema() guards against double-init via !_settings.isEmpty().
...
const AM32Setting* AM32EepromFactGroup::getSetting(const QString& name) const
{
...
bool AM32EepromFactGroup::settingsMatch(const AM32EepromFactGroup* other) const
{
...
QByteArray AM32EepromFactGroup::getEepromData() const
{
    ...
    if (data.isEmpty()) {
        data = QByteArray(AM32EepromFactGroupListModel::kAM32EepromSize, 0);
        data[0] = 1; // Start byte
    }
