...
struct AM32FieldDef {
    ...
    // Firmware version-specific overrides (firmwareVersion string -> override data)
    QMap<QString, QJsonObject> firmwareVersionOverrides;
    ...
    /// Apply firmware version-specific overrides (e.g. enum values for firmware:2.18+)
    void applyFirmwareVersionOverrides(const QString& firmwareVersion);

private:
    void _applyOverrideObject(const QJsonObject& override);
};
