...
{
    if (!_instance) {
        _instance = new AM32EepromSchema(qApp);
    }
    return _instance;
}
...
bool AM32EepromSchema::parseSchema(const QJsonObject& root)
{
    ...
    qCDebug(AM32EepromLog) << "AM32 Schema loaded:" << _fields.count() << "fields," << _groups.count() << "groups";
    return true;
}
...
AM32FieldDef AM32EepromSchema::parseField(const QString& name, const QJsonObject& fieldObj)
{
    ...
    for (auto it = versionsObj.begin(); it != versionsObj.end(); ++it) {
        ...
        if (versionKey.startsWith("eeprom:")) {
            ...
            verStr.remove('+');
            bool ok = false;
            int ver = verStr.toInt(&ok);
            if (!ok) {
                qCWarning(AM32EepromLog) << "Invalid eeprom version override key:" << versionKey;
                continue;
            }
            field.versionOverrides.insert(ver, it.value().toObject());
        }
        else if (versionKey.startsWith("firmware:")) {
            QString verStr = versionKey.mid(9);
            verStr.remove('+');
            field.firmwareVersionOverrides.insert(verStr, it.value().toObject());
        }
...
static QPair<int, int> parseFirmwareVersion(const QString& v)
{
    QString cleaned = v;
    cleaned.remove('v').remove('V');
    QStringList parts = cleaned.split('.');
    return qMakePair(parts.value(0).toInt(), parts.value(1).toInt());
}

static int compareFirmwareVersions(QPair<int, int> a, QPair<int, int> b)
{
    if (a.first != b.first) return a.first - b.first;
    return a.second - b.second;
}

void AM32FieldDef::_applyOverrideObject(const QJsonObject& override)
{
...
void AM32FieldDef::applyVersionOverrides(int eepromVersion)
{
    ...
    // Start at -1 so the "default" override (stored at key 0) can be selected
    int bestVersion = -1;
    ...
    if (bestOverride) {
        _applyOverrideObject(*bestOverride);
    }
}

void AM32FieldDef::applyFirmwareVersionOverrides(const QString& firmwareVersion)
{
    if (firmwareVersionOverrides.isEmpty()) {
        return;
    }

    QPair<int, int> current = parseFirmwareVersion(firmwareVersion);
    QPair<int, int> bestVer = qMakePair(-1, -1);
    QJsonObject* bestOverride = nullptr;

    for (auto it = firmwareVersionOverrides.begin(); it != firmwareVersionOverrides.end(); ++it) {
        QPair<int, int> overrideVer = parseFirmwareVersion(it.key());
        if (compareFirmwareVersions(overrideVer, current) <= 0 &&
            compareFirmwareVersions(overrideVer, bestVer) > 0) {
            bestVer = overrideVer;
            bestOverride = &it.value();
        }
    }
...
bool AM32FieldDef::isAvailableForFirmwareVersion(const QString& version) const
{
    ...
    QPair<int, int> current = parseFirmwareVersion(version);

    if (!minFirmwareVersion.isEmpty()) {
        QPair<int, int> minVer = parseFirmwareVersion(minFirmwareVersion);
        if (compareFirmwareVersions(current, minVer) < 0) {
            return false;
        }
    }

    if (!maxFirmwareVersion.isEmpty()) {
        QPair<int, int> maxVer = parseFirmwareVersion(maxFirmwareVersion);
        if (compareFirmwareVersions(current, maxVer) > 0) {
            return false;
        }
    }
