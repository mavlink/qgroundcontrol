...
void GimbalController::_handleGimbalManagerInformation(const mavlink_message_t &message)
{
    ...
    auto pair = _potentialGimbals.find(pairId);
    if (pair == _potentialGimbals.constEnd()) {
        pair = _potentialGimbals.insert(pairId, new Gimbal(this));
    }

    Gimbal *const gimbal = pair.value();
...
void GimbalController::_handleGimbalDeviceAttitudeStatus(const mavlink_message_t &message)
{
    ...
    if (attitude_status.gimbal_device_id == 0) {
        ...
        const auto foundGimbal = std::find_if(_potentialGimbals.constBegin(), _potentialGimbals.constEnd(), [pairId](const Gimbal *gimbal) {
            return (gimbal->deviceId()->rawValue().toUInt() == pairId.deviceId);
        });
...
void GimbalController::_checkComplete(Gimbal &gimbal, GimbalPairId pairId)
{
    ...
    if (!gimbal._receivedStatus && (gimbal._requestStatusRetries > 0) && (now - lastRequestStatusMessage) > 1000) {
        ...
        _vehicle->sendMavCommand(
            pairId.managerCompid,
            MAV_CMD_SET_MESSAGE_INTERVAL,
            false /* no error */,
            MAVLINK_MSG_ID_GIMBAL_MANAGER_STATUS,
            (gimbal._requestStatusRetries > 2) ? 0 : 5000000
         ); // request default rate, if we don't succeed, last attempt is fixed 0.2 Hz instead

        --gimbal._requestStatusRetries;
        qCDebug(GimbalControllerLog) << "attempt to set GIMBAL_MANAGER_STATUS message at"
                                     << (gimbal._requestStatusRetries > 2 ? "default rate" : "0.2 Hz")
                                     << " interval for device: "
...
void GimbalController::sendPitchAbsoluteYaw(float pitch, float yaw, bool showError)
{
    ...
    // qCDebug(GimbalControllerLog) << "sendPitch:" << pitch << "absoluteYaw:" << yaw;

    constexpr unsigned flags = GIMBAL_MANAGER_FLAGS_ROLL_LOCK
        | GIMBAL_MANAGER_FLAGS_PITCH_LOCK
