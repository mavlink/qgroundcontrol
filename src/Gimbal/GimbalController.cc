...
void GimbalController::_requestDeviceInformationResultHandler(void* resultHandlerData, MAV_RESULT result, VehicleTypes::RequestMessageResultHandlerFailureCode_t failureCode, const mavlink_message_t& message)
{
    ...
    if (result != MAV_RESULT_ACCEPTED) {
        qCDebug(GimbalControllerLog) << "GIMBAL_DEVICE_INFORMATION request failed - result:" << result << "failureCode:" << failureCode;
    }
}
...
void GimbalController::_checkComplete(Gimbal &gimbal, GimbalPairId pairId)
{
    ...
    if (pairId.deviceId != 0) {
        ...
        _vehicle->requestMessage(_requestDeviceInformationResultHandler,
                                 nullptr,
                                 gimbalDeviceCompid,
