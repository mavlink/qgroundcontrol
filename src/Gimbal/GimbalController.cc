...
void GimbalController::sendPitchYawFlags(uint32_t flags)
{
    ...
    _vehicle->sendMavCommand(
                ...
                MAV_CMD_DO_GIMBAL_MANAGER_PITCHYAW,
                true,
                qDegreesToRadians(_activeGimbal->absolutePitch()->rawValue().toFloat()),
                qDegreesToRadians(yaw_in_vehicle_frame ? _activeGimbal->bodyYaw()->rawValue().toFloat() : _activeGimbal->absoluteYaw()->rawValue().toFloat()),
                NAN,
                NAN,
                flags,
                0,
