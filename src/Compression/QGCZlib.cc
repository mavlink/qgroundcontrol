...
bool QGCZlib::inflateGzipFile(const QString& gzippedFileName, const QString& decompressedFilename)
{
    ...
    if (!outputFile.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        qWarning() << "QGCZlib::inflateGzipFile: open input file failed" << outputFile.fileName()
    ...
    do {
        ...
        do {
            ...
            if (cBytesWritten != cBytesInflated) {
                qWarning() << "QGCZlib::inflateGzipFile: output file write failed:" << outputFile.fileName()
                           << outputFile.errorString();
                goto Error;
            }
