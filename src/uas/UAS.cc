...
UAS::UAS(MAVLinkProtocol* protocol, Vehicle* vehicle, FirmwarePluginManager* firmwarePluginManager)
    : UASInterface()
    , lipoFull(4.2f)
    , lipoEmpty(3.5f)
    , uasId(vehicle->id())
 ...
 */
int UAS::getUASID() const { return uasId; }
 ...
 */
quint64 UAS::getUnixTimeFromMs(quint64 time) { return getUnixTime(time * 1000); }
...
void UAS::shutdownVehicle(void) { _vehicle = nullptr; }
