...
void TerrainProfile::componentComplete(void) { QQuickItem::componentComplete(); }
...
void TerrainProfile::setMissionController(MissionController* missionController)
{
    if (missionController != _missionController) {
        ...
        connect(_missionController, &MissionController::recalcTerrainProfile, this, &TerrainProfile::_updateSignal,
            Qt::QueuedConnection);
    }
}
...
void TerrainProfile::_createGeometry(
    QSGGeometryNode*& geometryNode, QSGGeometry*& geometry, QSGGeometry::DrawingMode drawingMode, const QColor& color)
{
...
void TerrainProfile::_updateSegmentCounts(FlightPathSegment* segment, int& cFlightProfileSegments,
    int& cTerrainProfilePoints, int& cMissingTerrainSegments, int& cTerrainCollisionSegments, double& minTerrainHeight,
...
void TerrainProfile::_addTerrainProfileSegment(FlightPathSegment* segment, double currentDistance, double amslAltRange,
    QSGGeometry::Point2D* terrainVertices, int& terrainProfileVertexIndex)
{
    ...
    for (int heightIndex = 0; heightIndex < segment->amslTerrainHeights().count(); heightIndex++) {
        ...
        // Move along the y axis which is a view or terrain height as a percentage between the min/max AMSL altitude for
        // all segments
...
void TerrainProfile::_addMissingTerrainSegment(FlightPathSegment* segment, double currentDistance,
    QSGGeometry::Point2D* missingTerrainVertices, int& missingterrainProfileVertexIndex)
{
    if (_shouldAddMissingTerrainSegment(segment)) {
        float x = currentDistance * _pixelsPerMeter;
        float y = height();
        missingTerrainVertices[missingterrainProfileVertexIndex++].set(x, y);
        missingTerrainVertices[missingterrainProfileVertexIndex++].set(
            x + (segment->totalDistance() * _pixelsPerMeter), y);
    }
}

void TerrainProfile::_addTerrainCollisionSegment(FlightPathSegment* segment, double currentDistance,
    double amslAltRange, QSGGeometry::Point2D* terrainCollisionVertices, int& terrainCollisionVertexIndex)
{
    if (segment->terrainCollision()) {
...
void TerrainProfile::_addFlightProfileSegment(FlightPathSegment* segment, double currentDistance, double amslAltRange,
    QSGGeometry::Point2D* flightProfileVertices, int& flightProfileVertexIndex)
{
    if (!_shouldAddFlightProfileSegment(segment)) {
        return;
    }
...
QSGNode* TerrainProfile::updatePaintNode(QSGNode* oldNode, QQuickItem::UpdatePaintNodeData* /*updatePaintNodeData*/)
{
    ...
    for (int viIndex = 0; viIndex < _visualItems->count(); viIndex++) {
        ...
        if (visualItem->simpleFlightPathSegment()) {
            ...
            _updateSegmentCounts(segment, cFlightProfileSegments, cTerrainProfilePoints, cMissingTerrainSegments,
                cTerrainCollisionSegments, minTerrainHeight, maxTerrainHeight);
        }
    ...
    qCDebug(TerrainProfileLog) << "missionController min/max" << _missionController->minAMSLAltitude()
                               << _missionController->maxAMSLAltitude();
    qCDebug(TerrainProfileLog) << QStringLiteral(
        "updatePaintNode counter:%1 cFlightProfileSegments:%2 cTerrainProfilePoints:%3 cMissingTerrainSegments:%4 "
        "cTerrainCollisionSegments:%5 _minAMSLAlt:%6 _maxAMSLAlt:%7 maxTerrainHeight:%8")
                                      .arg(counter++)
                                      .arg(cFlightProfileSegments)
                                      .arg(cTerrainProfilePoints)
                                      .arg(cMissingTerrainSegments)
                                      .arg(cTerrainCollisionSegments)
                                      .arg(_minAMSLAlt)
                                      .arg(_maxAMSLAlt)
                                      .arg(maxTerrainHeight);
            ...
            if (complexItem->flightPathSegments()->count() == 0) {
                ...
                for (int segmentIndex = 0; segmentIndex < complexItem->flightPathSegments()->count(); segmentIndex++) {
                    FlightPathSegment* segment
                        = complexItem->flightPathSegments()->value<FlightPathSegment*>(segmentIndex);

                    _addFlightProfileSegment(
                        segment, currentDistance, amslAltRange, flightProfileVertices, flightProfileVertexIndex);
                    _addTerrainProfileSegment(
                        segment, currentDistance, amslAltRange, terrainProfileVertices, terrainProfileVertexIndex);
                    _addMissingTerrainSegment(
                        segment, currentDistance, missingTerrainVertices, missingterrainProfileVertexIndex);
                    _addTerrainCollisionSegment(
                        segment, currentDistance, amslAltRange, terrainCollisionVertices, terrainCollisionVertexIndex);
