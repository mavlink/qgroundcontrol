...
public:
    ...
    //  This is the setting group name for default settings which are used when the user has not modified anything from
    //  the default setup. These settings will be overwritten prior to each use by the call to
    //  QGCCorePlugin::FactValueGridCreateDefaultSettings.

    // userSettingsGroup:
    //  This is the settings group name for user modified settings. Settings will be saved to here whenever the user
    //  modified anything. Also at that point in time the defaults settings group will be removed.

    // The combination of the two valuePage*SettingsGroup values allows each FactValueGrid to have it's own persistence
    // space.
    ...
    // This is only exposed for usage of FactValueGrid to be able to just read the settings and display no ui. For this
    // case create a FactValueGrid object with a null parent. Set the userSettingsGroup/defaultSettingsGroup
    // appropriately and then call _loadSettings. Then after that you can read the settings from the object. You should
    // not change any of the values. Destroy the FactValueGrid object when done.
