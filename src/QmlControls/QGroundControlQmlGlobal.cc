...
bool QGroundControlQmlGlobal::singleVehicleSupport(void)
{
    if (singleFirmwareSupport()) {
        return _firmwarePluginManager->supportedVehicleClasses(_firmwarePluginManager->supportedFirmwareClasses()[0])
                   .count()
            == 1;
    }
...
QString QGroundControlQmlGlobal::altitudeModeExtraUnits(AltMode altMode)
{
    ...
    case AltitudeModeMixed:
        qWarning() << "Internal Error: QGroundControlQmlGlobal::altitudeModeExtraUnits called with altMode == "
                      "AltitudeModeMixed";
        return QString();
    }
