...
GPSManager::~GPSManager() { disconnectGPS(); }
...
void GPSManager::connectGPS(const QString& device, const QString& gps_type)
{
    ...
    _gpsProvider = new GPSProvider(device, type, true, /* enableSatInfo */
        ...
        rtkSettings->fixedBasePositionAccuracy()->rawValue().toFloat(), _requestGpsStop);
    _gpsProvider->start();
...
{
    qCDebug(RTKGPSLog) << QString("GPS: got position update: alt=%1, long=%2, lat=%3")
                              .arg(msg.position_data.alt)
                              .arg(msg.position_data.lon)
                              .arg(msg.position_data.lat);
}
