...
#include "NTRIPConnectionSettings.h"
...
void NTRIPManager::init()
{
    ...
    const auto configureGga = [this] {
        _ggaProvider.configure(
            {static_cast<NTRIPGgaProvider::PositionSource>(_settings->ntripGgaPositionSource()->rawValue().toUInt()),
             std::chrono::milliseconds(_settings->ntripGgaIntervalSec()->rawValue().toLongLong() * 1000)});
    };
...
void NTRIPManager::startNTRIP()
{
    ...
    const auto config = ntripTransportConfig(*_settings);
...
void NTRIPManager::fetchMountpoints()
{
    ...
    _sourceTableController.fetch(ntripTransportConfig(*_settings), coordinate);
}
