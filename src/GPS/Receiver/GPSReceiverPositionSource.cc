...
QGC_LOGGING_CATEGORY(GPSReceiverPositionSourceLog, "GPS.Receiver.GPSReceiverPositionSource")

GPSReceiverPositionSource::GPSReceiverPositionSource(QObject* parent)
    : QGeoPositionInfoSource(parent)
    , _requestTimer(this)
    , _updateTimer(this)
{
    qCDebug(GPSReceiverPositionSourceLog) << this;
...
GPSReceiverPositionSource::~GPSReceiverPositionSource()
{
    qCDebug(GPSReceiverPositionSourceLog) << this;
}

QGeoPositionInfo GPSReceiverPositionSource::lastKnownPosition(bool /*fromSatellitePositioningMethodsOnly*/) const
{
    return _lastPosition;
}

void GPSReceiverPositionSource::startUpdates()
{
    if (_started) {
        return;
    }
...
void GPSReceiverPositionSource::stopUpdates()
{
    _started = false;
    _updateTimer.stop();
    _pendingFix.reset();
...
void GPSReceiverPositionSource::setUpdateInterval(int msec)
{
...
void GPSReceiverPositionSource::requestUpdate(int timeout)
{
...
void GPSReceiverPositionSource::reset()
{
    _lastPosition = {};
    _pendingFix.reset();
    _updateTimer.stop();
...
void GPSReceiverPositionSource::updatePosition(const GPSObservation& fix)
{
    ...
    if (_lastPosition.isValid()) {
        ...
        const bool requested = _requestTimer.isActive();
        _requestTimer.stop();
        qCDebug(GPSReceiverPositionSourceLog)
...
void GPSReceiverPositionSource::_emitPendingUpdate()
{
...
void GPSReceiverPositionSource::_reportUpdateTimeout()
{
...
QGeoPositionInfo GPSReceiverPositionSource::_positionInfo(const GPSObservation& fix)
{
