...
GPSIntegrityFactGroup::GPSIntegrityFactGroup(GPSIntegrityStore& store, QObject* parent)
    : FactGroup(1000, QStringLiteral(":/json/GPS/Integrity/GPSFact.json"), parent)
{
    ...
    if (store.thread() != thread()) {
        qCWarning(GPSIntegrityFactGroupLog) << "Store must share the projection thread";
    } else {
        _store = &store;
...
void GPSIntegrityFactGroup::_refresh()
{
    ...
    _availabilityNotificationPending |=
        _available != available || _systemErrorsKnown != observation.systemErrors.has_value();
    ...
    if (guard && revision == _revision && _availabilityNotificationPending) {
        _availabilityNotificationPending = false;
