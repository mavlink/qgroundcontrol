    ...
    while (!_requestStop) {
        ...
        if (_type == GPSType::trimble) {
            ...
            gpsDriver = new GPSDriverUBX(
                GPSDriverUBX::Interface::UART, &callbackEntry, this, &_reportGpsPos, _pReportSatInfo);
            baudrate = 0; // auto-configure
        }
        gpsDriver->setSurveyInSpecs(_surveyInAccMeters * 10000.0f, _surveryInDurationSecs);

        if (_useFixedBaseLoction) {
            gpsDriver->setBasePosition(
                _fixedBaseLatitude, _fixedBaseLongitude, _fixedBaseAltitudeMeters, _fixedBaseAccuracyMeters * 1000.0f);
        }
...
GPSProvider::GPSProvider(const QString& device, GPSType type, bool enableSatInfo, double surveyInAccMeters,
    int surveryInDurationSecs, bool useFixedBaseLocation, double fixedBaseLatitude, double fixedBaseLongitude,
    float fixedBaseAltitudeMeters, float fixedBaseAccuracyMeters, const std::atomic_bool& requestStop)
    : _device(device)
    , _type(type)
    ...
    , _fixedBaseAccuracyMeters(fixedBaseAccuracyMeters)
{
    ...
    if (enableSatInfo)
        _pReportSatInfo = new satellite_info_s();
}
...
int GPSProvider::callback(GPSCallbackType type, void* data1, int data2)
{
    ...
    case GPSCallbackType::surveyInStatus: {
        ...
        qCDebug(RTKGPSLog) << QString("Survey-in status: %1s cur accuracy: %2mm valid: %3 active: %4")
                                  .arg(status->duration)
                                  .arg(status->mean_accuracy)
                                  .arg((int)(status->flags & 1))
                                  .arg((int)((status->flags >> 1) & 1));
        emit surveyInStatus(status->duration, status->mean_accuracy, status->latitude, status->longitude,
            status->altitude, (int)(status->flags & 1), (int)((status->flags >> 1) & 1));
    } break;
