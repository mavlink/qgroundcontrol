...
/// Read-only projection of a store in the same thread.
...
public:
    explicit GPSRelativePositionModel(GPSRelativePositionStore& store, QObject* parent = nullptr);
...
private:
    ...
    void _project();
    std::array<QVariant, 23> _values() const;
