...
void GPSSourceHealth::setFreshnessTimeoutMs(int timeoutMs)
{
    ...
    if (!_positionInvalidated) {
        ...
        // A cleared or expired count must not be revived by changing the timeout.
        _updateFixSatelliteCount(_fixSatellitesInUseCount, ageMs);
        _schedulePositionExpiry();
        if (previousUsed != satellitesInUseCount()) {
            emit satellitesChanged();
        }
        if (guard && revision == _revision && ageMs >= _freshnessTimeoutMs) {
            _setState(Stale);
        }
    }
}
