...
GPSIntegrityStore::GPSIntegrityStore(QObject* parent, RuntimeScheduler* scheduler)
    : QObject(parent), _scheduler(scheduler ? scheduler : new QtRuntimeScheduler(this)), _expiryTask(_scheduler, this)
{
    qCDebug(GPSIntegrityStoreLog) << this;
    if (_scheduler->thread() != thread()) {
        qCWarning(GPSIntegrityStoreLog) << "Scheduler must share the store thread";
        _scheduler = nullptr;
        return;
    }
