...
{
    connect(this, &QThread::finished, this, [](){ qCDebug(NTRIPLog) << "NTRIP thread finished"; });
...
{
    if (isRunning()) {
        // As a last resort, stop the event loop and wait.
        quit();
        wait();
    }
...
void NTRIPTCPLink::requestStop()
{
    ...
    quit(); // stop the thread's event loop
}
...
void NTRIPManager::startNTRIP()
{
    ...
    _tcpLink = new NTRIPTCPLink(host,
                                port,
                                user,
                                pass,
                                mount,
...
void NTRIPManager::stopNTRIP()
{
    ...
    if (_tcpLink) {
        ...
        if (_tcpLink->isRunning()) {
            bool ok = QMetaObject::invokeMethod(_tcpLink, "requestStop",
                                                Qt::BlockingQueuedConnection);
            Q_UNUSED(ok);
        } else {
            _tcpLink->requestStop();
        }

        delete _tcpLink;   // safe after requestStop() quit the event loop
        _tcpLink = nullptr;
