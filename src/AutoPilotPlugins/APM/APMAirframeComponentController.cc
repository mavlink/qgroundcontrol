...
///     @param[in,out] frameType Specified frame type, or -1 to match first item in list (frameType found will be
///     returned)
...
APMAirframeComponentController::APMAirframeComponentController(void)
    : _frameClassFact(
        getParameterFact(FactSystem::defaultComponentId, QStringLiteral("FRAME_CLASS"), false /* reportMissing */))
    , _frameTypeFact(
          getParameterFact(FactSystem::defaultComponentId, QStringLiteral("FRAME_TYPE"), false /* reportMissing */))
...
void APMAirframeComponentController::_fillFrameClasses()
{
    ...
    if (qobject_cast<ArduCopterFirmwarePlugin*>(fwPlugin)) {
        ...
        frameTypeNotSupported << FRAME_CLASS_HELI << FRAME_CLASS_SINGLECOPTER << FRAME_CLASS_COAXCOPTER
                              << FRAME_CLASS_BICOPTER << FRAME_CLASS_HELI_DUAL << FRAME_CLASS_HELIQUAD;
        ...
        for (int i = 1; i < _frameClassFact->enumStrings().count(); i++) {
            ...
            _frameClassModel->append(
                new APMFrameClass(frameClassName, true /* copter */, frameClass, _frameTypeFact, _frameClassModel));
        }
        ...
        for (int i = 1; i < _frameClassFact->enumStrings().count(); i++) {
            ...
            int frameClass = _frameClassFact->enumValues()[i].toInt();
            _frameClassModel->append(
                new APMFrameClass(frameClassName, false /* copter */, frameClass, _frameTypeFact, _frameClassModel));
        }
    }
}
...
void APMAirframeComponentController::loadParameters(const QString& paramFile)
{
    ...
    QString paramFileUrl
        = QStringLiteral("https://api.github.com/repos/ArduPilot/ardupilot/contents/Tools/Frame_params/%1?ref=master");

    QGCFileDownload* downloader = new QGCFileDownload(this);
    connect(downloader, &QGCFileDownload::downloadComplete, this,
        &APMAirframeComponentController::_githubJsonDownloadComplete);
    downloader->download(paramFileUrl.arg(paramFile));
}

void APMAirframeComponentController::_githubJsonDownloadComplete(
    QString /*remoteFile*/, QString localFile, QString errorMsg)
{
    if (errorMsg.isEmpty()) {
        ...
        QGCFileDownload* downloader = new QGCFileDownload(this);
        connect(downloader, &QGCFileDownload::downloadComplete, this,
            &APMAirframeComponentController::_paramFileDownloadComplete);
...
void APMAirframeComponentController::_paramFileDownloadComplete(
    QString /*remoteFile*/, QString localFile, QString errorMsg)
{
    if (errorMsg.isEmpty()) {
            ...
            if (pFrameToImageInfo->frameClass == frameClass) {
                if (_defaultFrameType == -1) {
                    ...
                    _imageResourceDefault
                        = QStringLiteral("/qmlimages/Airframe/%1").arg(pFrameToImageInfo->imageResource);
                }
