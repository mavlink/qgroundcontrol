...
APMRadioComponent::APMRadioComponent(Vehicle* vehicle, AutoPilotPlugin* autopilot, QObject* parent)
    ...
    , _name(tr("Radio"))
{
    _mapParams << QStringLiteral("RCMAP_ROLL") << QStringLiteral("RCMAP_PITCH") << QStringLiteral("RCMAP_YAW")
...
QString APMRadioComponent::name(void) const { return _name; }

QString APMRadioComponent::description(void) const
{
    return tr("The Radio Component is used to setup which channels on your RC Transmitter you will use for each "
...
QString APMRadioComponent::iconResource(void) const { return QStringLiteral("/qmlimages/RadioComponentIcon.png"); }

bool APMRadioComponent::requiresSetup(void) const { return true; }
...
bool APMRadioComponent::setupComplete(void) const
{
    ...
    for (int i = 0; i < _mapParams.count(); i++) {
        mapValues << _vehicle->parameterManager()
                         ->getParameter(FactSystem::defaultComponentId, _mapParams[i])
                         ->rawValue()
                         .toInt();
        if (mapValues[i] <= 0) {
            return false;
    ...
    foreach (const QString& mapParam, _mapParams) {
        ...
        if (_vehicle->parameterManager()->getParameter(-1, QStringLiteral("RC%1_MIN").arg(channel))->rawValue().toInt()
            != 1100) {
            return true;
        }
        if (_vehicle->parameterManager()->getParameter(-1, QStringLiteral("RC%1_MAX").arg(channel))->rawValue().toInt()
            != 1900) {
            return true;
        }
        if (_vehicle->parameterManager()->getParameter(-1, QStringLiteral("RC%1_TRIM").arg(channel))->rawValue().toInt()
            != 1500) {
            return true;
        }
    }
...
void APMRadioComponent::_connectSetupTriggers(void)
{
    ...
    foreach (const QString& mapParam, _mapParams) {
        int channel
            = _vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId, mapParam)->rawValue().toInt();
