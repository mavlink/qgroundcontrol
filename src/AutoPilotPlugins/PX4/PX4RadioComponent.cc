...
bool PX4RadioComponent::setupComplete(void) const
{
    if (_autopilot->getParameterFact(-1, QStringLiteral("COM_RC_IN_MODE"))->rawValue().toInt() != 1) {
        ...
        attitudeMappings << QStringLiteral("RC_MAP_ROLL") << QStringLiteral("RC_MAP_PITCH") << QStringLiteral("RC_MAP_YAW") << QStringLiteral("RC_MAP_THROTTLE");
...
QStringList PX4RadioComponent::setupCompleteChangedTriggerList(void) const
{
    ...
    triggers << QStringLiteral("COM_RC_IN_MODE") << QStringLiteral("RC_MAP_ROLL") << QStringLiteral("RC_MAP_PITCH") << QStringLiteral("RC_MAP_YAW") << QStringLiteral("RC_MAP_THROTTLE");
...
{
    return QUrl::fromUserInput(QStringLiteral("qrc:/qml/PX4RadioComponentSummary.qml"));
}
