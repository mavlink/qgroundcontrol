    ...
    , _showCustomConfigPanel(false)
{
    if (!_typesRegistered) {
        ...
        qmlRegisterUncreatableType<AirframeType>(
            "QGroundControl.Controllers", 1, 0, "AiframeType", "Can only reference AirframeType");
        qmlRegisterUncreatableType<Airframe>(
            "QGroundControl.Controllers", 1, 0, "Aiframe", "Can only reference Airframe");
    }

    QStringList usedParams;
    usedParams << "SYS_AUTOSTART"
               << "SYS_AUTOCONFIG";
    ...
    for (int tindex = 0; tindex < AirframeComponentAirframes::get().count(); tindex++) {
        ...
        for (int index = 0; index < pType->rgAirframeInfo.count(); index++) {
            ...
            if (_autostartId == pInfo->autostartId) {
                if (autostartFound) {
                    qWarning() << "AirframeComponentController::AirframeComponentController duplicate ids found:"
                               << _autostartId;
                }
                autostartFound = true;
...
void AirframeComponentController::_rebootAfterStackUnwind(void)
{
    _vehicle->sendMavCommand(
        _vehicle->defaultComponentId(), MAV_CMD_PREFLIGHT_REBOOT_SHUTDOWN, true /* showError */, 1.0f);
