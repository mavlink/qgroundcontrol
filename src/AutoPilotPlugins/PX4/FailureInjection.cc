...
/// looked up by name.
QVariantList _buildCatalog(const char *enumName, const QStringList &prefixesLongestFirst, QVariantMap (*makeEntry)(const QString &, int))
{
    QVariantList list;
...
FailureInjection::FailureInjection(QObject *parent)
    : QObject(parent)
{
    _units = _buildCatalog("FAILURE_UNIT",
        { QStringLiteral("FAILURE_UNIT_SENSOR_"), QStringLiteral("FAILURE_UNIT_SYSTEM_"), QStringLiteral("FAILURE_UNIT_") }, _makeUnit);
    _types = _buildCatalog("FAILURE_TYPE", { QStringLiteral("FAILURE_TYPE_") }, _makeType);
}

void FailureInjection::logRow(const QString &unitName, const QString &typeName, const QString &instanceLabel, const QString &time)
{
    _activity.prepend(QVariantMap{
        {"time", time}, {"unitName", unitName}, {"typeName", typeName}, {"instance", instanceLabel}, {"result", QStringLiteral("pending")}
    });
    emit activityChanged();
}

void FailureInjection::logInjection(const QString &unitName, const QString &typeName, int unitEnum, const QString &instanceLabel, const QString &time)
{
...
{
    // ACKs for MAV_CMD_INJECT_FAILURE arrive in send order; resolve the oldest pending row (highest index, since newest is prepended).
    int pendingIndex = -1;
    ...
    case MAV_RESULT_ACCEPTED:             reason = QStringLiteral("accepted");                          break;
    case MAV_RESULT_TEMPORARILY_REJECTED: reason = tr("Temporarily rejected");                           break;
    case MAV_RESULT_DENIED:               reason = tr("Denied");                                         break;
    case MAV_RESULT_UNSUPPORTED:          reason = tr("Unsupported");                                    break;
    case MAV_RESULT_FAILED:               reason = tr("Failed");                                         break;
    case MAV_RESULT_CANCELLED:            reason = tr("Cancelled");                                      break;
    default:                              reason = QGCMAVLink::mavResultToString(static_cast<uint8_t>(ackResult)); break;
    }
