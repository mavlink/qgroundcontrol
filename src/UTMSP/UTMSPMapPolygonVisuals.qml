...
Item {
    ...
    property var             _StoreCoordinaates  : []
    ...
    function _saveCurrentVertices() {
        ...
        for (var i=0; i<mapPolygon.count; i++) {
            _StoreCoordinaates.push(mapPolygon.vertexCoordinate(i))
        }
    }
    ...
    function _restorePreviousVertices() {
        mapPolygon.beginReset()
        mapPolygon.clear()
        for (var i=0; i<_StoreCoordinaates.length; i++) {
            mapPolygon.appendVertex(_StoreCoordinaates[i])
        }
        mapPolygon.endReset()
    }
