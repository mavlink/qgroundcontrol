...
GeoFenceController::GeoFenceController(PlanMasterController* masterController, QObject* parent)
    ...
    , _breachReturnDefaultAltitude(
          qgcApp()->toolbox()->settingsManager()->appSettings()->defaultMissionItemAltitude()->rawValue().toDouble())
{
    if (_metaDataMap.isEmpty()) {
        _metaDataMap = FactMetaData::createMapFromJsonFile(
            QStringLiteral(":/json/BreachReturn.FactMetaData.json"), nullptr /* metaDataParent */);
    }
...
void GeoFenceController::start(bool flyView)
{
    ...
    connect(_masterController, &PlanMasterController::managerVehicleChanged, this,
        &GeoFenceController::_managerVehicleChanged);
...
void GeoFenceController::_managerVehicleChanged(Vehicle* managerVehicle)
{
    ...
    connect(
        _geoFenceManager, &GeoFenceManager::removeAllComplete, this, &GeoFenceController::_managerRemoveAllComplete);
    ...
    connect(_managerVehicle->parameterManager(), &ParameterManager::parametersReadyChanged, this,
        &GeoFenceController::_parametersReady);
    _parametersReady();
...
bool GeoFenceController::load(const QJsonObject& json, QString& errorString)
{
    ...
    if (json.contains(_jsonBreachReturnKey)) {
        if (!JsonHelper::loadGeoCoordinate(
                json[_jsonBreachReturnKey], true /* altitudeRequred */, _breachReturnPoint, errorString)) {
            return false;
        }
...
void GeoFenceController::_setFenceFromManager(
    const QList<QGCFencePolygon>& polygons, const QList<QGCFenceCircle>& circles)
{
    _polygons.clearAndDeleteContents();
    _circles.clearAndDeleteContents();
...
double GeoFenceController::paramCircularFence(void)
{
    if (_managerVehicle->isOfflineEditingVehicle()
        || !_managerVehicle->parameterManager()->parameterExists(
            FactSystem::defaultComponentId, _px4ParamCircularFence)) {
        return 0;
    }

    return _managerVehicle->parameterManager()
        ->getParameter(FactSystem::defaultComponentId, _px4ParamCircularFence)
        ->rawValue()
        .toDouble();
}
...
void GeoFenceController::_parametersReady(void)
{
    ...
    _px4ParamCircularFenceFact
        = _managerVehicle->parameterManager()->getParameter(FactSystem::defaultComponentId, _px4ParamCircularFence);
