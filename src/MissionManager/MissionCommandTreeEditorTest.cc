...
void MissionCommandTreeEditorTest::_testEditorsWorker(
    QGCMAVLink::FirmwareClass_t firmwareClass, QGCMAVLink::VehicleClass_t vehicleClass)
{
    ...
    if (firmwarePlugin->supportedMissionCommands(vehicleClass).count() == 0) {
        firmwarePlugin = qgcApp()->toolbox()->firmwarePluginManager()->firmwarePluginForAutopilot(
    ...
    qmlAppEngine->rootContext()->setContextProperty("imagePath",
