...
void LandingComplexItemTest::_testDirty(void)
{
    ...
    QList<Fact*> rgFacts;
    rgFacts << _item->finalApproachAltitude() << _item->landingHeading() << _item->loiterRadius()
            << _item->loiterClockwise() << _item->landingAltitude() << _item->landingDistance()
            << _item->useLoiterToAlt() << _item->stopTakingPhotos() << _item->stopTakingVideo();
...
void LandingComplexItemTest::_testItemCount(void)
{
    ...
    for (size_t i = 0; i < sizeof(rgTestCases) / sizeof(rgTestCases[0]); i++) {
        ...
        QCOMPARE(items.count(),
            3 + (testCase.stopTakingPhotos * CameraSection::stopTakingPhotosCommandCount())
...
void LandingComplexItemTest::_testAppendSectionItems(void)
{
    ...
    for (size_t i = 0; i < sizeof(rgTestCases) / sizeof(rgTestCases[0]); i++) {
        ...
        QCOMPARE(CameraSection::scanStopTakingVideo(simpleItems,
                     1 + (testCase.stopTakingPhotos ? CameraSection::stopTakingPhotosCommandCount() : 0),
...
void LandingComplexItemTest::_testScanForItems(void)
{
    ...
    for (size_t i = 0; i < sizeof(rgTestCases) / sizeof(rgTestCases[0]); i++) {
        ...
        QVERIFY(LandingComplexItem::_scanForItem(visualItems, false /* flyView */, _masterController,
            &SimpleLandingComplexItem::_isValidLandItem, &SimpleLandingComplexItem::_createItem));
...
void LandingComplexItemTest::_testSaveLoad(void)
{
    ...
    bool loadSuccess = newItem->_load(saveObject, 0 /* sequenceNumber */,
        SimpleLandingComplexItem::jsonComplexItemTypeValue, false /* useDeprecatedRelAltKeys */, errorString);
    if (!loadSuccess) {
...
void LandingComplexItemTest::_validateItem(LandingComplexItem* actualItem, LandingComplexItem* expectedItem)
{
    ...
    QCOMPARE(
        actualItem->stopTakingPhotos()->rawValue().toBool(), expectedItem->stopTakingPhotos()->rawValue().toBool());
    ...
    if (actualItem->useLoiterToAlt()->rawValue().toBool()) {
        ...
        QCOMPARE(
            actualItem->loiterClockwise()->rawValue().toBool(), expectedItem->loiterClockwise()->rawValue().toBool());
    }
}
...
MissionItem* SimpleLandingComplexItem::_createLandItem(
    int seqNum, bool altRel, double lat, double lon, double alt, QObject* parent)
{
...
void SimpleLandingComplexItem::_updateFlightPathSegmentsDontCallDirectly(void)
{
    ...
    if (useLoiterToAlt()->rawValue().toBool()) {
        _appendFlightPathSegment(FlightPathSegment::SegmentTypeGeneric, finalApproachCoordinate(), amslEntryAlt(),
            loiterTangentCoordinate(), amslEntryAlt()); // Best we can do to simulate loiter circle terrain profile
        _appendFlightPathSegment(FlightPathSegment::SegmentTypeLand, loiterTangentCoordinate(), amslEntryAlt(),
