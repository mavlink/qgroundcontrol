    ...
    , _appSettings(qgcApp()->toolbox()->settingsManager()->appSettings())
{
    ...
    connect(_planViewSettings->takeoffItemNotRequired(), &Fact::rawValueChanged, this,
        &MissionController::_takeoffItemNotRequiredChanged);
...
void MissionController::_resetMissionFlightStatus(void)
{
    ...
    _missionFlightStatus.vehicleSpeed = _controllerVehicle->multiRotor() || _managerVehicle->vtol()
    ...
    _missionFlightStatus.vtolMode
        = _missionContainsVTOLTakeoff ? QGCMAVLink::VehicleClassMultiRotor : QGCMAVLink::VehicleClassFixedWing;

    _controllerVehicle->firmwarePlugin()->batteryConsumptionData(_controllerVehicle, _missionFlightStatus.mAhBattery,
        _missionFlightStatus.hoverAmps, _missionFlightStatus.cruiseAmps);
    if (_missionFlightStatus.mAhBattery != 0) {
        double batteryPercentRemainingAnnounce = qgcApp()
                                                     ->toolbox()
                                                     ->settingsManager()
                                                     ->appSettings()
                                                     ...
                                                     ->rawValue()
                                                     .toDouble();
        _missionFlightStatus.ampMinutesAvailable = static_cast<double>(_missionFlightStatus.mAhBattery) / 1000.0 * 60.0
...
{
    qCDebug(MissionControllerLog) << "_newMissionItemsAvailableFromVehicle flyView:count" << _flyView
    ...
    if (_flyView || removeAllRequested || _itemsRequested || isEmpty()) {
        ...
        _updateContainsItems(); // This will clear containsItems which will be set again below. This will re-pop Start
        ...
        for (; i < newMissionItems.count(); i++) {
            ...
            if (TakeoffMissionItem::isTakeoffCommand(static_cast<MAV_CMD>(simpleItem->command()))) {
                ...
                _takeoffMissionItem = new TakeoffMissionItem(
                    *missionItem, _masterController, _flyView, settingsItem, false /* forLoad */);
    ...
    QmlObjectListModel* visualMissionItems, QList<MissionItem*>& rgMissionItems, QObject* missionItemParent)
{
    ...
    for (int i = 0; i < visualMissionItems->count(); i++) {
        ...
        qCDebug(MissionControllerLog) << "_convertToMissionItems seqNum:lastSeqNum:command"
                                      << visualItem->sequenceNumber() << lastSeqNum << visualItem->commandName();
    }
...
VisualMissionItem* MissionController::_insertSimpleMissionItemWorker(
    QGeoCoordinate coordinate, MAV_CMD command, int visualItemIndex, bool makeCurrentItem)
{
        ...
        if (!qgcApp()->toolbox()->missionCommandTree()->isLandCommand(command)) {
            ...
            if (_findPreviousAltitude(visualItemIndex, &prevAltitude, &prevAltMode)) {
                ...
                if (globalAltitudeMode() == QGroundControlQmlGlobal::AltitudeModeMixed) {
                    // We are in mixed altitude modes, so copy from previous. Otherwise alt mode will be set from global
                    // setting.
    ...
    // We send the click coordinate through here to be able to set the planned home position from the user click
    // location if needed
    ...
    QGeoCoordinate /*coordinate*/, int visualItemIndex, bool makeCurrentItem)
{
    int sequenceNumber = _nextSequenceNumber();
    _takeoffMissionItem
        = new TakeoffMissionItem(_controllerVehicle->vtol() ? MAV_CMD_NAV_VTOL_TAKEOFF : MAV_CMD_NAV_TAKEOFF,
            _masterController, _flyView, _settingsItem, false /* forLoad */);
...
VisualMissionItem* MissionController::insertLandItem(
    QGeoCoordinate coordinate, int visualItemIndex, bool makeCurrentItem)
{
    if (_controllerVehicle->fixedWing()) {
        FixedWingLandingComplexItem* fwLanding = qobject_cast<FixedWingLandingComplexItem*>(
            insertComplexMissionItem(FixedWingLandingComplexItem::name, coordinate, visualItemIndex, makeCurrentItem));
        return fwLanding;
    } else if (_controllerVehicle->vtol()) {
        VTOLLandingComplexItem* vtolLanding = qobject_cast<VTOLLandingComplexItem*>(
            insertComplexMissionItem(VTOLLandingComplexItem::name, coordinate, visualItemIndex, makeCurrentItem));
        return vtolLanding;
    } else {
        return _insertSimpleMissionItemWorker(coordinate,
            _controllerVehicle->vtol() ? MAV_CMD_NAV_VTOL_LAND : MAV_CMD_NAV_RETURN_TO_LAUNCH, visualItemIndex,
            makeCurrentItem);
    }
}
    ...
    QGeoCoordinate coordinate, int visualItemIndex, bool makeCurrentItem)
{
    SimpleMissionItem* simpleItem = qobject_cast<SimpleMissionItem*>(
        _insertSimpleMissionItemWorker(coordinate, MAV_CMD_DO_SET_ROI_LOCATION, visualItemIndex, makeCurrentItem));
...
VisualMissionItem* MissionController::insertComplexMissionItem(
    QString itemName, QGeoCoordinate mapCenterCoordinate, int visualItemIndex, bool makeCurrentItem)
{
...
void MissionController::_insertComplexMissionItemWorker(const QGeoCoordinate& mapCenterCoordinate,
    ComplexMissionItem* complexItem, int visualItemIndex, bool makeCurrentItem)
{
    int sequenceNumber = _nextSequenceNumber();
    bool surveyStyleItem = qobject_cast<SurveyComplexItem*>(complexItem)
        || qobject_cast<CorridorScanComplexItem*>(complexItem) || qobject_cast<StructureScanComplexItem*>(complexItem);
    ...
    if (surveyStyleItem) {
        ...
        // If the vehicle is known to have a gimbal then we automatically point the gimbal straight down if not already
        // set
...
void MissionController::removeVisualItem(int viIndex)
{
    if (viIndex <= 0 || viIndex >= _visualItems->count()) {
        qWarning() << "MissionController::removeVisualItem called with bad index - count:index" << _visualItems->count()
                   << viIndex;
        return;
    }

    bool removeSurveyStyle
        = _visualItems->value<SurveyComplexItem*>(viIndex) || _visualItems->value<CorridorScanComplexItem*>(viIndex);
    ...
    if (removeSurveyStyle) {
        ...
        if (!foundSurvey) {
            ...
            if (_controllerVehicle->firmwarePlugin()->hasGimbal(
                    _controllerVehicle, rollSupported, pitchSupported, yawSupported)
                && pitchSupported) {
                if (cameraSection->specifyGimbal() && cameraSection->gimbalPitch()->rawValue().toDouble() == -90.0
...
bool MissionController::_loadJsonMissionFileV1(
    const QJsonObject& json, QmlObjectListModel* visualItems, QString& errorString)
{
    ...
    do {
        qCDebug(MissionControllerLog)
            << "Json load: simple item loop nextSimpleItemIndex:nextComplexItemIndex:nextSequenceNumber"
        ...
        if (nextComplexItemIndex < surveyItems.count()) {
            ...
            if (complexItem->sequenceNumber() == nextSequenceNumber) {
                qCDebug(MissionControllerLog)
                    << "Json load: injecting complex item expectedSequence:actualSequence:" << nextSequenceNumber
                    << complexItem->sequenceNumber();
            ...
            if (item->load(itemObject, itemObject["id"].toInt(), errorString)) {
                if (TakeoffMissionItem::isTakeoffCommand(item->mavCommand())) {
                    ...
                    TakeoffMissionItem* takeoffItem
                        = new TakeoffMissionItem(_masterController, _flyView, settingsItem, true /* forLoad */);
...
bool MissionController::_loadJsonMissionFileV2(
    const QJsonObject& json, QmlObjectListModel* visualItems, QString& errorString)
{
    // Validate root object keys
    QList<JsonHelper::KeyValidateInfo> rootKeyInfoList = {
    ...
    qCDebug(MissionControllerLog) << "MissionController::_loadJsonMissionFileV2 itemCount:"
    ...
    MAV_TYPE planFileVehicleType = static_cast<MAV_TYPE>(
        QGCMAVLink::vehicleClassToMavType(appSettings->offlineEditingVehicleClass()->rawValue().toInt()));
    ...
    if (_masterController->offline()) {
        appSettings->offlineEditingFirmwareClass()->setRawValue(
            QGCMAVLink::firmwareClass(static_cast<MAV_AUTOPILOT>(json[_jsonFirmwareTypeKey].toInt())));
        if (json.contains(_jsonVehicleTypeKey)) {
    ...
    if (json.contains(_jsonGlobalPlanAltitudeModeKey)) {
        setGlobalAltitudeMode(
            json[_jsonGlobalPlanAltitudeModeKey].toVariant().value<QGroundControlQmlGlobal::AltMode>());
    }

    QGeoCoordinate homeCoordinate;
    if (!JsonHelper::loadGeoCoordinate(
            json[_jsonPlannedHomePositionKey], true /* altitudeRequired */, homeCoordinate, errorString)) {
        return false;
    }
    ...
    for (int i = 0; i < rgMissionItems.count(); i++) {
        ...
        if (itemType == VisualMissionItem::jsonTypeSimpleItemValue) {
            ...
            if (simpleItem->load(itemObject, nextSequenceNumber, errorString)) {
                ...
                qCDebug(MissionControllerLog)
                    << "Loading simple item: nextSequenceNumber:command" << nextSequenceNumber << simpleItem->command();
            ...
            if (complexItemType == SurveyComplexItem::jsonComplexItemTypeValue) {
                ...
                SurveyComplexItem* surveyItem
                    = new SurveyComplexItem(_masterController, _flyView, QString() /* kmlFile */);
                ...
                StructureScanComplexItem* structureItem
                    = new StructureScanComplexItem(_masterController, _flyView, QString() /* kmlFile */);
                ...
                CorridorScanComplexItem* corridorItem
                    = new CorridorScanComplexItem(_masterController, _flyView, QString() /* kmlFile */);
...
bool MissionController::_loadItemsFromJson(
    const QJsonObject& json, QmlObjectListModel* visualItems, QString& errorString)
{
...
bool MissionController::_loadTextMissionFile(QTextStream& stream, QmlObjectListModel* visualItems, QString& errorString)
{
    ...
    if (versionOk) {
        ...
        errorString
            = tr("The mission file is not compatible with this version of %1.").arg(qgcApp()->applicationName());
        return false;
    }
...
void MissionController::_calcPrevWaypointValues(VisualMissionItem* currentItem, VisualMissionItem* prevItem,
...
FlightPathSegment* MissionController::_createFlightPathSegmentWorker(VisualItemPair& pair, bool mavlinkTerrainFrame)
{
    ...
    FlightPathSegment::SegmentType segmentType
        = mavlinkTerrainFrame ? FlightPathSegment::SegmentTypeTerrainFrame : FlightPathSegment::SegmentTypeGeneric;
    ...
    FlightPathSegment* segment = new FlightPathSegment(
        segmentType, coord1, coord1AMSLAlt, coord2, coord2AMSLAlt, !_flyView /* queryTerrainData */, this);
    ...
    connect(pair.second, &VisualMissionItem::coordinateChanged, this,
        &MissionController::_recalcMissionFlightStatusSignal, Qt::QueuedConnection);

    connect(segment, &FlightPathSegment::totalDistanceChanged, this, &MissionController::recalcTerrainProfile,
        Qt::QueuedConnection);
    connect(segment, &FlightPathSegment::coord1AMSLAltChanged, this,
        &MissionController::_recalcMissionFlightStatusSignal, Qt::QueuedConnection);
    connect(segment, &FlightPathSegment::coord2AMSLAltChanged, this,
        &MissionController::_recalcMissionFlightStatusSignal, Qt::QueuedConnection);
    connect(segment, &FlightPathSegment::amslTerrainHeightsChanged, this, &MissionController::recalcTerrainProfile,
        Qt::QueuedConnection);
    connect(segment, &FlightPathSegment::terrainCollisionChanged, this, &MissionController::recalcTerrainProfile,
        Qt::QueuedConnection);
...
FlightPathSegment* MissionController::_addFlightPathSegment(
    FlightPathSegmentHashTable& prevItemPairHashTable, VisualItemPair& pair, bool mavlinkTerrainFrame)
{
    FlightPathSegment* segment = nullptr;

    if (prevItemPairHashTable.contains(pair)
        && (prevItemPairHashTable[pair]->segmentType() == FlightPathSegment::SegmentTypeTerrainFrame)
            != mavlinkTerrainFrame) {
...
void MissionController::_recalcFlightPathSegments(void)
{
    ...
    // Note: Although visual support for _incompleteComplexItemLines is still in the codebase. The support for
    // populating the list is not. This is due to the initial implementation being buggy and incomplete with respect to
    // correctly generating the line set. So for now we leave the code for displaying them in, but none are ever added
    // until we have time to implement the correct support.
    ...
    // We need to clear the simple flight path segments on all items since they are going to be rebuilt. We can't just
    // do this in the main loop below since that loop won't always process all items.
    for (int i = 1; i < _visualItems->count(); i++) {
    ...
    for (int i = 1; i < _visualItems->count(); i++) {
        ...
        if (simpleItem) {
            ...
            case MAV_CMD_NAV_VTOL_TAKEOFF:
                ...
                if (!linkEndToHome) {
                    // If we still haven't found the first coordinate item and we hit a takeoff command this means the
                    // mission starts from the ground. Link the first item back to home to show that.
        ...
        if (visualItem->specifiesCoordinate() && !visualItem->isStandaloneCoordinate()) {
            // Incomplete items are complex items which are waiting for the user to complete setup before there visuals
            // can become valid. They may not yet have valid entry/exit coordinates associated with them while in the
            // incomplete state. For examples a Survey item which has no polygon set yet.
            if (complexItem && complexItem->isIncomplete()) {
                ...
                if (lastFlyThroughVI != _settingsItem || (homePositionValid && linkStartToHome)) {
                    ...
                    if (i != 1) {
                        ...
                        // The reason for start with second segment is to prevent an arrow being added in between the
                        // home position and a takeoff item which may be right over each other. In that case the arrow
                    ...
                    if (!_flyView || addDirectionArrow) {
                        ...
                        bool mavlinkTerrainFrame
                            = simpleItem ? simpleItem->missionItem().frame() == MAV_FRAME_GLOBAL_TERRAIN_ALT : false;
                        FlightPathSegment* segment
                            = _addFlightPathSegment(oldSegmentTable, lastSegmentVisualItemPair, mavlinkTerrainFrame);
    ...
    if (linkEndToHome && lastFlyThroughVI != _settingsItem && homePositionValid) {
        ...
        FlightPathSegment* segment
            = _addFlightPathSegment(oldSegmentTable, lastSegmentVisualItemPair, false /* mavlinkTerrainFrame */);
        segment->setSpecialVisual(roiActive);
        lastFlyThroughVI->setSimpleFlighPathSegment(segment);
    }
    ...
    if (lastSegmentVisualItemPair.first) {
        ...
        // The pair may not be in the hash, this can happen in the fly view where only segments with arrows on them are
        // added to hash. check for that first and add if needed
        ...
        if (_flightPathSegmentHashTable.contains(lastSegmentVisualItemPair)) {
            ...
            // Create a new segment. Since this is the fly view there is no need to wire change signals or worry about
            // correct SegmentType
            coordVector = new FlightPathSegment(FlightPathSegment::SegmentTypeGeneric,
                lastSegmentVisualItemPair.first->isSimpleItem() ? lastSegmentVisualItemPair.first->coordinate()
                                                                : lastSegmentVisualItemPair.first->exitCoordinate(),
                lastSegmentVisualItemPair.first->isSimpleItem() ? lastSegmentVisualItemPair.first->amslEntryAlt()
                                                                : lastSegmentVisualItemPair.first->amslExitAlt(),
                lastSegmentVisualItemPair.second->coordinate(), lastSegmentVisualItemPair.second->amslEntryAlt(),
...
void MissionController::_updateBatteryInfo(int waypointIndex)
{
    if (_missionFlightStatus.mAhBattery != 0) {
        ...
        _missionFlightStatus.cruiseAmpsTotal
            = (_missionFlightStatus.cruiseTime / 60.0) * _missionFlightStatus.cruiseAmps;
        _missionFlightStatus.batteriesRequired
            = ceil((_missionFlightStatus.hoverAmpsTotal + _missionFlightStatus.cruiseAmpsTotal)
                / _missionFlightStatus.ampMinutesAvailable);
        // FIXME: Battery change point code pretty much doesn't work. The reason is that is treats complex items as a
        // black box. It needs to be able to look inside complex items in order to determine a swap point that is
        // interior to a complex item. Current the swap point display in PlanToolbar is disabled to do this problem.
...
void MissionController::_addTimeDistance(
    bool vtolInHover, double hoverTime, double cruiseTime, double extraTime, double distance, int seqNum)
{
...
void MissionController::_recalcMissionFlightStatus()
{
    ...
    for (int i = 0; i < _visualItems->count(); i++) {
        ...
        if (i != 0 && !foundRTL) {
            // We must set the mission flight status prior to querying for any values from the item. This is because
            // things like current speed, gimbal, vtol state  impact the values.
            ...
            if (firstCoordinateItem && simpleItem
                ...
                if (homePositionValid) {
                    ...
                    if (_controllerVehicle->multiRotor() || _controllerVehicle->vtol()) {
                        ...
                        double takeoffTime
                            = qAbs(altDifference) / _appSettings->offlineEditingAscentSpeed()->rawValue().toDouble();
                        _addHoverTime(takeoffTime, 0, -1);
                    }
                }
            }

            _addTimeDistance(_missionFlightStatus.vtolMode == QGCMAVLink::VehicleClassMultiRotor, 0, 0,
                item->additionalTimeDelay(), 0, -1);

            if (item->specifiesCoordinate()) {

                // Keep track of the min/max AMSL altitude for entire mission so we can calculate altitude percentages
                // in terrain status display
                if (simpleItem) {
                ...
                if (!item->isStandaloneCoordinate()) {
                    ...
                    if (simpleItem) {
                        ...
                        if (qIsNaN(newVehicleYaw)) {
                            // No specific vehicle yaw set. Current vehicle yaw is determined from flight path segment
                            // direction.
                            if (simpleItem != lastFlyThroughVI) {
                                _missionFlightStatus.vehicleYaw
                                    = lastFlyThroughVI->exitCoordinate().azimuthTo(simpleItem->coordinate());
                            }
                        } else {
                    ...
                    if (lastFlyThroughVI != _settingsItem || linkStartToHome) {
                        ...
                        _missionFlightStatus.maxTelemetryDistance
                            = qMax(_missionFlightStatus.maxTelemetryDistance, _calcDistanceToHome(item, _settingsItem));

                        // Calculate time/distance
                        double hoverTime = distance / _missionFlightStatus.hoverSpeed;
                        double cruiseTime = distance / _missionFlightStatus.cruiseSpeed;
                        _addTimeDistance(_missionFlightStatus.vtolMode == QGCMAVLink::VehicleClassMultiRotor, hoverTime,
                            cruiseTime, 0, distance, item->sequenceNumber());
                    }

                    if (complexItem) {
                        // Add in distance/time inside complex items as well
                        double distance = complexItem->complexDistance();
                        _missionFlightStatus.maxTelemetryDistance = qMax(_missionFlightStatus.maxTelemetryDistance,
    ...
    if (foundRTL && lastFlyThroughVI != _settingsItem && homePositionValid) {
        ...
        _addTimeDistance(_missionFlightStatus.vtolMode == QGCMAVLink::VehicleClassMultiRotor, hoverTime, cruiseTime,
            distance, landTime, -1);
    }
    ...
    if (linkStartToHome) {
        ...
        _minAMSLAltitude
            = std::fmin(_minAMSLAltitude, _settingsItem->plannedHomePositionAltitude()->rawValue().toDouble());
        _maxAMSLAltitude
            = std::fmax(_maxAMSLAltitude, _settingsItem->plannedHomePositionAltitude()->rawValue().toDouble());
    }
...
void MissionController::_initAllVisualItems(void)
{
    ...
    connect(
        _settingsItem, &MissionSettingsItem::coordinateChanged, this, &MissionController::plannedHomePositionChanged);
...
void MissionController::_initVisualItem(VisualMissionItem* visualItem)
{
    ...
    if (visualItem->isSimpleItem()) {
        ...
        SimpleMissionItem* simpleItem = qobject_cast<SimpleMissionItem*>(visualItem);
        if (simpleItem) {
            connect(&simpleItem->missionItem()._commandFact, &Fact::valueChanged, this,
...
void MissionController::_managerVehicleChanged(Vehicle* managerVehicle)
{
    ...
    connect(
        _missionManager, &MissionManager::currentIndexChanged, this, &MissionController::_currentMissionIndexChanged);
    connect(
        _missionManager, &MissionManager::lastCurrentIndexChanged, this, &MissionController::resumeMissionIndexChanged);
    connect(_missionManager, &MissionManager::resumeMissionReady, this, &MissionController::resumeMissionReady);
    connect(
        _missionManager, &MissionManager::resumeMissionUploadFail, this, &MissionController::resumeMissionUploadFail);
...
void MissionController::_inProgressChanged(bool inProgress) { emit syncInProgressChanged(inProgress); }

bool MissionController::_findPreviousAltitude(
    int newIndex, double* prevAltitude, QGroundControlQmlGlobal::AltMode* prevAltitudeMode)
{
    bool found = false;
    ...
    if (visualItems->count() > 1) {
        ...
        if (firstCoordSet) {
            _settingsItem->setInitialHomePositionFromUser(
                QGeoCoordinate((south + ((north - south) / 2)) - 90.0, (west + ((east - west) / 2)) - 180.0, 0.0));
        }
    }
}
...
int MissionController::resumeMissionIndex(void) const
{
    ...
    if (_flyView) {
        resumeIndex = _missionManager->lastCurrentIndex()
            + (_controllerVehicle->firmwarePlugin()->sendHomePositionToVehicle() ? 0 : 1);
        if (resumeIndex > 1
            && resumeIndex != _visualItems->value<VisualMissionItem*>(_visualItems->count() - 1)->sequenceNumber()) {
...
bool MissionController::syncInProgress(void) const { return _missionManager->inProgress(); }

bool MissionController::dirty(void) const { return _visualItems ? _visualItems->dirty() : false; }
    ...
    QmlObjectListModel* visualItems, PlanMasterController* masterController)
{
    ...
    while (scanIndex < visualItems->count()) {
        ...
        qCDebug(MissionControllerLog) << "MissionController::_scanForAdditionalSettings count:scanIndex"
...
void MissionController::_updateContainsItems(void) { emit containsItemsChanged(containsItems()); }

bool MissionController::containsItems(void) const { return _visualItems ? _visualItems->count() > 1 : false; }
...
bool MissionController::showPlanFromManagerVehicle(void)
{
    ...
    if (_masterController->offline()) {
        ...
        if (!_managerVehicle->initialPlanRequestComplete()) {
            // The vehicle hasn't completed initial load, we can just wait for newMissionItemsAvailable to be signalled
            // automatically
            qCDebug(MissionControllerLog) << "showPlanFromManagerVehicle: !initialPlanRequestComplete, wait for signal";
            return true;
        } else if (syncInProgress()) {
            // If the sync is already in progress, newMissionItemsAvailable will be signalled automatically when it is
            // done. So no need to do anything.
...
void MissionController::setCurrentPlanViewSeqNum(int sequenceNumber, bool force)
{
    if (_visualItems && (force || sequenceNumber != _currentPlanViewSeqNum)) {
        ...
        _onlyInsertTakeoffValid = !_planViewSettings->takeoffItemNotRequired()->rawValue().toBool()
        ...
        for (int viIndex = 0; viIndex < _visualItems->count(); viIndex++) {
            ...
            if (simpleItem) {
                ...
                if (currentSeqNumber < sequenceNumber && simpleItem->specifiesCoordinate()
                        ...
                        for (int j = viIndex - 1; j > 0; j--) {
                            ...
                            if (pPrev->specifiesCoordinate() && !pPrev->isStandaloneCoordinate()) {
                                ...
                                if (_flightPathSegmentHashTable.contains(splitPair)) {
                                    ...
                                    // The recalc of flight path segments hasn't happened yet since it is delayed and
                                    // compressed. So we need to register the fact that we need a split segment update
                                    // and it will happen in the recalc instead.
...
void MissionController::_updateTimeout()
{
                ...
                case MAV_CMD_NAV_LAND:
                    if (pSimpleItem->coordinate().isValid()) {
                        ...
                        if (!pSimpleItem->altitude()->rawValue().isNull()
                            && !qIsNaN(pSimpleItem->altitude()->rawValue().toDouble())) {
    ...
    boundingCube = QGCGeoBoundingCube(
        QGeoCoordinate(north - 90.0, west - 180.0, minAlt), QGeoCoordinate(south - 90.0, east - 180.0, maxAlt));
...
void MissionController::_complexBoundingBoxChanged() { _updateTimer.start(UPDATE_TIMEOUT); }

bool MissionController::isEmpty(void) const { return _visualItems->count() <= 1; }
...
QString MissionController::surveyComplexItemName(void) const { return SurveyComplexItem::name; }

QString MissionController::corridorScanComplexItemName(void) const { return CorridorScanComplexItem::name; }

QString MissionController::structureScanComplexItemName(void) const { return StructureScanComplexItem::name; }
...
{
    // As soon as the first item is added we lock the firmware/vehicle type to current values. So if you then connect a
    // vehicle it will not affect these values.
    _controllerVehicle->stopTrackingFirmwareVehicleTypeChanges();
}
...
QGroundControlQmlGlobal::AltMode MissionController::globalAltitudeMode(void) { return _globalAltMode; }
