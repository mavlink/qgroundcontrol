    ...
    , _flightSpeedFact(0, _flightSpeedName, FactMetaData::valueTypeDouble)
{
    if (_metaDataMap.isEmpty()) {
        _metaDataMap = FactMetaData::createMapFromJsonFile(
            QStringLiteral(":/json/SpeedSection.FactMetaData.json"), nullptr /* metaDataParent */);
    }
...
bool SpeedSection::settingsSpecified(void) const { return _specifyFlightSpeed; }

void SpeedSection::setAvailable(bool available)
{
    if (available != _available) {
        if (available
            && (_masterController->controllerVehicle()->multiRotor()
...
int SpeedSection::itemCount(void) const { return _specifyFlightSpeed ? 1 : 0; }
...
void SpeedSection::appendSectionItems(QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum)
{
    ...
    if (_specifyFlightSpeed) {
        MissionItem* item = new MissionItem(seqNum++, MAV_CMD_DO_CHANGE_SPEED, MAV_FRAME_MISSION,
            _masterController->controllerVehicle()->multiRotor() ? 1 /* groundspeed */ : 0 /* airspeed */
...
bool SpeedSection::scanForSection(QmlObjectListModel* visualItems, int scanIndex)
{
    ...
    if (missionItem.command() == MAV_CMD_DO_CHANGE_SPEED && missionItem.param3() == -1 && missionItem.param4() == 0
        && missionItem.param5() == 0 && missionItem.param6() == 0 && missionItem.param7() == 0) {
