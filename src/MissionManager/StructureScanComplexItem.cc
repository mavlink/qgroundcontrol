...
StructureScanComplexItem::StructureScanComplexItem(
    PlanMasterController* masterController, bool flyView, const QString& kmlOrShpFile)
    : ComplexMissionItem(masterController, flyView)
    , _metaDataMap(FactMetaData::createMapFromJsonFile(
          QStringLiteral(":/json/StructureScan.SettingsGroup.json"), this /* QObject parent */))
    , _sequenceNumber(0)
    , _entryVertex(0)
    , _ignoreRecalc(false)
    , _scanDistance(0.0)
    ...
    , _entranceAltFact(settingsGroup, _metaDataMap[_entranceAltName])
{
    ...
    _entranceAltFact.setRawValue(
        qgcApp()->toolbox()->settingsManager()->appSettings()->defaultMissionItemAltitude()->rawValue());
    ...
    connect(
        _cameraCalc.adjustedFootprintFrontal(), &Fact::valueChanged, this, &StructureScanComplexItem::_recalcLayerInfo);
    ...
    connect(
        &_structurePolygon, &QGCMapPolygon::isValidChanged, this, &StructureScanComplexItem::readyForSaveStateChanged);
    ...
    connect(
        &_structurePolygon, &QGCMapPolygon::countChanged, this, &StructureScanComplexItem::_updateLastSequenceNumber);
    ...
    connect(
        _cameraCalc.distanceToSurface(), &Fact::valueChanged, this, &StructureScanComplexItem::_rebuildFlightPolygon);

    connect(&_flightPolygon, &QGCMapPolygon::pathChanged, this, &StructureScanComplexItem::_recalcCameraShots);
    connect(
        _cameraCalc.adjustedFootprintSide(), &Fact::valueChanged, this, &StructureScanComplexItem::_recalcCameraShots);
    ...
    connect(
        this, &StructureScanComplexItem::wizardModeChanged, this, &StructureScanComplexItem::readyForSaveStateChanged);
    ...
    connect(
        this, &StructureScanComplexItem::topFlightAltChanged, this, &StructureScanComplexItem::minAMSLAltitudeChanged);
    connect(
        this, &StructureScanComplexItem::topFlightAltChanged, this, &StructureScanComplexItem::maxAMSLAltitudeChanged);
    connect(this, &StructureScanComplexItem::bottomFlightAltChanged, this,
        &StructureScanComplexItem::minAMSLAltitudeChanged);
    connect(this, &StructureScanComplexItem::bottomFlightAltChanged, this,
        &StructureScanComplexItem::maxAMSLAltitudeChanged);
    ...
    connect(
        &_flightPolygon, &QGCMapPolygon::pathChanged, this, &StructureScanComplexItem::_updateFlightPathSegmentsSignal);
    connect(&_startFromTopFact, &Fact::valueChanged, this, &StructureScanComplexItem::_updateFlightPathSegmentsSignal);
    connect(
        &_structureHeightFact, &Fact::valueChanged, this, &StructureScanComplexItem::_updateFlightPathSegmentsSignal);
    ...
    connect(this, &StructureScanComplexItem::_updateFlightPathSegmentsSignal, this,
        &StructureScanComplexItem::_updateFlightPathSegmentsDontCallDirectly, Qt::QueuedConnection);
...
void StructureScanComplexItem::_updateLastSequenceNumber(void) { emit lastSequenceNumberChanged(lastSequenceNumber()); }
...
bool StructureScanComplexItem::load(const QJsonObject& complexObject, int sequenceNumber, QString& errorString)
{
    ...
    if (!_cameraCalc.load(complexObject[_jsonCameraCalcKey].toObject(), false /* v1FollowTerrain */, errorString,
            false /* forPresets */)) {
        return false;
    }
...
void StructureScanComplexItem::appendMissionItems(QList<MissionItem*>& items, QObject* missionItemParent)
{
    ...
    item = new MissionItem(seqNum++, MAV_CMD_NAV_WAYPOINT, MAV_FRAME_GLOBAL_RELATIVE_ALT,
        0, // No hold time
        0.0, // No acceptance radius specified
        0.0, // Pass through waypoint
        std::numeric_limits<double>::quiet_NaN(), // Yaw unchanged
        entranceExitCoord.latitude(), entranceExitCoord.longitude(), _entranceAltFact.rawValue().toDouble(),
        true, // autoContinue
        false, // isCurrentItem
        missionItemParent);
    items.append(item);
    ...
    item = new MissionItem(seqNum++, MAV_CMD_DO_SET_ROI_WPNEXT_OFFSET, MAV_FRAME_MISSION, 0, 0, 0,
        0, // param 1-4 not used
    ...
    for (int layer = 0; layer < _layersFact.rawValue().toInt(); layer++) {
        ...
        do {
            ...
            item = new MissionItem(seqNum++, MAV_CMD_NAV_WAYPOINT, MAV_FRAME_GLOBAL_RELATIVE_ALT,
                ...
                0.0, // Pass through waypoint
                std::numeric_limits<double>::quiet_NaN(), // Yaw unchanged
                vertexCoord.latitude(), vertexCoord.longitude(), layerAltitude,
                true, // autoContinue
                false, // isCurrentItem
                missionItemParent);
            items.append(item);
    ...
    item = new MissionItem(seqNum++, MAV_CMD_DO_SET_ROI_NONE, MAV_FRAME_MISSION, 0, 0, 0, 0, 0, 0,
        0, // param 1-7 not used
        true, // autoContinue
        false, // isCurrentItem
        missionItemParent);
    items.append(item);
...
int StructureScanComplexItem::cameraShots(void) const { return _cameraShots; }
...
void StructureScanComplexItem::_setDirty(void) { setDirty(true); }

void StructureScanComplexItem::applyNewAltitude(double newAltitude) { _entranceAltFact.setRawValue(newAltitude); }
...
void StructureScanComplexItem::_recalcLayerInfo(void)
{
    double surfaceHeight
        = qMax(_structureHeightFact.rawValue().toDouble() - _scanBottomAltFact.rawValue().toDouble(), 0.0);

    // Layer count is calculated from surface and layer heights
    _layersFact.setRawValue(
        qMax(qCeil(surfaceHeight / _cameraCalc.adjustedFootprintFrontal()->rawValue().toDouble()), 1));
}
...
double StructureScanComplexItem::bottomFlightAlt(void) const
{
    if (_startFromTopFact.rawValue().toBool()) {
        ...
        double layerIncrement = (_cameraCalc.adjustedFootprintFrontal()->rawValue().toDouble() / 2.0)
            + ((_layersFact.rawValue().toInt() - 1) * _cameraCalc.adjustedFootprintFrontal()->rawValue().toDouble());
...
void StructureScanComplexItem::_recalcScanDistance()
{
    ...
    if (_flightPolygon.count() > 2) {
        ...
        qCDebug(StructureScanComplexItemLog)
            << "StructureScanComplexItem--_recalcScanDistance layers: " << _layersFact.rawValue().toInt()
            << " structure height: " << surfaceHeight << " scanDistance: " << _scanDistance;
    }
...
void StructureScanComplexItem::_updateFlightPathSegmentsDontCallDirectly(void)
{
    ...
    if (_flightPolygon.count() > 2) {
        ...
        double entranceAlt
            = _entranceAltFact.rawValue().toDouble() + _missionController->plannedHomePosition().altitude();
        _appendFlightPathSegment(
            FlightPathSegment::SegmentTypeGeneric, layerEntranceCoord, entranceAlt, layerEntranceCoord, layerAltitude);
        ...
        for (int layerIndex = 0; layerIndex < _layersFact.rawValue().toInt(); layerIndex++) {
            ...
            if (layerIndex != 0) {
                _appendFlightPathSegment(FlightPathSegment::SegmentTypeGeneric, layerEntranceCoord, prevLayerAltitude,
                    layerEntranceCoord, layerAltitude);
            }
            ...
            for (const QGeoCoordinate& coord : _flightPolygon.coordinateList()) {
                if (prevCoord.isValid()) {
                    _appendFlightPathSegment(
                        FlightPathSegment::SegmentTypeGeneric, prevCoord, layerAltitude, coord, layerAltitude);
                }
                prevCoord = coord;
            }
            _appendFlightPathSegment(FlightPathSegment::SegmentTypeGeneric, _flightPolygon.coordinateList().last(),
                layerAltitude, _flightPolygon.coordinateList().first(), layerAltitude);
        ...
        _appendFlightPathSegment(FlightPathSegment::SegmentTypeGeneric, layerEntranceCoord, prevLayerAltitude,
            layerEntranceCoord, entranceAlt);
    }
