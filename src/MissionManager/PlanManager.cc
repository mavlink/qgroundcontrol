...
void PlanManager::_writeMissionItemsWorker(void)
{
    ...
    qCDebug(PlanManagerLog) << QStringLiteral("writeMissionItems %1 count:").arg(_planTypeString())
...
void PlanManager::writeMissionItems(const QList<MissionItem*>& missionItems)
{
    ...
    if (inProgress()) {
        qCDebug(PlanManagerLog)
            << QStringLiteral("writeMissionItems %1 called while transaction in progress").arg(_planTypeString());
        return;
    }
    ...
    bool skipFirstItem
        = _planType == MAV_MISSION_TYPE_MISSION && !_vehicle->firmwarePlugin()->sendHomePositionToVehicle();
...
{
    qCDebug(PlanManagerLog) << QStringLiteral("_writeMissionCount %1 count:_retryCount").arg(_planTypeString())
                            << _writeMissionItems.count() << _retryCount;
    ...
    if (!weakLink.expired()) {
        ...
        mavlink_msg_mission_count_pack_chan(qgcApp()->toolbox()->mavlinkProtocol()->getSystemId(),
            qgcApp()->toolbox()->mavlinkProtocol()->getComponentId(), sharedLink->mavlinkChannel(), &message,
            _vehicle->id(), MAV_COMP_ID_AUTOPILOT1, _writeMissionItems.count(), _planType);
...
void PlanManager::loadFromVehicle(void)
{
    ...
    if (inProgress()) {
        qCDebug(PlanManagerLog)
            << QStringLiteral("loadFromVehicle %1 called while transaction in progress").arg(_planTypeString());
        return;
    }
...
{
    qCDebug(PlanManagerLog) << QStringLiteral("_requestList %1 _planType:_retryCount").arg(_planTypeString())
                            << _planType << _retryCount;
    ...
    if (!weakLink.expired()) {
        ...
        mavlink_msg_mission_request_list_pack_chan(qgcApp()->toolbox()->mavlinkProtocol()->getSystemId(),
            qgcApp()->toolbox()->mavlinkProtocol()->getComponentId(), sharedLink->mavlinkChannel(), &message,
            _vehicle->id(), MAV_COMP_ID_AUTOPILOT1, _planType);
...
void PlanManager::_ackTimeout(void)
{
    ...
    case AckNone:
        ...
        _sendError(InternalError,
            tr("Internal error occurred during Mission Item communication: _ackTimeOut:_expectedAck == AckNone"));
        break;
    ...
    case AckMissionItem:
        ...
        if (_retryCount > _maxRetryCount) {
            ...
            _retryCount++;
            qCDebug(PlanManagerLog) << tr("Retrying %1 MISSION_REQUEST retry Count").arg(_planTypeString())
                                    << _retryCount;
    ...
    case AckMissionRequest:
        ...
        if (_itemIndicesToWrite.count() == 0) {
            ...
            if (_retryCount > _maxRetryCount) {
                ...
                _retryCount++;
                qCDebug(PlanManagerLog)
                    << QStringLiteral("Retrying %1 MISSION_COUNT retry Count").arg(_planTypeString()) << _retryCount;
                _writeMissionCount();
            }
        } else {
            // Vehicle did not request all items from ground station
            _sendError(ProtocolError,
                tr("Vehicle did not request all items from ground station: %1").arg(_ackTypeToString(_expectedAck)));
            _expectedAck = AckNone;
            _finishTransaction(false);
        }
        break;
    case AckMissionClearAll:
        ...
        if (_retryCount > _maxRetryCount) {
            ...
            _retryCount++;
            qCDebug(PlanManagerLog) << tr("Retrying %1 MISSION_CLEAR_ALL retry Count").arg(_planTypeString())
                                    << _retryCount;
            _removeAllWorker();
        }
        break;
    case AckGuidedItem:
        // MISSION_REQUEST is expected, or MISSION_ACK to end sequence
    default:
        _sendError(AckTimeoutError,
            tr("Vehicle did not respond to mission item communication: %1").arg(_ackTypeToString(_expectedAck)));
        _expectedAck = AckNone;
        _finishTransaction(false);
    }
}
...
bool PlanManager::_checkForExpectedAck(AckType_t receivedAck)
{
    if (receivedAck == _expectedAck) {
        ...
        if (_expectedAck == AckNone) {
            ...
            // We just warn in this case, this could be crap left over from a previous transaction or the vehicle going
            // bonkers. Whatever it is we let the ack timeout handle any error output to the user.
            qCDebug(PlanManagerLog)
                << QString("Out of sequence ack %1 expected:received %2:%3")
                       .arg(_planTypeString().arg(_ackTypeToString(_expectedAck)).arg(_ackTypeToString(receivedAck)));
        }
        return false;
    }
}
...
void PlanManager::_readTransactionComplete(void)
{
    ...
    if (!weakLink.expired()) {
        ...
        mavlink_msg_mission_ack_pack_chan(qgcApp()->toolbox()->mavlinkProtocol()->getSystemId(),
            qgcApp()->toolbox()->mavlinkProtocol()->getComponentId(), sharedLink->mavlinkChannel(), &message,
            _vehicle->id(), MAV_COMP_ID_AUTOPILOT1, MAV_MISSION_ACCEPTED, _planType);
...
void PlanManager::_handleMissionCount(const mavlink_message_t& message)
{
    ...
    if (missionCount.mission_type != _planType) {
        ...
        qCDebug(PlanManagerLog) << QStringLiteral(
            "_handleMissionCount %1 Incorrect mission_type received expected:actual")
                                       .arg(_planTypeString())
    ...
    qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionCount %1 count:").arg(_planTypeString())
                            << missionCount.count;
...
void PlanManager::_requestNextMissionItem(void)
{
    if (_itemIndicesToRead.count() == 0) {
        _sendError(
            InternalError, tr("Internal Error: Call to Vehicle _requestNextMissionItem with no more indices to read"));
        return;
    }

    qCDebug(PlanManagerLog) << QStringLiteral("_requestNextMissionItem %1 sequenceNumber:retry").arg(_planTypeString())
                            << _itemIndicesToRead[0] << _retryCount;
    ...
    if (!weakLink.expired()) {
        ...
        mavlink_msg_mission_request_int_pack_chan(qgcApp()->toolbox()->mavlinkProtocol()->getSystemId(),
            qgcApp()->toolbox()->mavlinkProtocol()->getComponentId(), sharedLink->mavlinkChannel(), &message,
            _vehicle->id(), MAV_COMP_ID_AUTOPILOT1, _itemIndicesToRead[0], _planType);
        _vehicle->sendMessageOnLinkThreadSafe(sharedLink.get(), message);
    }
...
void PlanManager::_handleMissionItem(const mavlink_message_t& message)
{
    ...
    if (missionType != _planType) {
        qCDebug(PlanManagerLog) << QStringLiteral(
            "_handleMissionItem %1 dropping spurious item seq:command:missionType")
                                       .arg(_planTypeString())
                                << seq << command << missionType;
        return;
    }
    ...
    if (!_checkForExpectedAck(AckMissionItem)) {
        if (_vehicle->apmFirmware() && seq == 0 && _planType == MAV_MISSION_TYPE_MISSION) {
            ...
            qCDebug(PlanManagerLog) << QStringLiteral(
                "_handleMissionItem %1 dropping spurious item seq:command:current")
                                           .arg(_planTypeString())
    ...
    qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionItem %1 seq:command:current:ardupilotHomePositionUpdate")
                                   .arg(_planTypeString())
    ...
    if (_itemIndicesToRead.contains(seq)) {
        ...
        MissionItem* item = new MissionItem(seq, command, frame, param1, param2, param3, param4, param5, param6, param7,
            autoContinue, isCurrentItem, this);
        ...
        qCDebug(PlanManagerLog) << QStringLiteral(
            "_handleMissionItem %1 mission item received item index which was not requested, disregrarding:")
                                       .arg(_planTypeString())
                                << seq;
...
void PlanManager::_handleMissionRequest(const mavlink_message_t& message)
{
    ...
    if (missionRequestMissionType != _planType) {
        ...
        // a stale message here, for example when the MAV ran into a timeout and sent a message twice
        qCDebug(PlanManagerLog) << QStringLiteral(
            "_handleMissionRequest %1 Incorrect mission_type received expected:actual")
                                       .arg(_planTypeString())
    ...
    qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionRequest %1 sequenceNumber").arg(_planTypeString())
                            << missionRequestSeq;

    if (missionRequestSeq > _writeMissionItems.count() - 1) {
        _sendError(RequestRangeError,
            tr("Vehicle requested item outside range, count:request %1:%2. Send to Vehicle failed.")
                .arg(_writeMissionItems.count())
                .arg(missionRequestSeq));
        _finishTransaction(false);
        return;
    }
    ...
    if (!_itemIndicesToWrite.contains(missionRequestSeq)) {
        qCDebug(PlanManagerLog) << QStringLiteral(
            "_handleMissionRequest %1 sequence number requested which has already been sent, sending again:")
                                       .arg(_planTypeString())
                                << missionRequestSeq;
    } else {
    ...
    qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionRequest %1 sequenceNumber:command").arg(_planTypeString())
                            << missionRequestSeq << item->command();
    ...
    if (!weakLink.expired()) {
        ...
        mavlink_msg_mission_item_int_pack_chan(qgcApp()->toolbox()->mavlinkProtocol()->getSystemId(),
            ...
            _vehicle->id(), MAV_COMP_ID_AUTOPILOT1, missionRequestSeq, item->frame(), item->command(),
            missionRequestSeq == 0, item->autoContinue(), item->param1(), item->param2(), item->param3(),
            item->param4(), item->frame() == MAV_FRAME_MISSION ? item->param5() : item->param5() * 1e7,
            item->frame() == MAV_FRAME_MISSION ? item->param6() : item->param6() * 1e7, item->param7(), _planType);
...
void PlanManager::_handleMissionAck(const mavlink_message_t& message)
{
    ...
    if (missionAck.mission_type != _planType) {
        ...
        // a stale message here, for example when the MAV ran into a timeout and sent a message twice
        qCDebug(PlanManagerLog) << QStringLiteral(
            "_handleMissionAck %1 Incorrect mission_type received expected:actual")
                                       .arg(_planTypeString())
    ...
    if (_vehicle->apmFirmware() && missionAck.type == MAV_MISSION_INVALID_SEQUENCE) {
        // ArduPilot sends these Acks which can happen just due to noisy links causing duplicated requests being
        // responded to. As far as I'm concerned this is incorrect protocol implementation but we need to deal with it
        // anyway. So we just ignore it and if things really go haywire the timeouts will fire to fail the overall
        // transaction.
        qCDebug(PlanManagerLog) << QStringLiteral(
    ...
    qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionAck %1 type:").arg(_planTypeString())
    ...
    case AckNone:
        ...
        qCDebug(PlanManagerLog) << QStringLiteral("Vehicle sent unexpected MISSION_ACK message, error: %1")
                                       .arg(_missionResultToString((MAV_MISSION_RESULT)missionAck.type));
        break;
    case AckMissionCount:
        // MISSION_COUNT message expected
    ...
    case AckMissionRequest:
        ...
        if (missionAck.type == MAV_MISSION_ACCEPTED) {
            if (_itemIndicesToWrite.count() == 0) {
                qCDebug(PlanManagerLog)
                    << QStringLiteral("_handleMissionAck write sequence complete %1").arg(_planTypeString());
                _finishTransaction(true);
            } else {
                // FIXME: Protocol error
                _sendError(VehicleAckError, _missionResultToString((MAV_MISSION_RESULT)missionAck.type));
    ...
    case AckMissionClearAll:
        ...
        if (missionAck.type != MAV_MISSION_ACCEPTED) {
            _sendError(VehicleAckError,
                tr("Vehicle remove all failed. Error: %1")
                    .arg(_missionResultToString((MAV_MISSION_RESULT)missionAck.type)));
        }
    ...
    case AckGuidedItem:
        ...
        if (missionAck.type == MAV_MISSION_ACCEPTED) {
            qCDebug(PlanManagerLog)
                << QStringLiteral("_handleMissionAck %1 guided mode item accepted").arg(_planTypeString());
            _finishTransaction(true, true /* apmGuidedItemWrite */);
        } else {
            // FIXME: Protocol error
            _sendError(VehicleAckError,
                tr("Vehicle returned error: %1. %2Vehicle did not accept guided item.")
                    .arg(_missionResultToString((MAV_MISSION_RESULT)missionAck.type)));
...
{
    qCDebug(PlanManagerLog) << QStringLiteral("Sending error - _planTypeString(%1) errorCode(%2) errorMsg(%4)")
                                   .arg(_planTypeString())
                                   .arg(errorCode)
                                   .arg(errorMsg);
...
QString PlanManager::_lastMissionReqestString(MAV_MISSION_RESULT result)
{
    ...
    if (_lastMissionRequest >= 0 && _lastMissionRequest < _writeMissionItems.count()) {
        ...
        prefix = tr("Item #%1 Command: %2")
                     .arg(_lastMissionRequest)
                     .arg(_missionCommandTree->friendlyName(item->command()));
...
QString PlanManager::_missionResultToString(MAV_MISSION_RESULT result)
{
    ...
    case MAV_MISSION_DENIED:
        ...
        break;
    default:
        qWarning(PlanManagerLog)
            << QStringLiteral("Fell off end of switch statement %1 %2").arg(_planTypeString()).arg(result);
...
bool PlanManager::inProgress(void) const { return _transactionInProgress != TransactionNone; }
