...
void CameraCalc::_recalcTriggerDistance(void)
{
    ...
    if (focalLength <= 0 || sensorWidth <= 0 || sensorHeight <= 0 || imageWidth <= 0 || imageHeight <= 0
        || imageDensity <= 0) {
        return;
    }

    if (_valueSetIsDistanceFact.rawValue().toBool()) {
        _imageDensityFact.setRawValue(
            (_distanceToSurfaceFact.rawValue().toDouble() * sensorWidth * 100.0) / (imageWidth * focalLength));
    } else {
        _distanceToSurfaceFact.setRawValue(
            (imageWidth * _imageDensityFact.rawValue().toDouble() * focalLength) / (sensorWidth * 100.0));
    }
    ...
    _adjustedFootprintSideFact.setRawValue(
        _imageFootprintSide * ((100.0 - _sideOverlapFact.rawValue().toDouble()) / 100.0));
    _adjustedFootprintFrontalFact.setRawValue(
        _imageFootprintFrontal * ((100.0 - _frontalOverlapFact.rawValue().toDouble()) / 100.0));
...
bool CameraCalc::load(
    const QJsonObject& originalJson, bool deprecatedFollowTerrain, QString& errorString, bool forPresets)
{
    ...
    if (version == 0) {
        ...
        //  - _jsonCameraSpecTypeKeyDeprecated is only in v0 files and stores CameraSpecType. V2 files store same info
        //  in cameraNameName.
    ...
    if (version == 1) {
        ...
        //  - deprecatedFollowTerrain value was loaded from upper level callers and represents
        //  AltitudeModeCalcAboveTerrain. AtitudeModeTerrainFrame was not supported yet.
        if (deprecatedFollowTerrain) {
            json[distanceModeName] = QGroundControlQmlGlobal::AltitudeModeCalcAboveTerrain;
        } else {
            json[distanceModeName] = json[_jsonDistanceToSurfaceRelativeKeyDeprecated].toBool()
    ...
    // We have to clean up camera names. Older builds incorrectly used translated the camera names in the persisted plan
    // file. Newer builds use a canonical english camera name in plan files.
...
QString CameraCalc::xlatCustomCameraName(void) { return tr("Custom Camera"); }

QString CameraCalc::xlatManualCameraName(void) { return tr("Manual (no camera specs)"); }
...
void CameraCalc::_setDirty(void) { setDirty(true); }
    ...
    if (canonicalCameraName != canonicalCustomCameraName() && canonicalCameraName != canonicalManualCameraName()) {
        if (cameraName == xlatManualCameraName()) {
            ...
            for (int cameraIndex = 0; cameraIndex < _knownCameraList.count(); cameraIndex++) {
                CameraMetaData* cameraMetaData = _knownCameraList[cameraIndex].value<CameraMetaData*>();
                if (cameraName == cameraMetaData->canonicalName
                    || cameraName == cameraMetaData->deprecatedTranslatedName) {
