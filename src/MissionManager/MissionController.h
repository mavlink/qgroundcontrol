...
public:
    ...
    typedef struct {
        ...
        QGCMAVLink::VehicleClass_t
            vtolMode; ///< Either VehicleClassFixedWing, VehicleClassMultiRotor, VehicleClassGeneric (mode unknown)
        double cruiseSpeed;
        double hoverSpeed;
    ...
    Q_PROPERTY(QmlObjectListModel* simpleFlightPathSegments READ simpleFlightPathSegments
            CONSTANT) ///< Used by Plan view only for interactive editing
    Q_PROPERTY(QVariantList waypointPath READ waypointPath NOTIFY
            waypointPathChanged) ///< Used by Fly view only for static display
    Q_PROPERTY(QmlObjectListModel* directionArrows READ directionArrows CONSTANT)
    Q_PROPERTY(QmlObjectListModel* incompleteComplexItemLines READ incompleteComplexItemLines
            CONSTANT) ///< Segments which are not yet completed.
    Q_PROPERTY(QStringList complexMissionItemNames READ complexMissionItemNames NOTIFY complexMissionItemNamesChanged)
    Q_PROPERTY(QGeoCoordinate plannedHomePosition READ plannedHomePosition NOTIFY
            plannedHomePositionChanged) ///< Includes AMSL altitude
    Q_PROPERTY(QGeoCoordinate previousCoordinate MEMBER _previousCoordinate NOTIFY previousCoordinateChanged)
    Q_PROPERTY(FlightPathSegment* splitSegment MEMBER _splitSegment NOTIFY
            splitSegmentChanged) ///< Segment which show show + split ui element
    Q_PROPERTY(double progressPct READ progressPct NOTIFY progressPctChanged)
    Q_PROPERTY(int missionItemCount READ missionItemCount NOTIFY
            missionItemCountChanged) ///< True mission item command count (only valid in Fly View)
    Q_PROPERTY(int currentMissionIndex READ currentMissionIndex NOTIFY currentMissionIndexChanged)
    Q_PROPERTY(int resumeMissionIndex READ resumeMissionIndex NOTIFY
            resumeMissionIndexChanged) ///< Returns the item index two which a mission should be resumed. -1 indicates
                                       ///< resume mission not available.
    ...
    Q_PROPERTY(double minAMSLAltitude MEMBER _minAMSLAltitude NOTIFY
            minAMSLAltitudeChanged) ///< Minimum altitude associated with this mission. Used to calculate percentages
                                    ///< for terrain status.
    Q_PROPERTY(double maxAMSLAltitude MEMBER _maxAMSLAltitude NOTIFY
            maxAMSLAltitudeChanged) ///< Maximum altitude associated with this mission. Used to calculate percentages
                                    ///< for terrain status.

    Q_PROPERTY(QGroundControlQmlGlobal::AltMode globalAltitudeMode READ globalAltitudeMode WRITE setGlobalAltitudeMode
            NOTIFY globalAltitudeModeChanged)
    Q_PROPERTY(QGroundControlQmlGlobal::AltMode globalAltitudeModeDefault READ globalAltitudeModeDefault NOTIFY
            globalAltitudeModeChanged) ///< Default to use for newly created items
    ...
    Q_INVOKABLE VisualMissionItem* insertSimpleMissionItem(
    ...
    ///     @param makeCurrentItem: true: Make this item the current item
    /// @return Newly created item
    Q_INVOKABLE VisualMissionItem* insertTakeoffItem(
        QGeoCoordinate coordinate, int visualItemIndex, bool makeCurrentItem = false);
    ...
    ///     @param makeCurrentItem: true: Make this item the current item
    /// @return Newly created item
    Q_INVOKABLE VisualMissionItem* insertLandItem(
        QGeoCoordinate coordinate, int visualItemIndex, bool makeCurrentItem = false);
    ...
    ///     @param makeCurrentItem: true: Make this item the current item
    /// @return Newly created item
    Q_INVOKABLE VisualMissionItem* insertROIMissionItem(
        QGeoCoordinate coordinate, int visualItemIndex, bool makeCurrentItem = false);
    ...
    ///     @param makeCurrentItem: true: Make this item the current item
    /// @return Newly created item
    Q_INVOKABLE VisualMissionItem* insertComplexMissionItem(
        QString itemName, QGeoCoordinate mapCenterCoordinate, int visualItemIndex, bool makeCurrentItem = false);
    ...
    ///     @param makeCurrentItem: true: Make this item the current item
    /// @return Newly created item
    Q_INVOKABLE VisualMissionItem* insertComplexMissionItemFromKMLOrSHP(
        QString itemName, QString file, int visualItemIndex, bool makeCurrentItem = false);
    ...
    enum SendToVehiclePreCheckState {
        ...
        SendToVehiclePreCheckStateFirwmareVehicleMismatch, // Firmware/Vehicle type for plan mismatch with actual
                                                           // vehicle
    ...
    /// IMPORTANT NOTE: The return value is a VisualMissionItem::ReadForSaveState value. It is an int here to work
    /// around a nightmare of circular header dependency problems.
    ...
    {
        return _missionFlightStatus.batteryChangePoint;
...
private:
    ...
    void _calcPrevWaypointValues(VisualMissionItem* currentItem, VisualMissionItem* prevItem, double* azimuth,
        double* distance, double* altDifference);
    ...
    static bool _convertToMissionItems(
