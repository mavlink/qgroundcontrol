...
void KMLPlanDomDocument::_addFlightPath(Vehicle* vehicle, const QList<MissionItem*> &rgMissionItems)
{
    ...
    for (int i = 0; i < visualItems->count(); i++) {
        ...
        if (transectItem) {
            ...
            if (polygon && polygon->count() >= 3) {
                // Add as KML Placemark with Polygon
                QDomElement placemarkElement = addPlacemark(transectItem->patternName(), true);
                addPolygon(placemarkElement, polygon->coordinateList());
                addTextElement(placemarkElement, "styleUrl", QStringLiteral("#%1").arg(surveyPolygonStyleName));
                appendChildToRoot(placemarkElement);
            }
        }
    }
}

void KMLPlanDomDocument::addMission(Vehicle* vehicle, QmlObjectListModel* visualItems, const QList<MissionItem*> &rgMissionItems)
{
