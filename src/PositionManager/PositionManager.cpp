...
void QGCPositionManager::_positionUpdated(const QGeoPositionInfo& update)
{
    ...
    // At this point only plugins support gcs heading. The reason is that the quality of heading information from a
    // local position device (not a compass) is unknown. In many cases it can only be trusted if the GCS location is
    // moving above a certain rate of speed. When it is not, or the gcs is standing still the heading is just random. We
    // don't want these random heading to be shown on the fly view. So until we can get a true compass based heading or
    // some smarted heading quality information which takes into account the speed of movement we normally don't set a
    // heading. We do use the heading though if the plugin overrides the position source. In that case we assume that it
    // hopefully know what it is doing.
...
int QGCPositionManager::updateInterval() const { return _updateInterval; }
...
void QGCPositionManager::setPositionSource(QGCPositionManager::QGCPositionSource source)
{
    ...
    if (_currentSource != nullptr) {
        ...
        connect(_currentSource, SIGNAL(error(QGeoPositionInfoSource::Error)), this,
            SLOT(_error(QGeoPositionInfoSource::Error)));
