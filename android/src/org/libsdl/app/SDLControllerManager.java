    ...
    void pollInputDevices() {
        ...
        for (int device_id : deviceIds) {
            if (SDLControllerManager.isDeviceSDLJoystick(device_id)) {
                ...
                if (joystick == null) {
                    ...
                    Log.i(TAG, String.format("New joystick discovered: id=%d name='%s' axes=%d hats=%d rumble=%b led=%b vendor=0x%04X product=0x%04X",
                        joystick.device_id, joystick.name, joystick.axes.size(), joystick.hats.size()/2,
                        can_rumble, has_rgb_led, getVendorId(joystickDevice), getProductId(joystickDevice)));
        ...
        if (removedDevices != null) {
            for (int device_id : removedDevices) {
                Log.i(TAG, "Joystick removed: device_id=" + device_id);
