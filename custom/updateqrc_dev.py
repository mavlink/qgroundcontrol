    ...
    for line in file1:
        if line not in file2:
            ...
            if line.startswith('<file'):
                newLine = newLine.replace(">", ">../../../work/UpstreamQGC/", 1)
            file3.write(newLine + '\n')
        else:
            print 'Excluded:', line
    file3.close()
...
def main():
    if(os.path.isfile(qgc_exc)):
        process(os.path.join("../../../work/UpstreamQGC/",qgc_rc), qgc_exc, qgc_rc)
    if(os.path.isfile(res_exc)):
        process(os.path.join("../../../work/UpstreamQGC/",res_rc), res_exc, res_rc)
