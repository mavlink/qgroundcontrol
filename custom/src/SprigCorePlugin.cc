...
SprigFlyViewOptions::SprigFlyViewOptions(SprigOptions* options, QObject* parent)
...
SprigOptions::SprigOptions(SprigCorePlugin *plugin, QObject *parent)
    : QGCOptions(parent)
    , _plugin(plugin)
    , _flyViewOptions(new SprigFlyViewOptions(this, this))
{
    Q_CHECK_PTR(_plugin);
}
...
SprigCorePlugin::SprigCorePlugin(QObject *parent)
    : QGCCorePlugin(parent)
    , _options(new SprigOptions(this, this))
{
    qCDebug(SprigLog) << this;
...
SprigOverrideInterceptor::SprigOverrideInterceptor()
