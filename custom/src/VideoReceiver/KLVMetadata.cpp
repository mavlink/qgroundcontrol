...
std::optional<uint64_t> decodeBERNumber(uint8_t *data, size_t &i, size_t dataSize) {
    ...
    if (berField.has_value()) {
        uint64_t res = 0;
        // We could return nothing if data field is longer than 8 bytes (which shouldn't happen), but in order to avoid it we just interpret last 8 bytes of the field as the valid value
...
KLVMetadata::KLVMetadata(uint8_t *data, size_t size) {
...
std::unordered_map<uint32_t, std::vector<uint8_t>> KLVMetadata::getAllMetadata() {
    return _lastMetadata;
}

std::optional<uint64_t> KLVMetadata::getTimestamp() {
...
std::optional<QString> KLVMetadata::getMissionID() {
...
std::optional<QString> KLVMetadata::getImageSensor() {
...
uint64_t KLVMetadata::_decodeUint64(const std::vector<uint8_t> &value) {
    uint64_t res = 0;
...
QString KLVMetadata::_decodeString(const std::vector<uint8_t> &value) {
    QString res;
