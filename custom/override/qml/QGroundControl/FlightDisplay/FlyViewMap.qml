...
FlightMap {
    ...
    //TODO: add repeater that dynamically add polylines from settings
    ...
    MapPolyline{
        ...
        property int _src: 24 /*24 - MAVLINK_MSG_ID_GPS_RAW_INT*/
        Connections {
            target:                 QGroundControl.multiVehicleManager
            function onActiveVehicleChanged(activeVehicle) {
                gpsTrajectoryPolyline.path = _activeVehicle ? _activeVehicle.trajectoryPoints.list(trajectoryPolyline._src) : []
            }
        }

        Connections {
            target:                             _activeVehicle ? _activeVehicle.trajectoryPoints : null
            function onPointAdded(coordinate, src) {if(src === gpsTrajectoryPolyline._src) gpsTrajectoryPolyline.addCoordinate(coordinate) }
            function onUpdateLastPoint(coordinate, src) {if( src === gpsTrajectoryPolyline._src) gpsTrajectoryPolyline.replaceCoordinate(gpsTrajectoryPolyline.pathLength() - 1, coordinate) }
            function onPointsCleared() { gpsTrajectoryPolyline.path = [] }
        }
    }
    ...
    MapPolyline {
        ...
        z:          QGroundControl.zOrderTrajectoryLines
        visible:    !pipMode
        property int _src: 33 /*33 - MAVLINK_MSG_ID_GLOBAL_POSITION_INT*/
        Connections {
            target:                 QGroundControl.multiVehicleManager
            function onActiveVehicleChanged(activeVehicle) {
                trajectoryPolyline.path = _activeVehicle ? _activeVehicle.trajectoryPoints.list(trajectoryPolyline._src) : []
            }
        }
