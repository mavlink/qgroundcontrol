...
s3cmd --add-header=x-amz-meta-firmware-version:${version} -m text/plain --acl-public --add-header='Cache-Control: public, max-age=0' put CHANGELOG.md s3://${S3_BUCKET_NAME}/datapilot/latest/changelog;
s3cmd --add-header=x-amz-meta-firmware-version:${version} -m text/plain --acl-public --add-header='Cache-Control: public, max-age=0' put CHANGELOG.md s3://${S3_BUCKET_NAME}/datapilot/$version/changelog;
