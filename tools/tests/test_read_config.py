...
def test_export_bash_format(tmp_path: Path) -> None:
    ...
    assert 'export GSTREAMER_DEFAULT_VERSION="1.28.0"' in result.stdout
...
def test_github_output_includes_ios_modules(tmp_path: Path) -> None:
    ...
    assert "gstreamer_default_version=1.28.0" in output_text
    assert "gstreamer_version=1.28.0" in output_text
    assert "gstreamer_ios_version=1.28.0" in output_text
