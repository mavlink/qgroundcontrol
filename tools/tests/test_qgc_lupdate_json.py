...
_MODULE_PATH = _TOOLS_DIR / "translations" / "qgc_lupdate_json.py"

_spec = importlib.util.spec_from_file_location("qgc_lupdate_json", _MODULE_PATH)
assert _spec and _spec.loader
mod = importlib.util.module_from_spec(_spec)
sys.modules["qgc_lupdate_json"] = mod
...
def test_walk_directory_exits_on_duplicate_filenames(tmp_path: Path) -> None:
    (tmp_path / "a").mkdir()
    (tmp_path / "b").mkdir()
    (tmp_path / "a" / "Dup.json").write_text(
        '{"fileType": "MavCmdInfo", "label": "X"}'
    )
    (tmp_path / "b" / "Dup.json").write_text(
        '{"fileType": "MavCmdInfo", "label": "Y"}'
    )
...
def test_split_disambiguation_extracts_comment() -> None:
    assert mod._split_disambiguation(
        "#loc.disambiguation#foo context#actual", "ctx"
    ) == ("foo context", "actual")
