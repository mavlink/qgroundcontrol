...
def _qml_control(ctrl: ControlDef, indent: str, *, indexed: bool = False, dialog_counter: list[int] | None = None, tr_context: str = "") -> str:
    ...
    if control_type == "component" and ctrl.component:
        qml = _env.get_template("control_component.qml.j2").render(
            indent=indent,
    ...
    if control_type == "dialogButton" and ctrl.dialogButton:
        ...
        if ctrl.param:
            ...
            qml = _env.get_template("control_dialog_with_param.qml.j2").render(
                indent=indent,
                ri=ri,
    ...
    if control_type == "combobox":
        if ctrl.enumValues:
            qml = _env.get_template("control_combo_enum.qml.j2").render(
                indent=indent,
                fact_ref=fact_ref,
                display_label=qml_tr(ctrl.label, tr_context) if ctrl.label
...
def _qml_repeat_section(sec: SectionDef, sec_idx: int, tr_context: str = "") -> str:
    ...
    dialog_counter = [0]
    controls = [
        _qml_control(ctrl, ctrl_indent, indexed=True, dialog_counter=dialog_counter, tr_context=tr_context)
    ...
    return _env.get_template("repeat_section.qml.j2").render(
        ind=ind,
        safe=safe,
        visible=visible,
...
def _qml_disabled_companion_section(sec: SectionDef, tr_context: str = "") -> str:
    ...
    if rep.firstIndexOmitsNumber:
        loop_idx_expr = f'(i + {rep.startIndex}) === {rep.startIndex} ? "" : String(i + {rep.startIndex})'
        item_idx_expr = (
            f'(index + {rep.startIndex}) === {rep.startIndex} ? "" : String(index + {rep.startIndex})'
        )
    else:
    ...
    if apm_battery:
        ...
        label_expr = f'_{safe}Count > 1 ? {title_qtr} + " " + _battLabelForIndex(index) : {title_qtr}'
    else:
        fact_expr = f'controller.getParameterFact(-1, "{rep.paramPrefix}" + _idx + "{rep.enableParam}")'
        label_expr = f'_{safe}Count > 1 ? {title_qtr} + " " + String(index + {rep.startIndex}) : {title_qtr}'

    return _env.get_template("disabled_companion.qml.j2").render(
        ind=ind,
        safe=safe,
        camel=camel,
        dv=dv,
...
def generate_config_page_qml(page: PageDef) -> str:
    ...
    repeat_count_props = [
        _qml_repeat_count_property(sec) for sec in page.sections if sec.repeat
    ]
    ...
    for sec_idx, sec in enumerate(page.sections):
        if sec.repeat:
            ...
            sections_qml.append(_qml_component_section(sec, sec_idx, tr_context=tr_ctx))
        else:
            sections_qml.append(_qml_generated_section(sec, sec_idx, tr_context=tr_ctx))
