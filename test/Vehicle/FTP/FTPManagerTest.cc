...
void FTPManagerTest::_testUploadCancel(void)
{
    _connectMockLinkNoInitialConnectSequence();

    FTPManager* ftpManager = _vehicle->ftpManager();
    const QString remotePath(QStringLiteral("/mock/upload/cancelled.bin"));
    ...
    QVERIFY(spyUploadComplete.wait(1000) || spyUploadComplete.count() > 0);
    QCOMPARE(spyUploadComplete.count(), 1);

    QList<QVariant> arguments = spyUploadComplete.takeFirst();
    QVERIFY(arguments[1].toString().contains("Abort"));

    _disconnectMockLink();
}

void FTPManagerTest::_testDelete(void)
{
    _connectMockLinkNoInitialConnectSequence();
    ...
    QSignalSpy spyDeleteComplete(ftpManager, &FTPManager::deleteComplete);

    // MockLinkFTP should handle delete requests
    QVERIFY(ftpManager->deleteFile(MAV_COMP_ID_AUTOPILOT1, "/test/file.txt"));

    QCOMPARE(spyDeleteComplete.wait(10000), true);
    QCOMPARE(spyDeleteComplete.count(), 1);

    QList<QVariant> arguments = spyDeleteComplete.takeFirst();
    QCOMPARE(arguments[0].toString(), QStringLiteral("/test/file.txt"));
    // Check for success (empty error message)
    QVERIFY(arguments[1].toString().isEmpty());

    _disconnectMockLink();
}

void FTPManagerTest::_testDeleteNoResponse(void)
{
    _connectMockLinkNoInitialConnectSequence();
    ...
    QList<QVariant> arguments = spyDeleteComplete.takeFirst();
    // Should have error message due to timeout
    QVERIFY(!arguments[1].toString().isEmpty());

    _disconnectMockLink();
}

void FTPManagerTest::_testDeleteNakResponse(void)
{
    _connectMockLinkNoInitialConnectSequence();
    ...
    QList<QVariant> arguments = spyDeleteComplete.takeFirst();
    // Should have error message due to NAK
    QVERIFY(!arguments[1].toString().isEmpty());

    _disconnectMockLink();
}

void FTPManagerTest::_testDeleteCancel(void)
{
    _connectMockLinkNoInitialConnectSequence();
    ...
    // deleteComplete is signalled immediately on calling cancelDelete
    QCOMPARE(spyDeleteComplete.count(), 1);
    QList<QVariant> arguments = spyDeleteComplete.takeFirst();
    QCOMPARE(arguments[1].toString(), QStringLiteral("Aborted"));
...
void FTPManagerTest::_testIsOperationInProgress(void)
{
    _connectMockLinkNoInitialConnectSequence();
    ...
    // Initially no operation should be in progress
    QVERIFY(!ftpManager->isOperationInProgress());
    ...
    // Now an operation should be in progress
    QVERIFY(ftpManager->isOperationInProgress());

    // Wait for completion
    QCOMPARE(spyListDirectoryComplete.wait(10000), true);

    // After completion, no operation should be in progress
    QVERIFY(!ftpManager->isOperationInProgress());

    _disconnectMockLink();
}

void FTPManagerTest::_testConcurrentOperationsPrevented(void)
{
    _connectMockLinkNoInitialConnectSequence();
    ...
    // Set slow response mode to keep operation in progress
    _mockLink->mockLinkFTP()->setErrorMode(MockLinkFTP::errModeNoSecondResponseAllowRetry);
    ...
    QVERIFY(!ftpManager->listDirectory(MAV_COMP_ID_AUTOPILOT1, "/other"));
    QVERIFY(!ftpManager->deleteFile(MAV_COMP_ID_AUTOPILOT1, "/test.txt"));

    // Wait for first operation to complete
    QCOMPARE(spyListDirectoryComplete.wait(10000), true);

    // Now we should be able to start another operation
    QVERIFY(ftpManager->listDirectory(MAV_COMP_ID_AUTOPILOT1, "/other"));
