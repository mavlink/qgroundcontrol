...
void LogRemoteSinkTest::_testSetEndpointHostname()
{
    _sink->setEndpoint(QString("localhost:%1").arg(_udpPort));
    _sink->setEnabled(true);

    LogEntry entry;
    entry.timestamp = QDateTime::currentDateTime();
    entry.message = "hostname endpoint";
...
void LogRemoteSinkTest::_testTcpSend()
{
    ...
    // Extract the payload after the length header.
    QVERIFY2(data.size() >= 4, "TCP data too short for length-prefix frame");
    const quint32 frameLen = qFromBigEndian<quint32>(data.constData());
    QCOMPARE(static_cast<int>(frameLen), data.size() - 4);
    const QByteArray payload = data.mid(4);

    // Decompress (strips header byte)
    QByteArray jsonData = LogCompression::decompress(payload);
    QJsonDocument doc = QJsonDocument::fromJson(jsonData);
...
void LogRemoteSinkTest::_testMaxPendingEntries()
{
    ...
    _sink->setMaxPendingEntries(0);
    QCOMPARE(_sink->maxPendingEntries(), 1);

    _sink->setMaxPendingEntries(-10);
    QCOMPARE(_sink->maxPendingEntries(), 1);
