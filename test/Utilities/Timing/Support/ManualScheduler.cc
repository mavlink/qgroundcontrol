...
ManualScheduler::ManualScheduler(QObject* parent, quint64 initialUs) : RuntimeScheduler(parent), _nowUs(initialUs)
{
...
{
    if (!_isCurrentThread() || _advancing || targetUs < _nowUs) {
        return false;
    }
    _advancing = true;
...
{
    return _isCurrentThread() && duration.count() >= 0 &&
