...
void SkippableAsyncStateTest::_testSkippableAsyncStateExecute()
{
    ...
    // Use MultiSignalSpyV2 to verify advance() fired but skipped() did not
    ...
    QVERIFY(stateSpy.checkSignalByMask(stateSpy.signalNameToMask("advance")));
    QVERIFY(stateSpy.checkNoSignalByMask(stateSpy.signalNameToMask("skipped")));
}
...
void SkippableAsyncStateTest::_testSkippableAsyncStateSkip()
{
    ...
    // Use MultiSignalSpyV2 to verify skipped() fired but advance() did not
    MultiSignalSpyV2 stateSpy;
    QVERIFY(stateSpy.init(skippableState));
    ...
    QVERIFY(stateSpy.checkSignalByMask(stateSpy.signalNameToMask("skipped")));
...
void SkippableAsyncStateTest::_testSkippableAsyncStateTimeout()
{
    ...
    // Use MultiSignalSpyV2 to verify timeout() fired but advance() and skipped() did not
    MultiSignalSpyV2 stateSpy;
    QVERIFY(stateSpy.init(skippableState));
