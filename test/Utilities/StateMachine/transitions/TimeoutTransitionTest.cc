...
void TimeoutTransitionTest::_testTimeoutFires()
{
    ...
    timeoutTransition->attachToSourceState(startState);
...
void TimeoutTransitionTest::_testTimeoutRestartsOnReentry()
{
    ...
    timeoutTransition->attachToSourceState(waitState);
    waitState->addTransition(timeoutTransition);

    // Route by count: retry loop while count < 2, then complete.
    auto* toWait = new GuardedTransition(countState, &QGCState::advance, waitState,
        [&timeoutCount]() { return timeoutCount < 2; });
    countState->addTransition(toWait);
...
void TimeoutTransitionTest::_testMultipleTimeoutTransitions()
{
    ...
    shortTimeout->attachToSourceState(startState);
    longTimeout->attachToSourceState(startState);
...
UT_REGISTER_TEST(TimeoutTransitionTest, TestLabel::Unit, TestLabel::Utilities)
