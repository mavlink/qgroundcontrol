...
void GeoTest::_distanceProperties_test()
{
    ...
    const QGeoCoordinate a(47.3764, 8.5481, 0.0);
    const QGeoCoordinate b(48.0, 9.0, 0.0);
    QVERIFY(QGCGeo::geodesicDistance(a, b) >= 0.0);

    // Distance to self is zero
    QCOMPARE(QGCGeo::geodesicDistance(a, a), 0.0);

    // Symmetry: d(a,b) == d(b,a)
    QVERIFY(compareDoubles(QGCGeo::geodesicDistance(a, b), QGCGeo::geodesicDistance(b, a), 0.001));
}

void GeoTest::_nedRoundtripProperty_test()
{
    // Converting to NED and back should return the original coordinate
    const QGeoCoordinate ref(47.3764, 8.5481, 0.0);
    const QGeoCoordinate point(47.38, 8.55, 100.0);

    double north, east, down;
    QGCGeo::convertGeoToNed(point, ref, north, east, down);

    QGeoCoordinate result;
    QGCGeo::convertNedToGeo(north, east, down, ref, result);

    QVERIFY(compareDoubles(result.latitude(), point.latitude(), 0.00001));
    QVERIFY(compareDoubles(result.longitude(), point.longitude(), 0.00001));
    QVERIFY(compareDoubles(result.altitude(), point.altitude(), 0.1));
}
