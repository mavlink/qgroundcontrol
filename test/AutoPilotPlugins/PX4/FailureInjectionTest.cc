...
void FailureInjectionTest::_logRowAddsPendingEntryWithoutTracking()
{
    ...
    failureInjection.logRow(QStringLiteral("GPS"), QStringLiteral("Off"), QStringLiteral("1"), QStringLiteral("12:00:00"));

    QCOMPARE(activityChangedSpy.count(), 1);
    QCOMPARE(failureInjection.activity().count(), 1);
    QCOMPARE(failureInjection.activity().first().toMap().value(QStringLiteral("result")).toString(), QStringLiteral("pending"));
...
void FailureInjectionTest::_logInjectionTracksUnitOnce()
{
    ...
    failureInjection.logInjection(QStringLiteral("GPS"), QStringLiteral("Off"), FAILURE_UNIT_SENSOR_GPS, QStringLiteral("1"), QStringLiteral("12:00:00"));
    ...
    failureInjection.logInjection(QStringLiteral("GPS"), QStringLiteral("Stuck"), FAILURE_UNIT_SENSOR_GPS, QStringLiteral("2"), QStringLiteral("12:00:01"));
    QCOMPARE(failureInjection.injectedUnits().count(), 1);
...
void FailureInjectionTest::_resolveResultResolvesOldestPendingRow()
{
    ...
    failureInjection.logRow(QStringLiteral("GPS"), QStringLiteral("Off"), QStringLiteral("1"), QStringLiteral("12:00:00"));
    failureInjection.logRow(QStringLiteral("GYRO"), QStringLiteral("Stuck"), QStringLiteral("2"), QStringLiteral("12:00:01"));

    failureInjection.resolveResult(MAV_RESULT_ACCEPTED);

    const QVariantList activity = failureInjection.activity();
    QCOMPARE(activity.at(1).toMap().value(QStringLiteral("result")).toString(), QStringLiteral("accepted")); // oldest (GPS) resolved first
    QCOMPARE(activity.at(0).toMap().value(QStringLiteral("result")).toString(), QStringLiteral("pending"));  // newest (GYRO) still pending
}
