...
void NTRIPSourceTableTest::_testParseSTRLine()
{
    ...
    QCOMPARE(mp->property("format").toString(), QStringLiteral("RTCM 3.2"));
    QCOMPARE(mp->carrier(), 2);
    QCOMPARE(mp->property("navSystem").toString(), QStringLiteral("GPS+GLO+GAL"));
    QCOMPARE(mp->property("network").toString(), QStringLiteral("NET01"));
    QCOMPARE(mp->property("country").toString(), QStringLiteral("USA"));
    QVERIFY(qFuzzyCompare(mp->latitude(), 40.0));
    QVERIFY(qFuzzyCompare(mp->longitude(), -74.0));
    QVERIFY(!mp->property("nmea").toBool());
    QVERIFY(mp->property("solution").toBool());
    QCOMPARE(mp->property("generator").toString(), QStringLiteral("QGC Test"));
    QCOMPARE(mp->property("authentication").toString(), QStringLiteral("B"));
    QVERIFY(!mp->property("fee").toBool());
    QCOMPARE(mp->property("bitrate").toInt(), 4800);
