...
void NTRIPIntegrationTest::testRtcmFlowsToRouter()
{
    ...
    auto conn = connect(router, &RTCMRouter::routeToVehicles, this, [&](const QByteArray &data) {
        routedData.append(data);
    });
    ...
    // Note: routeAll calls routeToVehicles + routeToBaseStation
    // The router->routeAll is called from _rtcmDataReceived, not directly connected
    // So we check stats instead
...
void NTRIPIntegrationTest::testGgaSentAfterConnect()
{
    ...
    // Process events to let the GGA timer fire
    QTest::qWait(1500);

    // GGA may or may not be sent depending on position availability
    // The key test is that no crash occurs and the transport is properly wired
    QCOMPARE(mgr->connectionStatus(), NTRIPManager::ConnectionStatus::Connected);
...
UT_REGISTER_TEST(NTRIPIntegrationTest, TestLabel::Unit)
