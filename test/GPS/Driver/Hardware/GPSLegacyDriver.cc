...
QGC_LOGGING_CATEGORY(GPSLegacyDriverLog, "GPS.GPSLegacyDriver")
QGC_LOGGING_CATEGORY(GPSDriversLog, "GPS.LegacyDrivers")  // backs the px4 GPS_INFO/WARN/ERR macros in definitions.h

namespace {
int callbackTrampoline(GPSCallbackType type, void* data1, int data2, void* user)
{
    return static_cast<GPSLegacyDriver*>(user)->handleCallback(static_cast<int>(type), data1, data2);
}
}  // namespace

struct GPSLegacyDriver::State
{
...
GPSLegacyDriver::GPSLegacyDriver(GPSType type, GPSTransport& transport, const GPSReceiverConfig& config,
...
bool GPSLegacyDriver::configure()
{
    ...
    if (!_state->driver) {
        qCWarning(GPSLegacyDriverLog) << "Unsupported GPS type:" << static_cast<int>(_type);
        return false;
    }
    ...
    if (_state->driver->configure(baudrate, gpsConfig) != 0) {
        qCWarning(GPSLegacyDriverLog) << "Driver configuration failed for type" << static_cast<int>(_type);
        _state->driver.reset();
        return false;
    }
...
int GPSLegacyDriver::receive(unsigned timeoutMs)
{
    if (!_state->driver) {
        return -1;
    }
...
int GPSLegacyDriver::handleCallback(int type, void* data1, int data2)
{
    switch (static_cast<GPSCallbackType>(type)) {
        ...
        case GPSCallbackType::gotRTCMMessage:
            if (!data1 || data2 <= 0) {
                qCWarning(GPSLegacyDriverLog) << "Invalid RTCM callback payload";
                return -1;
            }
            if (_sinks.onRTCM) {
