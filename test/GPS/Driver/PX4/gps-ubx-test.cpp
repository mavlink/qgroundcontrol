...
static void nmeaOutput(bool legacy, bool base_capable)
{
	...
	CHECK(f.configure(GPSHelper::OutputMode::GPS, GPSDriverUBX::OutputProtocol::NMEA) == 0);
	CHECK(f.driver.receiverReady());
	if (legacy) {
...
static void nmeaOutputFailures()
{
	...
	for (auto output : {GPSHelper::OutputMode::RTCM, GPSHelper::OutputMode::GPSAndRTCM}) {
		Fixture f;
	...
	{
		Fixture f(GPSDriverUBX::UBXMode::MovingBaseUART1);
		CHECK(f.configure(GPSHelper::OutputMode::GPS, GPSDriverUBX::OutputProtocol::NMEA) < 0);
		CHECK(f.receiver.current_settings.empty());
	}
	{
		Fixture f;
		CHECK(f.configure(GPSHelper::OutputMode::GPS, static_cast<GPSDriverUBX::OutputProtocol>(99)) < 0);
		CHECK(f.receiver.current_settings.empty());
	}
}
