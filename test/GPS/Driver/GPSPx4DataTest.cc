...
int main()
{
    ...
    auto report = GPSPx4Data::position(raw);
    ...
    report = GPSPx4Data::position(raw);
    ...
    for (const bool valid : {false, true}) {
        ...
        const auto motion = GPSPx4Data::position(raw);
        const bool expectedMotion = valid ? motion.speedMetersPerSecond == 12.5f && motion.courseRadians == 1.25f
                                          : std::isnan(motion.speedMetersPerSecond) && std::isnan(motion.courseRadians);
        if (!expectedMotion || motion.fixType != GPSPositionReport::FixType::RTKFixed) {
            std::cerr << "Motion validity must be preserved independently of the position fix\n";
            return 7;
        }
    }
    ...
    satellites.snr[0] = 0;
    const auto ublox = GPSPx4Data::satellites(satellites, GPSType::ublox);
    ...
    const auto sbf = GPSPx4Data::satellites(satellites, GPSType::septentrio);
    const auto ashtech = GPSPx4Data::satellites(satellites, GPSType::trimble);
