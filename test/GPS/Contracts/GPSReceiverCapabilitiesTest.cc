...
void GPSReceiverCapabilitiesTest::_completeValidation()
{
    const auto capabilities = GPSReceiverCapabilities::forType(GPSType::u_blox);
    GPSReceiverConfig config;
    QVERIFY(!capabilities.validationError(config).isEmpty());
    config.base.surveyInAccMeters = 1.0;
    config.base.surveyInDurationSecs = 60;
    ...
    QVERIFY(!capabilities.validationError(config).isEmpty());
    config.base.fixedBaseLatitude = 47.0;
    QVERIFY(capabilities.validationError(config).isEmpty());
