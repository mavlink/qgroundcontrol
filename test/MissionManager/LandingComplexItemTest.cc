...
namespace {
// Tolerance for coordinate comparison after float-precision round trips through mission items
// 1e-5 degrees ≈ 1 meter, which accommodates float precision loss in param5/param6
constexpr double kCoordTolerance = 1e-5;

bool fuzzyCompareLatLon(const QGeoCoordinate& coord1, const QGeoCoordinate& coord2)
{
    return QGC::fuzzyCompare(coord1.latitude(), coord2.latitude(), kCoordTolerance) &&
           QGC::fuzzyCompare(coord1.longitude(), coord2.longitude(), kCoordTolerance);
}
}  // namespace
...
void LandingComplexItemTest::init()
{
    ...
    _item = new SimpleLandingComplexItem(planController(), false /* flyView */);
    // Start in a clean state
    QVERIFY(!_item->dirty());
    _item->setLandingCoordinate(Coord::seattle());
...
void LandingComplexItemTest::_testDirty()
{
    ...
    _item->setDirty(true);
    QVERIFY(_item->dirty());
    QVERIFY(_viMultiSpy->onlyEmittedOnce("dirtyChanged"));
    QVERIFY(_viMultiSpy->argument<bool>("dirtyChanged"));
    _item->setDirty(false);
    _viMultiSpy->clearAllSignals();
    // These facts should set dirty when changed
    QList<Fact*> rgFacts;
    rgFacts << _item->finalApproachAltitude() << _item->useDoChangeSpeed() << _item->finalApproachSpeed()
            << _item->landingHeading() << _item->loiterRadius() << _item->loiterClockwise() << _item->landingAltitude()
...
void LandingComplexItemTest::_testItemCount()
{
    ...
    for (size_t i = 0; i < sizeof(rgTestCases) / sizeof(rgTestCases[0]); i++) {
        ...
        QCOMPARE(items.count(), 3 + (testCase.stopTakingPhotos * CameraSection::stopTakingPhotosCommandCount()) +
                                    (testCase.stopTakingVideo * CameraSection::stopTakingVideoCommandCount()));
...
void LandingComplexItemTest::_testAppendSectionItems()
{
    ...
    for (size_t i = 0; i < sizeof(rgTestCases) / sizeof(rgTestCases[0]); i++) {
        ...
        QCOMPARE(CameraSection::scanStopTakingPhotos(simpleItems, 1, false /* removeScannedItems */),
                 testCase.stopTakingPhotos);
        QCOMPARE(CameraSection::scanStopTakingVideo(
                     simpleItems, 1 + (testCase.stopTakingPhotos ? CameraSection::stopTakingPhotosCommandCount() : 0),
                     false /* removeScannedItems */),
                 testCase.stopTakingVideo);
        // Lastly is final approach item and land
        int finalApproachIndex = 1 + (testCase.stopTakingPhotos ? CameraSection::stopTakingPhotosCommandCount() : 0) +
...
void LandingComplexItemTest::_testScanForItems()
{
    ...
    for (size_t i = 0; i < sizeof(rgTestCases) / sizeof(rgTestCases[0]); i++) {
        ...
        QVERIFY(LandingComplexItem::_scanForItems(visualItems, false /* flyView */, planController(),
                                                  &SimpleLandingComplexItem::_isValidLandItem,
                                                  &SimpleLandingComplexItem::_createItem));
...
void LandingComplexItemTest::_testSaveLoad()
{
    ...
    SimpleLandingComplexItem* newItem = new SimpleLandingComplexItem(planController(), false /* flyView */);
    bool loadSuccess =
    ...
    newItem = new SimpleLandingComplexItem(planController(), false /* flyView */);
    ...
    saveObject[LandingComplexItem::_jsonDeprecatedLoiterCoordinateKey] =
        saveObject[LandingComplexItem::_jsonFinalApproachCoordinateKey];
...
{
    QVERIFY(actualItem);
    QCOMPARE(actualItem->stopTakingPhotos()->rawValue().toBool(),
             expectedItem->stopTakingPhotos()->rawValue().toBool());
    ...
    QCOMPARE(actualItem->finalApproachAltitude()->rawValue().toInt(),
             expectedItem->finalApproachAltitude()->rawValue().toInt());
    QCOMPARE(actualItem->useDoChangeSpeed()->rawValue().toBool(),
             expectedItem->useDoChangeSpeed()->rawValue().toBool());
    QCOMPARE(actualItem->finalApproachSpeed()->rawValue().toInt(),
             expectedItem->finalApproachSpeed()->rawValue().toInt());
    ...
    if (actualItem->useLoiterToAlt()->rawValue().toBool()) {
        ...
        QCOMPARE(actualItem->loiterClockwise()->rawValue().toBool(),
                 expectedItem->loiterClockwise()->rawValue().toBool());
    }
}
...
SimpleLandingComplexItem::SimpleLandingComplexItem(PlanMasterController* masterController, bool flyView)
    : LandingComplexItem(masterController, flyView),
      _metaDataMap(
          FactMetaData::createMapFromJsonFile(QStringLiteral(":/json/VTOLLandingPattern.FactMetaData.json"), this)),
...
MissionItem* SimpleLandingComplexItem::_createLandItem(int seqNum, bool altRel, double lat, double lon, double alt,
                                                       QObject* parent)
{
    return new MissionItem(seqNum, MAV_CMD_NAV_LAND, altRel ? MAV_FRAME_GLOBAL_RELATIVE_ALT : MAV_FRAME_GLOBAL, 0.0,
...
bool SimpleLandingComplexItem::_isValidLandItem(const MissionItem& missionItem)
{
    if (missionItem.command() != MAV_CMD_NAV_LAND ||
        ...
        missionItem.param1() != 0 || missionItem.param2() != 0 || missionItem.param3() != 0 ||
        missionItem.param4() != 0 || qIsNaN(missionItem.param5()) || qIsNaN(missionItem.param6()) ||
        qIsNaN(missionItem.param7())) {
        return false;
    } else {
        return true;
    }
...
void SimpleLandingComplexItem::_updateFlightPathSegmentsDontCallDirectly()
{
    ...
    if (useLoiterToAlt()->rawValue().toBool()) {
        _appendFlightPathSegment(FlightPathSegment::SegmentTypeGeneric, finalApproachCoordinate(), amslEntryAlt(),
                                 slopeStartCoordinate(),
                                 amslEntryAlt());  // Best we can do to simulate loiter circle terrain profile
        _appendFlightPathSegment(FlightPathSegment::SegmentTypeLand, slopeStartCoordinate(), amslEntryAlt(),
                                 landingCoordinate(), amslExitAlt());
    } else {
        _appendFlightPathSegment(FlightPathSegment::SegmentTypeLand, finalApproachCoordinate(), amslEntryAlt(),
                                 landingCoordinate(), amslExitAlt());
    }
...
UT_REGISTER_TEST(LandingComplexItemTest, TestLabel::Unit, TestLabel::MissionManager)
