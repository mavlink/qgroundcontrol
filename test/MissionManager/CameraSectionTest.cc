...
void CameraSectionTest::init()
{
    ...
    _validGimbalItem = new SimpleMissionItem(
        planController(),
        false,                                                                            // flyView
        MissionItem(0, MAV_CMD_DO_MOUNT_CONTROL, MAV_FRAME_MISSION, 10.1234, 0, 20.1234,  // pitch, roll, yaw
                    0, 0, 0,                           // alt, lat, lon (all 0 since unused)
                    MAV_MOUNT_MODE_MAVLINK_TARGETING,  // control gimbal with pitch, roll, yaw settings
                    true, false));
    _validTimeItem = new SimpleMissionItem(planController(),
                                           false,                           // flyView
                                           MissionItem(0, MAV_CMD_IMAGE_START_CAPTURE, MAV_FRAME_MISSION,
                                                       0,                   // Reserved, must be 0
                                                       48,                  // time interval
                                                       0,                   // 0 = capture forever
                                                       NAN, NAN, NAN, NAN,  // Reserved
                                                       true, false));
    _validDistanceItem = new SimpleMissionItem(planController(),
                                               false,           // flyView
                                               MissionItem(0, MAV_CMD_DO_SET_CAM_TRIGG_DIST, MAV_FRAME_MISSION,
                                                           72,  // trigger distance
                                                           0,   // not shutter integration
                                                           1,   // trigger immediately
                                                           0, 0, 0, 0, true, false));
    _validStartVideoItem = new SimpleMissionItem(
        planController(),
        false,                                      // flyView
        MissionItem(0,                              // sequence number
                    MAV_CMD_VIDEO_START_CAPTURE, MAV_FRAME_MISSION,
                    0,                              // Reserved (Set to 0)
    ...
    _validCameraPhotoModeItem =
        new SimpleMissionItem(planController(),
                              false,                                                   // flyView
                              MissionItem(0,                                           // sequence number
                                          MAV_CMD_SET_CAMERA_MODE, MAV_FRAME_MISSION,
                                          0,                                           // Reserved (Set to 0)
                                          CAMERA_MODE_IMAGE, NAN, NAN, NAN, NAN, NAN,  // param 3-7 reserved
                                          true,                                        // autocontinue
                                          false));                                     // isCurrentItem
    _validCameraVideoModeItem =
        new SimpleMissionItem(planController(),
                              ...
                              MissionItem(0,                                           // sequence number
                                          MAV_CMD_SET_CAMERA_MODE, MAV_FRAME_MISSION,
                                          0,                                           // Reserved (Set to 0)
                                          CAMERA_MODE_VIDEO, NAN, NAN, NAN, NAN, NAN,  // param 3-7 reserved
                                          true,                                        // autocontinue
                                          false));                                     // isCurrentItem
    _validCameraSurveyPhotoModeItem =
        new SimpleMissionItem(planController(),
                              ...
                              MissionItem(0,                                                  // sequence number
                                          MAV_CMD_SET_CAMERA_MODE, MAV_FRAME_MISSION,
                                          0,                                                  // Reserved (Set to 0)
                                          CAMERA_MODE_IMAGE_SURVEY, NAN, NAN, NAN, NAN, NAN,  // param 3-7 reserved
                                          true,                                               // autocontinue
                                          false));                                            // isCurrentItem
    _validTakePhotoItem = new SimpleMissionItem(planController(),
                                                false,                                        // flyView
                                                MissionItem(0, MAV_CMD_IMAGE_START_CAPTURE, MAV_FRAME_MISSION,
                                                            0,                                // Reserved (Set to 0)
                                                            0,                                // Interval (none)
                                                            ...
                                                            true,                             // autoContinue
                                                            false));                          // isCurrentItem
    _validStopVideoItem = createValidStopVideoItem(planController());
    _validStopDistanceItem = createValidStopDistanceItem(planController());
    _validStopTimeItem = createValidStopTimeItem(planController());
}
...
void CameraSectionTest::_testDirty()
{
    ...
    QVERIFY(_spySection->noneEmitted());
    ...
    QVERIFY(_spySection->noneEmitted());
    QCOMPARE(_cameraSection->dirty(), false);
    _cameraSection->cameraPhotoIntervalDistance()->setRawValue(
        _cameraSection->cameraPhotoIntervalDistance()->rawValue());
    QVERIFY(_spySection->noneEmitted());
    QCOMPARE(_cameraSection->dirty(), false);
    // Check for no duplicate dirty signalling on change
    _cameraSection->setSpecifyGimbal(!_cameraSection->specifyGimbal());
    QVERIFY(_spySection->emittedOnce("dirtyChanged"));
    QCOMPARE(_spySection->argument<bool>("dirtyChanged"), true);
    QCOMPARE(_cameraSection->dirty(), true);
    _spySection->clearAllSignals();
    _cameraSection->setSpecifyGimbal(!_cameraSection->specifyGimbal());
    QVERIFY(_spySection->notEmitted("dirtyChanged"));
    QCOMPARE(_cameraSection->dirty(), true);
    ...
    _cameraSection->setDirty(false);
    _spySection->clearAllSignals();
    _cameraSection->cameraPhotoIntervalTime()->setRawValue(
        _cameraSection->cameraPhotoIntervalTime()->rawValue().toInt() + 1);
    QVERIFY(_spySection->emittedOnce("dirtyChanged"));
    QCOMPARE(_spySection->argument<bool>("dirtyChanged"), true);
    QCOMPARE(_cameraSection->dirty(), true);
    _cameraSection->setDirty(false);
    _spySection->clearAllSignals();
    _cameraSection->cameraPhotoIntervalDistance()->setRawValue(
        _cameraSection->cameraPhotoIntervalDistance()->rawValue().toDouble() + 1);
    QVERIFY(_spySection->emittedOnce("dirtyChanged"));
    QCOMPARE(_spySection->argument<bool>("dirtyChanged"), true);
    QCOMPARE(_cameraSection->dirty(), true);
    _cameraSection->setDirty(false);
...
void CameraSectionTest::_testSettingsAvailable()
{
    ...
    QVERIFY(_spyCamera->emittedOnce("specifyGimbalChanged"));
    QCOMPARE(_spyCamera->argument<bool>("specifyGimbalChanged"), true);
    QVERIFY(_spySection->emittedOnce("settingsSpecifiedChanged"));
    QCOMPARE(_spySection->argument<bool>("settingsSpecifiedChanged"), true);
    _spySection->clearAllSignals();
    ...
    QCOMPARE(_cameraSection->settingsSpecified(), false);
    QVERIFY(_spyCamera->emittedOnce("specifyGimbalChanged"));
    QCOMPARE(_spyCamera->argument<bool>("specifyGimbalChanged"), false);
    QVERIFY(_spySection->emittedOnce("settingsSpecifiedChanged"));
    QCOMPARE(_spySection->argument<bool>("settingsSpecifiedChanged"), false);
    _spySection->clearAllSignals();
    _spyCamera->clearAllSignals();
    _cameraSection->setSpecifyCameraMode(true);
    QCOMPARE(_cameraSection->specifyCameraMode(), true);
    QCOMPARE(_cameraSection->settingsSpecified(), true);
    QVERIFY(_spyCamera->emittedOnce("specifyCameraModeChanged"));
    QCOMPARE(_spyCamera->argument<bool>("specifyCameraModeChanged"), true);
    QVERIFY(_spySection->emittedOnce("settingsSpecifiedChanged"));
    QCOMPARE(_spySection->argument<bool>("settingsSpecifiedChanged"), true);
    _spySection->clearAllSignals();
    _spyCamera->clearAllSignals();
    ...
    QCOMPARE(_cameraSection->settingsSpecified(), false);
    QVERIFY(_spyCamera->emittedOnce("specifyCameraModeChanged"));
    QCOMPARE(_spyCamera->argument<bool>("specifyCameraModeChanged"), false);
    QVERIFY(_spySection->emittedOnce("settingsSpecifiedChanged"));
    QCOMPARE(_spySection->argument<bool>("settingsSpecifiedChanged"), false);
    _spySection->clearAllSignals();
    _spyCamera->clearAllSignals();
...
void CameraSectionTest::_checkAvailable()
{
    MissionItem missionItem(1,           // sequence number
                            ...
                            true,        // autoContinue
                            false);      // isCurrentItem
    SimpleMissionItem* item = new SimpleMissionItem(planController(), false /* flyView */, missionItem);
    QVERIFY(item->cameraSection());
    QCOMPARE(item->cameraSection()->available(), false);
}

void CameraSectionTest::_testItemCount()
{
    // No settings specified to start
    ...
    QVERIFY(_spySection->emitted("itemCountChanged"));
    QCOMPARE(_spySection->argument<int>("itemCountChanged"), 1);
    _spySection->clearAllSignals();
    _spyCamera->clearAllSignals();
    _cameraSection->setSpecifyGimbal(false);
    QCOMPARE(_cameraSection->itemCount(), 0);
    QVERIFY(_spySection->emitted("itemCountChanged"));
    QCOMPARE(_spySection->argument<int>("itemCountChanged"), 0);
    _spySection->clearAllSignals();
    _spyCamera->clearAllSignals();
    _cameraSection->setSpecifyCameraMode(true);
    QCOMPARE(_cameraSection->itemCount(), 1);
    ...
    rgCameraActions << CameraSection::TakePhotosIntervalTime << CameraSection::TakePhotoIntervalDistance
    ...
    QCOMPARE(_cameraSection->itemCount(), 2);
    QVERIFY(_spySection->emitted("itemCountChanged"));
    QCOMPARE(_spySection->argument<int>("itemCountChanged"), 2);
    _spySection->clearAllSignals();
    _spyCamera->clearAllSignals();
}
...
void CameraSectionTest::_testAppendSectionItems()
{
    ...
    QCOMPARE(seqNum, 3);
    _missionItemsEqual(*rgMissionItems[0],
                       _validCameraPhotoModeItem->missionItem());  // Camera mode change must always be first
...
void CameraSectionTest::_testScanForGimbalSection()
{
    ...
    SimpleMissionItem* newValidGimbalItem =
        new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
    ...
    */
    ...
    SimpleMissionItem invalidSimpleItem(planController(), false /* flyView */, _validGimbalItem->missionItem());
    ...
    visualItems.clear();
    invalidSimpleItem.missionItem() = _validGimbalItem->missionItem();
    invalidSimpleItem.missionItem().setParam7(
        MAV_MOUNT_MODE_RETRACT);  // Only MAV_MOUNT_MODE_MAVLINK_TARGETING supported
    visualItems.append(&invalidSimpleItem);
    QCOMPARE(_cameraSection->scanForSection(&visualItems, scanIndex), false);
    QCOMPARE(visualItems.count(), 1);
    QCOMPARE(_cameraSection->specifyGimbal(), false);
...
void CameraSectionTest::_testScanForCameraModeSection()
{
    QCOMPARE(_cameraSection->available(), true);
    int scanIndex = 0;
    QmlObjectListModel visualItems;
    _commonScanTest(_cameraSection);
    // Check for a scan success
    SimpleMissionItem* newValidCameraModeItem =
        new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
    ...
    */
    ...
    SimpleMissionItem invalidSimpleItem(planController(), false /* flyView */,
                                        _validCameraPhotoModeItem->missionItem());
    std::function<void(MissionItem&, double)> rgSetParamFns[] = {
        &MissionItem::setParam1, &MissionItem::setParam2, &MissionItem::setParam3, &MissionItem::setParam4,
        &MissionItem::setParam5, &MissionItem::setParam6, &MissionItem::setParam7};
...
void CameraSectionTest::_testScanForPhotoIntervalTimeSection()
{
    QCOMPARE(_cameraSection->available(), true);
    int scanIndex = 0;
    QmlObjectListModel visualItems;
...
*/
    SimpleMissionItem* newValidTimeItem =
        new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
    ...
    QCOMPARE(_cameraSection->cameraPhotoIntervalTime()->rawValue().toInt(),
             (int)_validTimeItem->missionItem().param2());
    visualItems.clear();
    scanIndex = 0;
    // Image start command but incorrect settings
    SimpleMissionItem invalidSimpleItem(planController(), false /* flyView */, _validTimeItem->missionItem());
...
void CameraSectionTest::_testScanForPhotoIntervalDistanceSection()
{
    QCOMPARE(_cameraSection->available(), true);
    int scanIndex = 0;
    QmlObjectListModel visualItems;
    _commonScanTest(_cameraSection);
    /*
    MAV_CMD_DO_SET_CAM_TRIGG_DIST	Mission command to set camera trigger distance for this flight. The camera is
    trigerred each time this distance is exceeded. This command can also be used to set the shutter integration time for
    the camera. Mission Param #1	Camera trigger distance (meters). 0 to stop triggering. Mission Param #2	Camera
    shutter integration time (milliseconds). -1 or 0 to ignore Mission Param #3	Trigger camera once immediately. (0 = no
    trigger, 1 = trigger) Mission Param #4	Empty Mission Param #5	Empty Mission Param #6	Empty Mission Param #7
    Empty
    */
    SimpleMissionItem* newValidDistanceItem =
        new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
    ...
    QCOMPARE(_cameraSection->cameraPhotoIntervalDistance()->rawValue().toInt(),
             (int)_validDistanceItem->missionItem().param1());
    visualItems.clear();
    scanIndex = 0;
    // Trigger distance command but incorrect settings
    SimpleMissionItem invalidSimpleItem(planController(), false /* flyView */, _validDistanceItem->missionItem());
...
void CameraSectionTest::_testScanForStartVideoSection()
{
    QCOMPARE(_cameraSection->available(), true);
    int scanIndex = 0;
    QmlObjectListModel visualItems;
    ...
    Mission Param #1	Reserved (Set to 0)
    Mission Param #2	Frequency CAMERA_CAPTURE_STATUS messages should be sent while recording (0 for no messages,
    otherwise frequency in Hz) Mission Param #3	Reserved
    */
    SimpleMissionItem* newValidStartVideoItem =
        new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
    ...
    SimpleMissionItem invalidSimpleItem(planController(), false /* flyView */, _validStartVideoItem->missionItem());
...
void CameraSectionTest::_testScanForStopVideoSection()
{
    QCOMPARE(_cameraSection->available(), true);
    int scanIndex = 0;
    QmlObjectListModel visualItems;
    ...
    Mission Param #1 Reserved (Set to 0)
    */
    SimpleMissionItem* newValidStopVideoItem =
        new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
    ...
    scanIndex = 0;
    // Trigger distance command but incorrect settings
    SimpleMissionItem invalidSimpleItem(planController(), false /* flyView */, _validStopVideoItem->missionItem());
    ...
    invalidSimpleItem.missionItem().setParam2(VIDEO_CAPTURE_STATUS_INTERVAL +
                                              1);  // must be VIDEO_CAPTURE_STATUS_INTERVAL
    visualItems.append(&invalidSimpleItem);
    QCOMPARE(_cameraSection->scanForSection(&visualItems, scanIndex), false);
    QCOMPARE(visualItems.count(), 1);
    QCOMPARE(_cameraSection->settingsSpecified(), false);
...
void CameraSectionTest::_testScanForStopPhotoSection()
{
    QCOMPARE(_cameraSection->available(), true);
    int scanIndex = 0;
    QmlObjectListModel visualItems;
    _commonScanTest(_cameraSection);
    SimpleMissionItem* newValidStopDistanceItem =
        new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
    SimpleMissionItem* newValidStopTimeItem =
        new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
    ...
    SimpleMissionItem validStopDistanceItem(planController(), false /* flyView */, false /* forLoad */);
    SimpleMissionItem validStopTimeItem(planController(), false /* flyView */, false /* forLoad */);
...
void CameraSectionTest::_testScanForTakePhotoSection()
{
    QCOMPARE(_cameraSection->available(), true);
    int scanIndex = 0;
    QmlObjectListModel visualItems;
    ...
    */
    SimpleMissionItem* newValidTakePhotoItem =
        new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
...
void CameraSectionTest::_resetSection()
{
...
void CameraSectionTest::_testScanForMultipleItems()
{
    QCOMPARE(_cameraSection->available(), true);
    int scanIndex = 0;
    QmlObjectListModel visualItems;
    ...
    rgActionItems << _validDistanceItem << _validTimeItem << _validStartVideoItem << _validStopVideoItem
                  << _validTakePhotoItem;
    ...
    for (SimpleMissionItem* actionItem : rgActionItems) {
        for (SimpleMissionItem* cameraItem : rgCameraItems) {
            SimpleMissionItem* item1 =
                new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
            item1->missionItem() = actionItem->missionItem();
            SimpleMissionItem* item2 =
                new SimpleMissionItem(planController(), false /* flyView */, false /* forLoad */);
...
void CameraSectionTest::_testSpecifiedGimbalValuesChanged()
{
    // specifiedGimbal[Yaw|Pitch]Changed SHOULD NOT signal if values are changed when specifyGimbal IS NOT set
    _cameraSection->setSpecifyGimbal(false);
    _spyCamera->clearAllSignals();
    _cameraSection->gimbalYaw()->setRawValue(_cameraSection->gimbalYaw()->rawValue().toDouble() + 1);
    QVERIFY(_spyCamera->notEmitted("specifiedGimbalYawChanged"));
    _cameraSection->gimbalPitch()->setRawValue(_cameraSection->gimbalPitch()->rawValue().toDouble() + 1);
    QVERIFY(_spyCamera->notEmitted("specifiedGimbalPitchChanged"));
    // specifiedGimbal[Yaw|Pitch]Changed SHOULD signal if values are changed when specifyGimbal IS set
    _cameraSection->setSpecifyGimbal(true);
    _spyCamera->clearAllSignals();
    _cameraSection->gimbalYaw()->setRawValue(_cameraSection->gimbalYaw()->rawValue().toDouble() + 1);
    QVERIFY(_spyCamera->emittedOnce("specifiedGimbalYawChanged"));
    _spyCamera->clearAllSignals();
    _cameraSection->gimbalPitch()->setRawValue(_cameraSection->gimbalPitch()->rawValue().toDouble() + 1);
    QVERIFY(_spyCamera->emittedOnce("specifiedGimbalPitchChanged"));
}
...
SimpleMissionItem* CameraSectionTest::createValidStopVideoItem(PlanMasterController* masterController)
{
    return new SimpleMissionItem(masterController,
                                 false,  // flyView
                                 MissionItem(0, MAV_CMD_VIDEO_STOP_CAPTURE, MAV_FRAME_MISSION, 0, qQNaN(), qQNaN(),
                                             qQNaN(), qQNaN(), qQNaN(), qQNaN(), true, false));
}

SimpleMissionItem* CameraSectionTest::createValidStopDistanceItem(PlanMasterController* masterController)
{
    return new SimpleMissionItem(
        masterController,
        false,  // flyView
...
SimpleMissionItem* CameraSectionTest::createValidStopTimeItem(PlanMasterController* masterController)
{
    return new SimpleMissionItem(masterController,
                                 false,  // flyView
                                 MissionItem(1, MAV_CMD_IMAGE_STOP_CAPTURE, MAV_FRAME_MISSION, 0, qQNaN(), qQNaN(),
                                             qQNaN(), qQNaN(), qQNaN(), qQNaN(), true, false));
}
...
UT_REGISTER_TEST(CameraSectionTest, TestLabel::Unit, TestLabel::MissionManager)
