...
void SpeedSectionTest::_testDirty()
{
    ...
    QVERIFY(_spySection->emittedOnce("dirtyChanged"));
    QCOMPARE(_spySection->argument<bool>("dirtyChanged"), false);
    QCOMPARE(_speedSection->dirty(), false);
    _spySection->clearAllSignals();
...
void SpeedSectionTest::_testSettingsAvailable()
{
    ...
    QVERIFY(_spySpeed->emittedOnce("specifyFlightSpeedChanged"));
    QCOMPARE(_spySpeed->argument<bool>("specifyFlightSpeedChanged"), true);
    ...
    QCOMPARE(_speedSection->settingsSpecified(), false);
    QVERIFY(_spySpeed->emittedOnce("specifyFlightSpeedChanged"));
    QCOMPARE(_spySpeed->argument<bool>("specifyFlightSpeedChanged"), false);
    QVERIFY(_spySection->emittedOnce("settingsSpecifiedChanged"));
...
void SpeedSectionTest::_testAppendSectionItems()
{
    ...
    QCOMPARE(seqNum, 1);
    MissionItem expectedSpeedItem(0, MAV_CMD_DO_CHANGE_SPEED, MAV_FRAME_MISSION,
                                  _controllerVehicle->multiRotor() ? 1 : 0,
                                  _speedSection->flightSpeed()->rawValue().toDouble(), -1, 0, 0, 0, 0, true, false);
...
void SpeedSectionTest::_testScanForSection()
{
    ...
    MissionItem validSpeedItem(0, MAV_CMD_DO_CHANGE_SPEED, MAV_FRAME_MISSION, _controllerVehicle->multiRotor() ? 1 : 0,
                               flightSpeed, -1, 0, 0, 0, 0, true, false);
    SimpleMissionItem simpleItem(planController(), false /* flyView */, validSpeedItem);
    ...
    MissionItem waypointMissionItem(0, MAV_CMD_NAV_WAYPOINT, MAV_FRAME_GLOBAL_RELATIVE_ALT, 0, 0, 0, 0, 0, 0, 0, true,
                                    false);
    SimpleMissionItem simpleWaypointItem(planController(), false /* flyView */, waypointMissionItem);
    simpleMissionItem = validSpeedItem;
...
void SpeedSectionTest::_testSpecifiedFlightSpeedChanged()
{
    ...
    _spySpeed->clearAllSignals();
    _speedSection->flightSpeed()->setRawValue(_speedSection->flightSpeed()->rawValue().toDouble() + 1);
    QVERIFY(_spySpeed->notEmitted("specifiedFlightSpeedChanged"));
    // specifiedFlightSpeedChanged SHOULD signal if flight speed is changed when specifyFlightSpeed IS set
    _speedSection->setSpecifyFlightSpeed(true);
    _spySpeed->clearAllSignals();
    _speedSection->flightSpeed()->setRawValue(_speedSection->flightSpeed()->rawValue().toDouble() + 1);
    QVERIFY(_spySpeed->emittedOnce("specifiedFlightSpeedChanged"));
}

UT_REGISTER_TEST(SpeedSectionTest, TestLabel::Unit, TestLabel::MissionManager)
