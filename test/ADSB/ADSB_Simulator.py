...
class Aircraft:
    def __init__(self):
        ...
        self.lat = random.uniform(34.8, 41.8)  # Latitude range for Greece
        self.lon = random.uniform(19.8, 29.6)  # Longitude range for Greece
        self.altitude = random.randrange(0, 40000)
        self.velocity = random.randrange(50, 600)
        self.heading = random.randrange(0, 360)
        self.verticalRate = random.randrange(-3000, 3000)  # Vertical rate in feet per minute
        ...
        # We use `random.randrange(0, 4096)` to generate values from 0 to 4095 (decimal),
        # and then format it as an octal string with `{value:04o}` to ensure the correct 4-digit format.
        self.squawk = f"{random.randrange(0, 4096):04o}"

    def generate_icao_address(self):
        # Generate a random ICAO address in the format of 6 hexadecimal digits
        return f"{random.randrange(0, 0x1000000):06X}"  # Generates a number from 0 to 16777215

    def update(self):
        # Update position to move in a more linear direction
        distance_movement = random.uniform(0.05, 0.15)  # Small constant step for smooth movement
        self.lat += distance_movement * math.cos(math.radians(self.heading))
        self.lon += distance_movement * math.sin(math.radians(self.heading))
        ...
        self.altitude += random.choice([-20, 20, 0])  # Climb or descend 20 feet or stay level
        self.altitude = max(0, self.altitude)  # Ensure altitude doesn't go below 0

        # Velocity can change with small gradual adjustments
        self.velocity += random.uniform(-2, 2)  # Smooth adjustments to speed
        ...
        heading_change = random.uniform(-1, 1)  # Slightly adjust heading
    ...
    def generate_adsb_message(self):
        ...
        msg_type = random.choice([1, 3, 4, 5, 6, 8])
        msg_parts = [
