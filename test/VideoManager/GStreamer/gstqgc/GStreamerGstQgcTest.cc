...
void GStreamerTest::_testQgcVideoSinkBinGpuZeroCopyProperty()
{
    ...
    (defined(Q_OS_LINUX) && defined(QGC_HAS_GST_DMABUF_GPU_PATH) && defined(QGC_HAS_GST_GLMEMORY_GPU_PATH))
    ...
    (defined(Q_OS_LINUX) && defined(QGC_HAS_GST_DMABUF_GPU_PATH) && defined(QGC_HAS_GST_GLMEMORY_GPU_PATH))
        ...
        // Caps are on the format capsfilter; qgcqvideosink is a GstVideoSink with no "caps" property.
...
#if defined(Q_OS_LINUX) && defined(QGC_HAS_GST_DMABUF_GPU_PATH) && defined(QGC_HAS_GST_GLMEMORY_GPU_PATH)
        // Linux must prefer QGC's direct DMABuf importer when it is compiled; otherwise gst-gl/glupload consumes the
        // stream first and DMABuf dispatch never gets exercised in normal gpu-zerocopy pipelines.
...
#elif defined(QGC_GST_BIN_USE_GLUPLOAD)
        // Linux desktop: glupload → glcolorconvert → format_capsfilter → qgcqvideosink (4 children).
        QCOMPARE(elementCount, 4);
...
{
    // Single-source-of-truth guard: every format buildCpuCapsString() advertises (shared with
    // buildGpuCapsString via kFormats) must map to a non-Invalid Qt format. A format added to the
    // caps that Qt can't render dead-ends negotiation at GST_FLOW_ERROR — fail in CI, not in the field.
