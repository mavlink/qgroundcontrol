...
if(NOT DEFINED GSTREAMER_APIS OR NOT DEFINED GSTREAMER_EXTRA_DEPS)
...
foreach(_comp IN LISTS QGCGStreamer_FIND_COMPONENTS)
    gstreamer_resolve_component("${_comp}" _ _api _ _)
    if(NOT _api)
        # Unknown component: derive api via CamelCase → snake_case (matches
        # gstreamer_build_apis_and_deps' fallback).
        string(REGEX REPLACE "([a-z0-9])([A-Z])" "\\1_\\2" _snake "${_comp}")
        string(TOLOWER "${_snake}" _snake)
        set(_api "api_${_snake}")
    endif()
    if(TARGET GStreamer::${_api})
        set(QGCGStreamer_${_comp}_FOUND TRUE)
        set(GStreamer_${_comp}_FOUND TRUE)
    endif()
endforeach()
...
if(NOT DEFINED _gst_available_basenames)
    gstreamer_scan_plugin_basenames(_gst_available_basenames "${GSTREAMER_PLUGIN_PATH}")
endif()
...
foreach(_qgc_req IN ITEMS GStreamer_ROOT_DIR GSTREAMER_LIB_PATH GSTREAMER_PLUGIN_PATH)
    if(NOT ${_qgc_req})
        message(FATAL_ERROR "QGCGStreamer: required variable ${_qgc_req} is not set after platform discovery.")
    endif()
endforeach()
