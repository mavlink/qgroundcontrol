...
# Tolerates extra trailing whitespace in the mandatory field (legacy "Core::: 1").
function(_gstreamer_registry_split ENTRY OUT_NAME OUT_API OUT_PC OUT_MAND)
    ...
    if(_n LESS 4)
        message(FATAL_ERROR "GSTREAMER_COMPONENT_REGISTRY: malformed entry '${ENTRY}' (need 4 colon-separated fields)")
    endif()
...
# gstreamer_registry_api_to_pc(<api_name> <out_var>)
# Returns the pkg-config module name for a registered api_ entry, or derives
# a name via the standard api_foo -> gstreamer-foo-1.0 convention as fallback.
# Kept for backward-compat; new callers should use gstreamer_resolve_component.
function(gstreamer_registry_api_to_pc API_NAME OUTPUT_VAR)
    gstreamer_resolve_component("${API_NAME}" _ _ _pc _)
    set(${OUTPUT_VAR} "${_pc}" PARENT_SCOPE)
endfunction()
...
function(gstreamer_build_apis_and_deps APIS_OUT DEPS_OUT)
    ...
    foreach(_comp IN LISTS ARGN)
        ...
        if(NOT _api)
            ...
            string(REGEX REPLACE "([a-z0-9])([A-Z])" "\\1_\\2" _snake "${_comp}")
            string(TOLOWER "${_snake}" _snake)
            set(_api "api_${_snake}")
        endif()
