...
if (ANDROID)
    ...
    # The repo ships its own GStreamer.java (android/src/org/freedesktop/gstreamer/)
    # with the correct init calls already enabled. Qt deploys it into android-build/src/
    # automatically, so we no longer read the SDK template here. The JAVA_INPUT variable
    # is intentionally left empty to skip the template-copy target below.
    set(JAVA_INPUT "")
endif()
...
if (GSTREAMER_IS_MOBILE AND TARGET GStreamerMobile)
    ...
    # Save and restrict library search to static archives in the SDK
    ...
    foreach(LOCAL_LIB IN LISTS _gst_mobile_core_libs)
        ...
        # System libraries are linked by name (the linker resolves them)
        if ("${LOCAL_LIB}" IN_LIST _gst_IGNORED_SYSTEM_LIBRARIES)
            target_link_libraries(GStreamerMobile PRIVATE "${LOCAL_LIB}")
            continue()
        endif()

        # Resolve library path via find_library. Use a cache variable keyed
        # by a sanitised form of the library name to avoid repeated lookups.
        string(MAKE_C_IDENTIFIER "_gst_${LOCAL_LIB}" _gst_cache_var)
        find_library(${_gst_cache_var}
            NAMES ${LOCAL_LIB}
            HINTS ${GSTREAMER_LIB_PATH}
            NO_DEFAULT_PATH
        )
        if(NOT ${_gst_cache_var})
            message(WARNING "GStreamerMobile: Library '${LOCAL_LIB}' not found in ${GSTREAMER_LIB_PATH}, skipping")
            continue()
        endif()

        if(APPLE)
            target_link_libraries(GStreamerMobile PRIVATE
                "-Wl,-force_load,${${_gst_cache_var}}"
            )
        else()
            target_link_libraries(GStreamerMobile PRIVATE
                "-Wl,--whole-archive,${${_gst_cache_var}},--no-whole-archive"
            )
        endif()
    endforeach()
    ...
    # PRIVATE includes for compiling GStreamerMobile's own sources
    # (e.g. gstreamer_android.c which includes <gst/gst.h>).
    target_include_directories(
        GStreamerMobile
        PRIVATE
            $<TARGET_PROPERTY:GStreamer::GStreamer,INTERFACE_INCLUDE_DIRECTORIES>
    ...
    if (G_IO_MODULES_LIBS)
        ...
        # Link each GIO module library (e.g. gioopenssl) from the SDK's gio/modules dir.
        # Replaces the old _gst_apply_link_libraries(OFF ...) macro from _FindGStreamer.cmake.
        set(_gio_save_suffixes ${CMAKE_FIND_LIBRARY_SUFFIXES})
        set(_gio_save_prefixes ${CMAKE_FIND_LIBRARY_PREFIXES})
        if(APPLE)
        ...
        endforeach()
        set(CMAKE_FIND_LIBRARY_SUFFIXES ${_gio_save_suffixes})
        set(CMAKE_FIND_LIBRARY_PREFIXES ${_gio_save_prefixes})
