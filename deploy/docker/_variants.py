...
def load_variants(path: Path = VARIANTS_JSON) -> list[DockerVariant]:
    ...
    for index, value in enumerate(payload["variants"]):
        ...
        for field in _STRING_FIELDS:
            ...
            if field != "package_pattern" and not value[field].strip():
        ...
        ci_variant = value["ci_variant"]
        if ci_variant in seen_ci_variants:
            raise ValueError(f"{context}.ci_variant duplicates {ci_variant!r}")
        seen_ci_variants.add(ci_variant)
