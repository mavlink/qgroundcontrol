...
def test_validation_uses_shared_container_action_for_every_platform():
    steps = WORKFLOW["jobs"]["validate"]["steps"]
    assert all("if" not in step and "run" not in step for step in steps)
    builds = [step for step in steps if step.get("uses") == "./.github/actions/qgc-dev"]
    assert len(builds) == 1
    assert builds[0]["with"] == {
        "platform": "${{ matrix.platform }}",
        "scope": "${{ matrix.scope }}",
        "revision": "${{ needs.plan.outputs.source }}",
    }
    checkout = next(step for step in steps if step.get("uses", "").startswith("actions/checkout@"))
    assert "submodules" not in checkout["with"]


def test_every_job_uses_runson_without_hosted_fallbacks():
    for name, job in WORKFLOW["jobs"].items():
        pool = (
            "${{ matrix.pool }}" if name in {"validate", "build-digests"} else "linux-x64-builder"
        )
        assert job["runs-on"] == "runs-on=${{ github.run_id }}/runner=" + pool


def test_bake_matrix_contains_only_native_on_demand_runson_pools(monkeypatch):
    monkeypatch.setattr(
        qgc_dev,
        "command",
        lambda *args: json.dumps(
            {
                "target": {
                    "qgc-dev": {"target": "qgc-dev", "platforms": ["linux/amd64", "linux/arm64"]}
                }
            }
        ),
    )
    assert qgc_dev.bake_matrix() == {
        "include": [
            {"platform": "linux/amd64", "pool": "linux-x64-builder", "scope": "linux-amd64"},
            {"platform": "linux/arm64", "pool": "linux-arm64-builder", "scope": "linux-arm64"},
        ]
    }
    runners = yaml.safe_load((ROOT / ".github/runs-on.yml").read_text())["runners"]
    for pool in qgc_dev.PLATFORMS.values():
        assert runners[pool]["spot"] is False
