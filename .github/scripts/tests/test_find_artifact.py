...
def test_optional_ambiguous_patterns_are_deterministic_but_required_patterns_fail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, gh_output: Path
) -> None:
    ...
    assert _run(monkeypatch, tmp_path, "--match", "apk=*.apk") == 0
    assert f"apk={linux}" in gh_output.read_text()

    gh_output.write_text("")
    assert _run(monkeypatch, tmp_path, "--match", "apk=*.apk", "--required") == 1
