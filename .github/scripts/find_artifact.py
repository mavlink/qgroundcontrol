...
def main() -> int:
    ...
    if args.pattern is not None:
        try:
            finder = find_unique if args.required else find_first
            artifact = finder(args.build_dir, args.pattern, recursive=not args.top_level)
    ...
    for name, pattern in args.match:
        try:
            finder = find_unique if args.required else find_first
            artifact = finder(args.build_dir, pattern, recursive=not args.top_level)
        except ValueError as error:
